$(function(){
	...
	$("[nc_type='privacyoption']").live('click',function(){
	    ...
	    ajaxget('index.php?m=m_snsindex&a='+op+'&id='+data_str.sid+'&privacy='+data_str.v);
	});
	//表单权限模块
	...
	$("[nc_type='sharegoods']").bind('click',function(){
		var data_str = $(this).attr('data-param');
	    eval( "data_str = "+data_str);
	    ajaxget('index.php?m=m_snsindex&a=sharegoods_one&dialog=1&gid='+data_str.gid);
	...
	$("#weibobtn_goods").die('click').live("click",function(){
		if($("#sharegoods_form").valid()){
			...
			if(cookienum >= max_recordnum && $("#sg_seccode").css('display') == 'none'){
				...
				$("#sg_seccode").find("[name='codeimage']").attr('src','index.php?m=seccode&a=makecode&nchash='+nchash+'&t=' + Math.random());
	...
	$("[nc_type='sharestore']").bind('click',function(){
		var data_str = $(this).attr('data-param');
	    eval( "data_str = "+data_str);	    
	    ajaxget('index.php?m=m_snsindex&a=sharestore_one&dialog=1&sid='+data_str.sid);
	});
	//删除分享和喜欢的商品
	$("[nc_type='delbtn']").die('click').live('click',function(){
        ...
        showDialog('您确定要删除该信息吗？','confirm', '', function(){
        	ajaxget('index.php?m=m_snsindex&a=delgoods&id='+data_str.sid+'&type='+data_str.tabtype);
			return false;
		});
	});
	//喜欢操作
	$("[nc_type='likebtn']").die('click').live('click',function(){
		...
		var data_str = $(this).attr('data-param');
        eval( "data_str = "+data_str);
        ajaxget('index.php?m=m_snsindex&a=editlike&inajax=1&id='+data_str.gid);
	});
    //展示和隐藏评论列表
	$("[nc_type='fd_commentbtn']").die('click').live('click',function(){
		...
		if($('#tracereply_'+data.txtid).css("display")=='none'){
			//加载评论列表
	        $("#tracereply_"+data.txtid).load('index.php?m=m_snshome&a=commenttop&type=0&id='+data.txtid+'&mid='+data.mid);
	...
	$("[nc_type='fd_del']").die('click').live('click',function(){
		var data_str = $(this).attr('data-param');
        eval("data_str = "+data_str);
        var url = "index.php?m=m_snsindex&a=deltrace&id="+data_str.txtid;
	...
	$("[nc_type='forwardbtn']").die('click').live('click',function(){
		...
		if($("#forwardform_"+data.txtid).valid()){
			...
			if(cookienum >= max_recordnum && $("#forwardseccode"+data.txtid).css('display') == 'none'){
				...
				$("#forwardseccode"+data.txtid).find("[name='codeimage']").attr('src','index.php?m=seccode&a=makecode&nchash='+nchash+'&t=' + Math.random());
