...
function login_dialog(){
    CUR_DIALOG = ajax_form('login','登录',MEMBER_SITE_URL+'/index.php?m=login&inajax=1',360,1);
}
...
function collect_store(fav_id,jstype,jsobj){
    $.get('index.php?m=index&a=login', function(result){
        if(result=='0'){
            login_dialog();
        }else{
            var url = 'index.php?m=m_favorite_store&a=favoritestore';
...
function collect_goods(fav_id,jstype,jsobj){
    $.get('index.php?m=index&a=login', function(result){
        if(result=='0'){
            ...
            var url = 'index.php?m=m_favorite_goods&a=favoritegoods';
            $.getJSON(url, {'fid':fav_id}, function(data){
                if (data.done)
            {
                showDialog(data.msg, 'succ','','','','','','','','',2);
...
function load_cart_information(){
	//$.getJSON(SITEURL+'/index.php?m=cart&a=ajax_load&callback=?', function(result){
...
//  	        obj.find('.list_detail').perfectScrollbar({suppressScrollX:true});
		        ...
		        $('#cartAddTip').html(result.cart_goods_num);
...
function drop_topcart_item(cart_id){
    $.getJSON(SITEURL+'/index.php?m=cart&a=del&cart_id='+cart_id+'&callback=?', function(result){
        if(result.state){
            var obj = $('.mod_minicart');
            //删除成功
            if(result.quantity == 0){
                ...
                $('li[ncTpye="cart_item_'+ cart_id+'"]').remove();
	         	...
	         	obj.find('.checkout_box .fl').html(html);
		        obj.find('.cart_num1').html(result.quantity);
    	        obj.find('.list_detail').perfectScrollbar('destroy');
    	        obj.find('.list_detail').perfectScrollbar({suppressScrollX:true});
    	        ...
    	        if ($('#rtoolbar_cartlist > ul').children().size() != result.quantity) {
    	        	$("#rtoolbar_cartlist").load(SHOP_SITE_URL+ '/index.php?m=cart&a=ajax_load&type=html');return ;
    	        }
            }
        }else{
            alert(result.msg);
...
function load_history_information(){
    $.getJSON(SITEURL+'/index.php?m=index&a=viewed_info', function(result){
        var obj = $('.head-user-menu .my-mall');
        if(result['m_id'] >0){
            if (typeof result['consult'] !== 'undefined') obj.find('#m_consult').html(result['consult']);
            if (typeof result['consult'] !== 'undefined') obj.find('#m_voucher').html(result['voucher']);
        }
        var goods_id = 0;
        var text_append = '';
        var n = 0;
...
(function($) {
    $.show_nc_login = function(options) {
        var settings = $.extend({}, {action:'/index.php?m=login&a=login&inajax=1', nchash:'', formhash:'' ,anchor:''}, options);
        var login_dialog_html = $('<div class="quick-login"></div>');
        var ref_url = document.location.href;
        var add_html = '<span class="other">';
        if (connect_qq == '1'){
            add_html += '<a href="'+MEMBER_SITE_URL+'/index.php?m=connect_qq" title="QQ账号登录" class="qq"><i></i></a>';
        }
        if (connect_sn == '1'){
            add_html += '<a href="'+MEMBER_SITE_URL+'/index.php?m=connect_sina" title="新浪微博账号登录" class="sina"><i></i></a>';
        }
        if (connect_wx == '1'){
            add_html += '<a href="javascript:void(0);" onclick="ajax_form(\'weixin_form\', \'微信账号登录\', \''+LOGIN_SITE_URL+'/index.php?m=connect_wx&a=index\', 360);" title="微信账号登录" class="wx"><i></i></a>';
        }
        add_html += '</span>';
        login_dialog_html.append('<form class="bg" method="post" id="ajax_login" action="'+LOGIN_SITE_URL+settings.action+'"></form>').find('form')
        	...
        	.append('<dl><dt>验证码</dt><dd><input type="text" size="10" maxlength="4" class="text fl w60" name="captcha"><img border="0" onclick="this.src=\'index.php?m=seccode&amp;op=makecode&amp;nchash='+settings.nchash+'&amp;t=\' + Math.random()" name="codeimage" title="看不清，换一张" src="index.php?m=seccode&amp;op=makecode&amp;nchash='+settings.nchash+'" class="fl ml10"><span>不区分大小写</span></dd></dl>')
        	.append('<ul><li>›&nbsp;如果您没有登录账号，请先<a class="register" href="'+LOGIN_SITE_URL+'/index.php?m=login&amp;op=register">注册会员</a>然后登录</li><li>›&nbsp;如果您<a class="forget" href="'+LOGIN_SITE_URL+'/index.php?m=login&amp;op=forget_password">忘记密码</a>？，申请找回密码</li></ul>')
...
(function($){
	$.fn.membershipCard = function(options){
		...
		return this.each(function(){
			...
			var _uri = SITEURL+'/index.php?m=m_card&callback=?&uid='+data_str.id+'&from='+options.type;
			var _dl = '';
			$this.qtip({
	            content: {
	                text: 'Loading...',
	                ajax: {
		                ...
		                success: function(data) {
		                	if(data){
	                			...
	                			.append('<a href="'+SHOP_SITE_URL+'/index.php?m=m_snshome&mid='+data.id+'" target="_blank">'+data.name+'</a>'+(data.truename != ''?'('+data.truename+')':''));
		                		//show membergrade
		                		...
		                		// avatar
		                		$('<dd class="avatar"><a href="'+SHOP_SITE_URL+'/index.php?m=m_snshome&mid='+data.id+'" target="_blank"><img src="'+data.avatar+'" /></a><dd>')
		                			.appendTo(_dl);
		                		// info
		                		var _info = '';
		                		...
		                		// ajax info
		                		if(data.url != ''){
			                		$.getJSON(data.url+'/index.php?m=m_card&a=mcard_info&uid='+data.id, function(d){
			                			if(d){
		                		...
		                		if(data.follow != 2){
		                			...
		                			$('<div class="send-msg"> <a href="'+MEMBER_SITE_URL+'/index.php?m=m_message&a=sendmsg&m_id='+data.id+'" target="_blank"><i></i>站内信</a> </div>').appendTo(_bottom);
		                		}
		...
		function follow(o){
			...
			$.getJSON(MEMBER_SITE_URL+'/index.php?m=m_snsfriend&a=addfollow&callback=?&mid='+data_str.mid, function(data){
				if(data){
		...
		function onfollow(o){
			var data_str = o.parent().attr('data-param');
			eval( "data_str = "+data_str);
			$.getJSON(MEMBER_SITE_URL+'/index.php?m=m_snsfriend&a=delfollow&callback=?&mid='+data_str.mid, function(data){
				if(data){
		...
		function circle_function(d){
			...
			$.each(d,function(i, n){
				rs.append('<div class="rank-div" title="'+n.circle_name+'圈等级'+n.cm_level+'，经验值'+n.cm_exp+'"><a href="'+CIRCLE_SITE_URL+'/index.php?m=group&c_id='+n.circle_id+'" target="_blank">'+n.circle_name+'</a><i></i><em class="rank-em rank-'+n.cm_level+'">'+n.cm_level+'</em></div>');
			});
			return rs;
		}
	...
	$.fn.nc_region = function(options) {
		...
		this.each(function() {
			...
			if ($inputArea.val() && /^\d+$/.test($inputArea.val())) {
				$.getJSON(SITEURL + "/index.php?m=index&a=json_area_show&area_id=" + $inputArea.val() + "&callback=?", function(data) {
		...
		function loadAreaArray(callback) {
			if (typeof nc_a === "undefined") {
				$.getJSON(SITEURL + "/index.php?m=index&a=json_area&src=" + settings.src + "&callback=?", function(data) {
					nc_a = data;
					callback()
				})
			} else {
...
function addcart(goods_id,quantity,callbackfunc) {
    ...
    var url = 'index.php?m=cart&a=add';
    ...
    $.getJSON(url, {'goods_id':goods_id, 'quantity':quantity}, function(data) {
        if (data != null) {
            if (data.state) {
                ...
                $("#rtoolbar_cartlist").load('index.php?m=cart&a=ajax_load&type=html');
            } else {
                alert(data.msg);
            }
        }
