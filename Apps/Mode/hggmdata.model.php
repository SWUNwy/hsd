...
class hggmdataModel extends Model{
	...
	private function OrderXmlBody($order)
	{
		...
		if($order['store_storage_id'] == 3){
			$ent_code = "50122604QT";
			$ent_name = "重庆圆通快递有限公司";
			$address = preg_replace("/(\s|\&nbsp\;|　|\xc2\xa0)/","_",$reciver_info['address']);
		}else{
			$address = preg_replace("/(\s|\&nbsp\;|　|\xc2\xa0)/","",$reciver_info['address']);
		}	
		$str = "<MessageBody>
				 <DTCFlow>
				  <ORDER_HEAD>
			        ...
			        <UNI_ESHOP_ENT_CODE>".$ent_code."</UNI_ESHOP_ENT_CODE>
			        <UNI_ESHOP_ENT_NAME>".$ent_name."</UNI_ESHOP_ENT_NAME>
