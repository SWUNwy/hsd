...
class queueLogic {
        ...
        foreach ($voucher_list as $store_id => $voucher_info) {
            ...
            if ($update) {
                ...
                if ($update) {
                    ...
                    $param['voucher_url'] = urlMember('m_voucher', 'index');
                    $send->send($param);
                } else {
     ...
     * @throws Exception
	 */
	public function editPoints($member_info,$points_list,$order_list)
	{
		if (empty($points_list) || !is_array($points_list)) return;
		$points_model = Model('points');
        foreach ($order_list as $order_id => $order) {
          	$points_model->savePointsLog('buy',array('pl_memberid'=>$member_info['member_id'],'pl_membername'=>$member_info['member_name'],'order_sn'=>$order['order_sn']),true,$points_list[$order['store_id']]['points']);
        }
		return callback(true);
	}
    ...
    public function editRptState($input_rpt_info, $pay_sn) {
        ...
        if ($update) {
            ...
            if ($update) {
                ...
                $param['rpacket_url'] = urlMember('m_redpacket', 'index');
                $send->send($param);
            } else {
