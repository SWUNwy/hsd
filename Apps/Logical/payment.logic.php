...
class paymentLogic {
    ...
    public function getRealOrderInfo($pay_sn, $member_id = null) {
        ...
        $order_pay_info = $model_order->getOrderPayList($condition);
        if(count($order_pay_info)!=count($pay_sn_arr)){
            return callback(false,'部份订单不存在');
        }
        $state = 1;
        foreach ($order_pay_info as $value) {
        ...
        if(count($pay_sn_arr)>1){
            ...
            $order_pay_info['subject'] = '实物订单_'.$pay_sn;  
        }
    ...
    public function getDingPayOrderInfo($pay_sn, $member_id = null) {
        ...
        if(count($pay_sn_arr)>1){
            ...
            $order_pay_info['subject'] = '实物订单_'.$out_trade_no;  
        }
        $order_pay_info['order_type'] = 'real_order';
