    ...
    <div class="ncs-store-info">
      ...
      <div class="service">
        <div class="displayed"><i></i>在线客服
          <div class="sub">
            <?php include template('Shop/callcenter');?>
          </div>
        </div>
      </div>
    </div>
...
$(function(){
	$('a[nctype="search_in_store"]').click(function(){
		...
		$('<input type="hidden" value="<?php echo $output['store_info']['store_id'];?>" name="store_id" /> <input type="hidden" name="a" value="goods_all" />').appendTo("#formSearch");
		$('#formSearch').submit();
	});
	...
	$('.favorites').mouseover(function(){
		var $this = $(this);
		if(storeTrends){
			$.getJSON('index.php?m=show_store&a=ajax_store_trend_count&store_id=<?php echo $output['store_info']['store_id'];?>', function(data){
	...
	$('a[nctype="share_store"]').click(function(){
		...
		login_dialog();
		<?php } else {?>
		ajax_form('sharestore', '分享店铺', 'index.php?m=m_snsindex&a=sharestore_one&inajax=1&sid=<?php echo $output['store_info']['store_id'];?>');
		<?php }?>
	});
