...
<div class="ncc-main">
  <div class="ncc-title">
    <h3>在线支付</h3>
    <h5><a href="index.php?m=m_vr_order&a=show_order&order_id=<?php echo $output['order_info']['order_id'];?>" target="_blank">查看订单详情</a></h5>
  </div>
  <form action="index.php?m=payment&a=vr_order" method="POST" id="buy_form">
   ...
   还需在线支付 <strong id="api_pay_amount"><?php echo ncPriceFormat($output['pay']['payd_diff_amount']);?></strong>元。</dd>
      <?php } ?>
        ...
        <?php if (!$output['pay']['member_paypwd']) {?>
              还未设置支付密码，<a href="<?php echo urlMember('m_security', 'auth', array('type' => 'modify_paypwd'));?>" target="_blank">马上设置</a>
        <?php } ?>
        </dd>
      <?php } ?>
      </dl>
    ...
    <div class="ncc-receipt-info">
      ...
      <div class="nopay"><?php echo $lang['cart_step2_paymentnull_1']; ?> <a href="<?php echo MEMBER_SITE_URL;?>/index.php?m=m_message&a=sendmsg&member_id=<?php echo $output['order']['seller_id'];?>"><?php echo $lang['cart_step2_paymentnull_2'];?></a> <?php echo $lang['cart_step2_paymentnull_3'];?></div>
      <?php } else {?>
      <div class="ncc-receipt-info-title">
        <h3>支付选择</h3>
      </div>
