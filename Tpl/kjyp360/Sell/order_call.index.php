...
<form method="get" action="index.php" target="_self">
  <table class="search-form">
    <input type="hidden" name="m" value="order_call" />
...
<span class="fr mr5"> <a href="index.php?m=store_order_print&order_id=<?php echo $order_id;?>" class="ncsc-btn-mini" target="_blank" title="打印发货单"/><i class="icon-print"></i>A5打印</a></span>
<span class="fr mr5"> <a href="index.php?m=store_order_print&a=small&order_id=<?php echo $order_id;?>" class="ncsc-btn-mini" target="_blank" title="打印发货单"/><i class="icon-print"></i>小票打印</a></span>
 </th>
    </tr>
    <?php $i = 0;?>
    ...
    <tr>
      ...
      <td class="bdl bdr" rowspan="<?php echo $order['goods_count'];?>"><p><?php echo $order['state_desc']; ?>
        ...
        <!-- 订单查看 -->
        <p><a href="index.php?m=store_order&a=show_order&order_id=<?php echo $order_id;?>" target="_blank"><?php echo $lang['store_order_view_order'];?></a></p>
        
        <!-- 物流跟踪 -->
        <p>
          <?php if ($order['if_deliver']) { ?>
          <a href='index.php?m=store_deliver&a=search_deliver&order_sn=<?php echo $order['order_sn']; ?>'><?php echo $lang['store_order_show_deliver'];?></a>
          <?php } ?>
        </p>
...
function refresh()
{
	$.ajax({
		type: "GET",
		url: SHOP_SITE_URL+'/index.php?m=order_call&a=getajaxorder',
		dataType:"json",
		async: true,
		success: function(data){
		    ...
		    {	
				for(var i = 0; i < order_list.length; i++)
				{
					...
					html_str += '<tr><th colspan="20"><span class="ml10">订单编号：<em>'+order_list[i].order_sn+'</em></span> <span>下单时间：<em class="goods-time">'+order_list[i].add_time+'</em></span> <span class="fr mr5"> <a href="index.php?m=store_order_print&amp;order_id='+order_list[i].order_id+'" class="ncsc-btn-mini" target="_blank" title="打印发货单"><i class="icon-print"></i>打印发货单</a></span></th></tr>';
					...
					for(var k = 0; k < order_list[i].goods_list.length; k++)
					{
						...
						if ((order_list[i].goods_count > 0 && k==0) || (order_list[i].goods_count) == 1 )
						{
							...
							//订单查看
							html_str += '	<p><a href="index.php?m=store_order&a=show_order&order_id='+order_list[i].order_id+'" target="_blank">订单详情</a></p>';
								
							//物流跟踪
							html_str += '		<p>';
							if(order_list[i].if_deliver)
							{
								html_str += '<a href=\'index.php?m=store_deliver&a=search_deliver&order_sn='+order_list[i].order_sn+'\'>查看物流</a>';
							}
							...
							if(order_list[i].if_send){
								html_str += '<p>';
								html_str += '<a class="ncsc-btn ncsc-btn-green mt10" href="index.php?m=store_deliver&a=send&order_id='+order_list[i].order_id+'">';
...
{
    if(length > 0){
		var url = SHOP_SITE_URL+'/index.php?m=order_call_print&a=index';
...
{
	if(length > 0){
		var url = SHOP_SITE_URL+'/index.php?m=order_call_print&a=small';
	    window.open(url,'_blank');
	}
}
...
function select_type(obj)
{
	...
	$.ajax({
		type: "GET",
		url: SHOP_SITE_URL+'/index.php?m=order_call&a=select_type&type='+type,
		dataType:"json",
		async: true,
		success: function(data){
			if(data == 1)
