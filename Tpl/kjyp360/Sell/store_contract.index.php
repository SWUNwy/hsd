                ...
                <div style="border: 1px solid #f5f5f5;background: none repeat scroll 0 0 #f5f5f5; margin: 20px 20px 0px 0px; height: 60px; padding: 30px;">
                    ...
                    <div style="float: left; margin-left: 15px; width: 320px;">
                        <div style="height: 35px;">
                            <em style="float:left; margin-bottom: 12px;font-size: 16px;font-weight: 700;"><?php echo $v['cti_name']; ?></em>
                                ...
                                <?php if($v['ct_auditstate_key'] == 'auditpass' && $v['ct_closestate_key'] == 'open' && $v['ct_joinstate_key'] == 'applying'){ //申请通过，待付款?>
                                    <span style="float:right;">审核通过，<a href="index.php?m=store_contract&a=applypay&itemid=<?php echo $v['cti_id']; ?>">点击支付保证金</a></span>
                                <?php } ?>
                                <?php if($v['ct_auditstate_key'] == 'auditfailure' && $v['ct_closestate_key'] == 'open'){//申请未通过?>
                                    <span style="float:right;"><?php echo $v['ct_auditstate_text']; ?>，<a href="index.php?m=store_contract&a=contractlog&itemid=<?php echo $v['cti_id']; ?>">点击查看详情</a></span>
                                <?php } ?>
                                ...
                                <?php if($v['ct_auditstate_key'] == 'costfailure' && $v['ct_closestate_key'] == 'open'){ //保证金审核失败?>
                                    <span style="float:right;"><?php echo $v['ct_auditstate_text']; ?>，<a href="index.php?m=store_contract&a=applypay&itemid=<?php echo $v['cti_id']; ?>">重新提交</a></span>
                                <?php } ?>
                                ...
                                <?php if($v['ct_state_sign'] == 'added' && $v['ct_quitstate_key'] == 'applyfailure' && $v['ct_closestate_key'] == 'open'){ //显示退出申请审核失败 ?>
                                    <span style="float:right;"><?php echo $v['ct_quitstate_text']; ?>，<a href="index.php?m=store_contract&a=contractlog&itemid=<?php echo $v['cti_id']; ?>">点击查看详情</a></span>
                                <?php } ?>
                        </div>
                        <div style="text-align: left;">
                            ...
                            <a href="index.php?m=store_contract&a=contractlog&itemid=<?php echo $v['cti_id']; ?>" style="margin-left: 30px;">查看服务详情</a>
                            <?php } ?>
...
$(document).ready(function(){
    $("[nc_type='applybtn']").click(function(){
        ...
        if(data_str){
            ...
            ajaxget('index.php?m=store_contract&a=ctiapply&itemid='+itemid);
        }
    });
    $("[nc_type='quitbtn']").click(function(){
        ...
        if(data_str){
            eval( "data_str = "+data_str);
            var itemid = parseInt(data_str.itemid);
            ajaxget('index.php?m=store_contract&a=ctiquit&itemid='+itemid);
        }
    });
});
</script> 
