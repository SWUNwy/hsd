...
<form method="get">
	<table class="search-form">
		<input type="hidden" id='m' name='m' value='store_voucher' />
		<input type="hidden" id='a' name='a' value='templatelist' />
		<tr>
			<td>&nbsp;</td>
      ...
      <tr class="bd-line">
        	...
        	<?php if($val['voucher_t_state']==$output['templatestate_arr']['usable'][0] && (!$val['voucher_t_giveout']) && (!$val['voucher_t_isbuild'])) {//代金券模板有效并且没有领取时可以编辑?>
        		<span> <a class="btn-bluejeans"
					href="index.php?m=store_voucher&a=templateedit&tid=<?php echo $val['voucher_t_id'];?>">
						<i class="icon-edit"></i>
						<p>编辑</p>
				</a>
			</span>
        	...
        	<!-- 代金券详细 -->
        	<span>
        	   <a class="btn-bluejeans" href="index.php?m=store_voucher&a=templateinfo&tid=<?php echo $val['voucher_t_id'];?>"><i class="icon-th-list"></i><p>详细</p></a>
			</span>
        	<?php if ((!$val['voucher_t_giveout']) && (!$val['voucher_t_isbuild'])){//该模板没有发放过代金券时可以删除?>
        	<span> <a class="btn-grapefruit" href="javascript:void(0)" onclick="ajax_get_confirm('<?php echo $lang['nc_ensure_del'];?>','index.php?m=store_voucher&a=templatedel&tid=<?php echo $val['voucher_t_id'];?>');"><i class="icon-trash"></i><p>删除</p></a></span>
        	<?php }?>
        </td>
		</tr>
      <?php }?>
