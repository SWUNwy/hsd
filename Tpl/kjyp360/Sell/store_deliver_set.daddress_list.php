...
<table class="ncsc-default-table" >
  ...
  <tbody>
    ...
    <tr class="bd-line">
      ...
      <td class="nscs-table-handle"><span><a href="javascript:void(0);" dialog_id="my_address_edit" dialog_width="640" dialog_title="<?php echo $lang['store_daddress_edit_address'];?>" nc_type="dialog" uri="index.php?m=store_deliver_set&a=daddress_add&address_id=<?php echo $address['address_id'];?>" class="btn-bluejeans"><i class="icon-edit"></i>
        <p><?php echo $lang['nc_edit'];?></p>
        </a></span><span> <a href="javascript:void(0)" onclick="ajax_get_confirm('<?php echo $lang['nc_ensure_del'];?>', 'index.php?m=store_deliver_set&a=daddress_del&address_id=<?php echo $address['address_id'];?>');" class="btn-grapefruit"><i class="icon-trash"></i>
...
$(function (){
	$('input[name="is_default"]').on('click',function(){
		$.get('index.php?m=store_deliver_set&a=daddress_default_set&address_id='+$(this).val(),function(result){})
	});
});
</script> 
