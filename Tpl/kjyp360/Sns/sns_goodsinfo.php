...
<div class="left-content">
  ...
  <div class="goback">
    <a href="index.php?m=m_snshome&a=shareglist&type=<?php if ($_GET['type'] == 'like'){?>like<?php }else { ?>share<?php } ?>&mid=<?php echo $output['sharegoods_info']['share_memberid'];?>">&#8249;&nbsp;<?php echo $lang['sns_goback_all_goods'];?></a>
  </div>
...
$(function(){
	...
	$("[nc_type='imgflipover']").bind('click',function(){
	    ...
	    eval( "data = "+data);
	    var type = $("#rtype").val();
	    window.location.href="index.php?m=m_snshome&a=goodsinfo&type="+type+"&mid=<?php echo $output['sharegoods_info']['share_memberid'];?>&id="+data.id;
	});
	//评论加载
	$("#tracereply_<?php echo $output['sharegoods_info']['share_id'];?>").load('index.php?m=m_snshome&a=commentlist&type=1&mid=<?php echo $output['sharegoods_info']['share_memberid'];?>&id=<?php echo $output['sharegoods_info']['share_id'];?>');


	// 追加
	$('a[nctype="add_share"]').click(function(){
	    eval( "data_str = "+$(this).attr('data-param'));
		ajax_form('add_share', '<?php echo $lang['sns_upload_treasure_buyer_show'];?>', SITEURL+'/index.php?m=m_snshome&a=add_share&type=refresh&sid='+data_str.sid+'&gid='+data_str.gid, 580);
	});
});
</script>