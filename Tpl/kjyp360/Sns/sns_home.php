...
<div class="left-content">
  ...
  <div class="tabmenu">
    <ul class="tab">
      <li class="active"><a mxf="sqde" href="index.php?m=m_snshome&a=shareglist<?php if(!empty($output['master_id'])){ echo '&mid='.$output['master_id']; }?>"><?php if ($output['relation'] == 3){echo $lang['sns_me']; }else {?>TA<?php }echo $lang['sns_share_of_goods'];?></a></li>
    </ul><span class="more"><a href="index.php?m=m_snshome&a=shareglist<?php if(!empty($output['master_id'])){ echo '&mid='.$output['master_id']; }?>"><?php echo $lang['nc_more'];?></a></span>
  </div>
  <ul class="sns-home-share">
    <?php foreach ($output['goodslist'] as $k=>$v){ ?>
    <li id="recordone_<?php echo $v['share_id'];?>"><a href="index.php?m=m_snshome&a=goodsinfo&mid=<?php echo $v['share_memberid'];?>&id=<?php echo $v['share_id'];?>" title="<?php echo $v['snsgoods_goodsname']?>" class="pic" style=" background-image:url(<?php echo cthumb($v['snsgoods_goodsimage'],240,$v['snsgoods_storeid']);?>)"> </a>
      <p class="pinterest-cmt"><?php echo $v['share_content'];?></p>
      <div class="ops"> <span class="ops-like" id="likestat_<?php echo $v['share_goodsid'];?>"> <a href="javascript:void(0);" nc_type="likebtn" data-param='{"gid":"<?php echo $v['share_goodsid'];?>"}' class="<?php echo $v['snsgoods_havelike']==1?'noaction':''; ?>"><i class="<?php echo $v['snsgoods_havelike']==1?'noaction':''; ?> pngFix"></i><?php echo $lang['sns_like'];?></a> <em nc_type="likecount_<?php echo $v['share_goodsid'];?>"><?php echo $v['snsgoods_likenum'];?></em> </span> <span class="ops-comment"><a href="index.php?m=m_snshome&a=goodsinfo&mid=<?php echo $v['share_memberid'];?>&id=<?php echo $v['share_id'];?>" title="<?php echo $lang['sns_comment'];?>"><i class="pngFix"></i></a><em><?php echo $v['share_commentcount'];?></em> </span> </div>
    </li>
    <?php }?>
  </ul>
  <?php }?>
  ...
  <div class="tabmenu">
    <ul class="tab">
      <li class="active"><a href="index.php?m=sns_album<?php if(!empty($output['master_id'])){ echo '&mid='.$output['master_id']; }?>"><?php if ($output['relation'] == 3){echo $lang['sns_me']; }else {?>TA<?php }echo $lang['sns_of_album'];?></a></li>
    </ul><span class="more"><a href="index.php?m=sns_album<?php if(!empty($output['master_id'])){ echo '&mid='.$output['master_id']; }?>"><?php echo $lang['nc_more'];?></a></span>
  </div>
  <ul class="sns-home-album" >
    <?php foreach($output['pic_list'] as $v){?>
    <li><a href="index.php?m=sns_album&a=album_pic_info&id=<?php echo $v['ap_id'];?>&class_id=<?php echo $v['ac_id']?>&mid=<?php echo $output['master_id'];?><?php if(!empty($_GET['sort'])){?>&sort=<?php echo $_GET['sort']; }?>" title="<?php echo $v['ap_name']?>" style=" background-image:url(<?php echo UPLOAD_SITE_URL.DS.ATTACH_MALBUM.DS.$output['master_id'].DS.str_ireplace('.', '_240.', $v['ap_cover']);?>)"> </a>
  ...
  <div class="tabmenu">
    <ul class="tab">
      <li class="active"><a href="index.php?m=m_snshome&a=storelist<?php if(!empty($output['master_id'])){ echo '&mid='.$output['master_id']; }?>"><?php if ($output['relation'] == 3){echo $lang['sns_me']; }else {?>TA<?php }echo $lang['sns_share_of_store'];?></a></li>
    </ul>
    <span class="more"><a href="index.php?m=m_snshome&a=storelist<?php if(!empty($output['master_id'])){ echo '&mid='.$output['master_id']; }?>"><?php echo $lang['nc_more'];?></a></span>
  </div>
  ...
  <div class="tabmenu">
    <ul class="tab">
      <li class="active"><a href="index.php?m=m_snshome&a=trace<?php if(!empty($output['master_id'])){ echo '&mid='.$output['master_id']; }?>"><?php if ($output['relation'] == 3){echo $lang['sns_me']; }else {?>TA<?php }echo $lang['sns_of_fresh_news'];?></a></li>
    </ul><span class="more"><a href="index.php?m=m_snshome&a=trace<?php if(!empty($output['master_id'])){ echo '&mid='.$output['master_id']; }?>"><?php echo $lang['nc_more'];?></a></span>
  </div>
    ...
    <li nc_type="tracerow_<?php echo $v['trace_id']; ?>">
      <dl class="fd-wrap">
        <dt><img src="<?php echo getMemberAvatarForID($v['trace_memberid']);?>"  data-param="{'id':<?php echo $v['trace_memberid'];?>}" nctype="mcard">
          <h3><a href="index.php?m=m_snshome&mid=<?php echo $v['trace_memberid'];?>" target="_blank" data-param="{'id':<?php echo $v['trace_memberid'];?>}" nctype="mcard"><?php echo $v['trace_membername'];?><?php echo $lang['nc_colon'];?></a><?php echo parsesmiles($v['trace_title']);?></h3>
        </dt>
        <dd>
          ...
          <div class="fd-forward">
            ...
            <div class="stat"><span><?php echo $lang['sns_original_forward']; ?><?php echo $v['trace_orgcopycount']>0?"({$v['trace_orgcopycount']})":''; ?></span>&nbsp;&nbsp; <span><a href="index.php?m=m_snshome&a=traceinfo&mid=<?php echo $v['trace_originalmemberid'];?>&id=<?php echo $v['trace_originalid'];?>" target="_blank"><?php echo $lang['sns_original_comment']; ?><?php echo $v['trace_orgcommentcount']>0?"({$v['trace_orgcommentcount']})":''; ?></a></span> </div>
            <?php }?>
            <?php }?>
          </div>
          <?php } else {?>
        ...
        <div id="forward_<?php echo $v['trace_id'];?>" style="display:none;">
          <div class="forward-widget">
            <div class="forward-edit">
              <form id="forwardform_<?php echo $v['trace_id'];?>" method="post" action="index.php?m=m_snsindex&a=addforward&type=<?php echo $output['type'];?>&irefresh=1">
...
$(function(){
	...
	$("[nc_type='storedelbtn']").live('click',function(){
        ...
        showDialog('<?php echo $lang['nc_common_op_confirm'];?>','confirm', '', function(){
        	ajax_get_confirm('','index.php?m=m_snsindex&a=delstore&id='+data_str.sid);
			return false;
		});
	});
});
