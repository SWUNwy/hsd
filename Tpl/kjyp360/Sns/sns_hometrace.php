...
<?php if ($output['is_owner']){ ?>
    ...
    <div class="release-content"><span class="arrow"></span>
      <form id="weiboform" method="post" action="index.php?m=m_snsindex&a=addtrace">
        ...
        <div id="weiboseccode" class="weiboseccode">
          ...
          <a href="javascript:void(0)" class="ml5 fl"><img src="" title="<?php echo $lang['wrong_checkcode_change'];?>" name="codeimage" border="0" id="codeimage" onclick="this.src='index.php?m=seccode&a=makecode&nchash=<?php echo $output['nchash'];?>&t=' + Math.random()" /></a>
...
$(function(){
	...
	$('#friendtrace').lazyshow({url:"index.php?m=m_snshome&a=tracelist&mid=<?php echo $output['master_info']['member_id'];?>&page=1",'iIntervalId':true});
	...
	$("#weibobtn").bind('click',function(){			
		if($("#weiboform").valid()){
			...
			if(cookienum >= max_recordnum && $("#weiboseccode").css('display') == 'none'){
				//显示验证码
				$("#weiboseccode").show();
				$("#weiboseccode").find("#codeimage").attr('src','index.php?m=seccode&a=makecode&nchash=<?php echo $output['nchash'];?>&t=' + Math.random());
