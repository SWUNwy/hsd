...
<ul class="fd-list">
  ...
  <li nc_type="tracerow_<?php echo $output['traceinfo']['trace_id']; ?>">
    <dl class="fd-wrap">
      <dt><a href="index.php?m=m_snshome&mid=<?php echo $output['traceinfo']['trace_memberid'];?>" target="_blank"> <img src="<?php if ($output['traceinfo']['trace_memberavatar']!='') { echo UPLOAD_SITE_URL.'/'.ATTACH_AVATAR.DS.$output['traceinfo']['trace_memberavatar']; } else {  echo UPLOAD_SITE_URL.'/'.ATTACH_COMMON.DS.C('default_user_portrait'); } ?>"> </a>
        <h3><a href="index.php?m=m_snshome&mid=<?php echo $output['traceinfo']['trace_memberid'];?>" target="_blank"><?php echo $output['traceinfo']['trace_membername'];?><?php echo $lang['nc_colon'];?></a><?php echo parsesmiles($output['traceinfo']['trace_title']);?></h3>
      ...
      <dd>
        ...
        <div class="stat"> <span><?php echo $lang['sns_original_forward']; ?><?php echo $output['traceinfo']['trace_orgcopycount']>0?"({$output['traceinfo']['trace_orgcopycount']})":''; ?></span>&nbsp;&nbsp; <span><a href="index.php?m=m_snshome&a=traceinfo&mid=<?php echo $output['traceinfo']['trace_originalmemberid'];?>&id=<?php echo $output['traceinfo']['trace_originalid'];?>" target="_blank"><?php echo $lang['sns_original_comment']; ?><?php echo $output['traceinfo']['trace_orgcommentcount']>0?"({$output['traceinfo']['trace_orgcommentcount']})":''; ?></a></span> </div>
        <?php }?>
        <?php } else {?>
      ...
      <div id="forward_<?php echo $output['traceinfo']['trace_id'];?>" style="display:none;">
        <div class="forward-widget">
          <div class="forward-edit">
            <form id="forwardform_<?php echo $output['traceinfo']['trace_id'];?>" method="post" action="index.php?m=m_snsindex&a=addforward">
...
$(function(){
	$("#tracereply_<?php echo $output['traceinfo']['trace_id'];?>").load('index.php?m=m_snshome&a=commentlist&mid=<?php echo $output['traceinfo']['trace_memberid'];?>&type=0&id=<?php echo $output['traceinfo']['trace_id'];?>');
});
</script>
