...
<div class="comment-widget">
  <div class="comment-edit">
    <form id="commentform_<?php echo $output['tid'];?>" method="post" action="index.php?m=m_snsindex&a=addcomment">
  ...
  <ul class="comment-list">
    ...
    <li nc_type="commentrow_<?php echo $v['comment_id']; ?>"> <a target="_blank" href="index.php?m=m_snshome&mid=<?php echo $v['comment_memberid'];?>"><img width="30" height="30" class="clogo" src="<?php if ($v['comment_memberavatar']!='') { echo UPLOAD_SITE_URL.'/'.ATTACH_AVATAR.DS.$v['comment_memberavatar']; } else {  echo UPLOAD_SITE_URL.'/'.ATTACH_COMMON.DS.C('default_user_portrait'); } ?>" onload="javascript:DrawImage(this,30,30);"></a>
      <div class="detail">
        <p class="words"><a target="_blank" href="index.php?m=m_snshome&mid=<?php echo $v['comment_memberid'];?>" class="name"><?php echo $v['comment_membername'];?></a><?php echo $lang['nc_colon']; ?><?php echo parsesmiles($v['comment_content']);?></p>
        <p class="other">
  ...
  <div class="more"><a target="_blank" href="index.php?m=m_snshome&a=traceinfo&mid=<?php echo $output['master_info']['member_id'];?>&id=<?php echo $output['tid'];?>"><?php echo $lang['sns_comment_more'];?></a></div>
...
$(function(){
	...
	$("[nc_type='comment_del']").live('click',function(){
		...
		showDialog('<?php echo $lang['nc_common_op_confirm'];?>','confirm', '', function(){
	        ...
	        ajax_get_confirm('','index.php?m=m_snsindex&a=delcomment&id='+data_str.cid);
			return false;
		});
	});
	//评论提交
	$("[nc_type='commentbtn']").die().live('click',function(){
		...
		if($("#commentform_"+data.txtid).valid()){			
			...
			if(cookienum >= <?php echo $output['max_recordnum'];?> && $("#commentseccode"+data.txtid).css('display')=="none"){
				...
				$("#commentseccode"+data.txtid).find("[name='codeimage']").attr('src','index.php?m=seccode&a=makecode&nchash='+nchash+'&t=' + Math.random());
