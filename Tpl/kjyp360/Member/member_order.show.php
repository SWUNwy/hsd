  ...
  <div class="ncm-order-info">
    ...
    <div class="ncm-order-condition">
      ...
      <ul>
        ...
        <li>1. 您尚未对该订单进行支付，请<a href="index.php?m=buy&a=pay&pay_sn=<?php echo $output['order_info']['pay_sn'];?>" class="ncbtn-mini ncbtn-bittersweet"><i></i>支付订单</a>以确保商家及时发货。</li>
    ...
    <div class="ncm-order-condition">
      ...
      <ul>
        ...
        <li>2. 将感兴趣的<a href="index.php?m=m_favorite_goods&a=fglist" class="ncbtn-mini">收藏商品</a>加入购物车进行购买。</li>
    ...
    <?php } ?>
    <?php } ?>
    <div class="mall-msg">有疑问可咨询<a href="javascript:void(0);" onclick="ajax_form('mall_consult', '平台客服', '<?php echo urlShop('m_mallconsult', 'add_mallconsult', array('inajax' => 1));?>', 640);"><i class="icon-comments-alt"></i>平台客服</a></div>
  </div>
      ...
      <tbody>
        ...
        <tr class="bd-line">
          ...
          <td class="tl"><dl class="goods-name">
              ...
              <dd>退款单号：<a target="_blank" href="index.php?m=m_refund&a=view&refund_id=<?php echo $output['refund_all']['refund_id'];?>"><?php echo $output['refund_all']['refund_sn'];?></a></dd>
              <?php }else if($goods['extend_refund']['refund_type'] == 1) {?>
              <dd>退款单号：<a target="_blank" href="index.php?m=m_refund&a=view&refund_id=<?php echo $goods['extend_refund']['refund_id'];?>"><?php echo $goods['extend_refund']['refund_sn'];?></a></dd>
              <?php }else if($goods['extend_refund']['refund_type'] == 2) {?>
              <dd>退货单号：<a target="_blank" href="index.php?m=m_return&a=view&return_id=<?php echo $goods['extend_refund']['refund_id'];?>"><?php echo $goods['extend_refund']['refund_sn'];?></a></dd>
              <?php } ?>
            </dl></td>
          <td></td>
          ...
          <td><!-- 退款 -->
            ...
            <p><a href="index.php?m=m_refund&a=add_refund&order_id=<?php echo $output['order_info']['order_id']; ?>&goods_id=<?php echo $goods['rec_id']; ?>">退款/退货</a></p>
            <?php }?>
            
            <!-- 投诉 -->
            
            <?php if ($output['order_info']['if_complain']){ ?>
            <p><a href="index.php?m=m_complain&a=complain_new&order_id=<?php echo $output['order_info']['order_id']; ?>&goods_id=<?php echo $goods['rec_id']; ?>">交易投诉</a></p>
            <?php } ?></td>
          ...
          <td rowspan="<?php echo $output['order_info']['goods_count'];?>"><?php if ($output['order_info']['if_lock']) { ?>
            ...
            <p><a href="index.php?m=m_refund&a=add_refund_all&order_id=<?php echo $output['order_info']['order_id']; ?>" class="ncbtn">订单退款</a></p>
            <?php } ?>
            
            <!-- 收货 -->
            
            <?php if ($output['order_info']['if_receive']) { ?>
            <p><a href="javascript:void(0)" class="ncbtn" nc_type="dialog" dialog_id="buyer_order_confirm_order" dialog_width="480" dialog_title="<?php echo $lang['member_order_ensure_order'];?>" uri="index.php?m=m_order&a=change_state&state_type=order_receive&order_sn=<?php echo $output['order_info']['order_sn']; ?>&order_id=<?php echo $output['order_info']['order_id']; ?>" id="order_action_confirm"><?php echo $lang['member_order_ensure_order'];?></a></p>
            <?php } ?>
            
            <!-- 评价 -->
            
            <?php if ($output['order_info']['if_evaluation']) { ?>
            <p><a class="ncbtn ncbtn-aqua" href="index.php?m=m_evaluate&a=add&order_id=<?php echo $output['order_info']['order_id']; ?>"><?php echo $lang['member_order_want_evaluate'];?></a></p>
            <?php } ?>
...
$(function(){
    $('#show_shipping').on('hover',function(){
    	...
    	$.getJSON('index.php?m=m_order&a=get_express&e_code=<?php echo $output['order_info']['express_info']['e_code']?>&shipping_code=<?php echo $output['order_info']['shipping_code']?>&t=<?php echo random(7);?>',function(data){
    		if(data){
