...
<div class="wrap">
  ...
  <div class="ncm-favorite-store">
    <div class="store-info">
      ...
      <div class="handle"><a href="javascript:void(0);" nc_type="sharestore" data-param='{"sid":"<?php echo $favorites['store']['store_id'];?>"}' title="<?php echo $lang['nc_snsshare'];?>"><i class="icon-share-alt"></i></a><a href="javascript:void(0)" onclick="ajax_get_confirm('<?php echo $lang['nc_ensure_del'];?>', 'index.php?m=m_favorite_store&a=delfavorites&type=store&fav_id=<?php echo $favorites['fav_id'];?>');" class="ml10" title="<?php echo $lang['nc_del'];?>"><i class="icon-trash"></i></a></div>
    </div>
    <div class="store-goods"><a href="<?php echo urlShop('m_favorite_store', 'more', array('store_id' => $favorites['store']['store_id']));?>" class="more">查看更多></a>
      ...
      <?php if (!empty($favorites['goods']['new'])) {?>
        <ul>
         <?php foreach ($favorites['goods']['new'] as $goods) {?>
          <li>
            <div class="goods-thumb"><a href="index.php?m=goods&goods_id=<?php echo $goods['goods_id'];?>" title="<?php echo $goods['goods_name'];?>" target="_blank"><img src="<?php echo thumb($goods, 240);?>" /></a></div>
...
$(function(){	
	...
	$('.show-tab a').click(function(){
	    ...
	    var _href = '<?php echo urlShop('m_favorite_store', 'more');?>';
