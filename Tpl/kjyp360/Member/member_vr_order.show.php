...
<div class="ncm-oredr-show">
  <div class="ncm-order-info">
      ...
      <div class="content">
        <dl>
          ...
          <dd><?php echo $output['order_info']['buyer_phone'];?>
            ...
            <a href="javascript:void(0);" class="ncbtn-mini ncbtn-bittersweet" dialog_id="vr_code_resend" dialog_title="发送电子兑换码" dialog_width="480" nc_type="dialog" uri="<?php echo urlShop('m_vr_order', 'resend',array('buyer_phone'=>$output['order_info']['buyer_phone'],'order_id'=>$output['order_info']['order_id']));?>"><i class="icon-mobile-phone"></i>重新发送</a>
            <?php } ?>
          </dd>
        </dl>
        <dl class="line">
        ...
        <dl class="line">
          <dt>商&#12288;&#12288;家：</dt>
          <dd><?php echo $output['order_info']['store_name'];?><a href="javascript:void(0)" class="ncbtn" nc_type="dialog" dialog_width="800" dialog_title="查看地图" dialog_id="show_map" uri="index.php?m=show_map&a=index&w=440&h=400&store_id=<?php echo $output['order_info']['store_id'];?>"><i class="icon-map-marker"></i>商家所在信息</a></dd>
        </dl>
      </div>
    </div>
    ...
    <div class="ncm-order-condition">
      ...
      <ul>
        <li>1. 您尚未对该订单进行支付，请<a href="index.php?m=buy_virtual&a=pay&order_id=<?php echo $output['order_info']['order_id']; ?>" class="ncbtn-mini ncbtn-bittersweet"><i></i>支付订单</a>以确保及时获取电子兑换码。</li>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td> <?php if($output['order_info']['if_refund']){ ?>
              <a href="index.php?m=m_vr_refund&a=add_refund&order_id=<?php echo $output['order_info']['order_id']; ?>">退款</a>
              <?php } ?></td>
          <td class="bdl"><?php echo $output['order_info']['state_desc'];?></td>
          <td class="bdl bdr">

          <?php if ($output['order_info']['if_cancel']){ ?>
            <a href="javascript:void(0)" class="ncbtn ncbtn-grapefruit" nc_type="dialog" dialog_width="480" dialog_title="取消订单" dialog_id="buyer_order_cancel_order" uri="index.php?m=m_vr_order&a=change_state&state_type=order_cancel&order_id=<?php echo $output['order_info']['order_id'];?>"  id="order_action_cancel"><i class="icon-ban-circle"></i>取消</a>
            <?php }?>

            <!-- 评价 -->
            <?php if ($output['order_info']['if_evaluation']) { ?>
            <p><a class="ncbtn ncbtn-aqua" href="index.php?m=m_evaluate&a=add_vr&order_id=<?php echo $output['order_info']['order_id']; ?>"><?php echo $lang['member_order_want_evaluate'];?></a></p>
            <?php } ?>
