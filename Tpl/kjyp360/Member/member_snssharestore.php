...
<div class="dialog-share">
  <form method="post" action="index.php?m=m_snsindex&a=sharestore&<?php if($_GET['irefresh'] == 1){?>irefresh=1<?php }?>" id="sharestore_form">
    ...
    <div class="share-widget">同步分享到：
        <ul class="s-app">
          ...
          <?php }?>
          <?php }?>
          <div class="tip"><span class="arrow"></span>点亮图标分享商品到热门社交网络去~<a target="_blank" href="<?php echo urlMember('m_sharemanage');?>"><?php echo $lang['sharebind_alsosharesetting'];?></a></div>
          <?php }?>
        </ul>
      </div>
...
$(function(){
	...
	$("#weibobtn_store").click(function(){
		if($("#sharestore_form").valid()){
			...
			if(cookienum >= max_recordnum && $("#ss_seccode").css('display') == 'none'){
				...
				$("#ss_seccode").find("[name='codeimage']").attr('src','index.php?m=seccode&a=makecode&nchash='+nchash+'&t=' + Math.random());
	...
	$("[nc_type='bindbtn']").bind('click',function(){
	    ...
	    if(isbind == '1'){//已经绑定
		    ...
		    window.open(MEMBER_SITE_URL + '/api.php?m=sharebind&type='+data_str.apikey);
		}
	});
	$("[nc_type='finishbtn']").live('click',function(){
	    ...
	    eval( "data_str = "+data_str);
		//验证是否绑定成功
		var url = '<?php echo urlMember('m_sharemanage', 'checkbind');?>';
