...
<div class="wrap">
  ...
  <div class="ncm-favorite-store-more">
    ...
    <div class="store-list">
      ...
      <div class="store-info"><a href="<?php echo urlShop('m_favorite_store', 'more', array('store_id' => $favorites['store']['store_id']));?>"> <span class="store-pic"><img src="<?php echo getStoreLogo($favorites['store']['store_avatar']);?>"/></span> <span class="store-name"><?php echo $favorites['store']['store_name'];?></span></a> </div>
      <?php }?>
    </div>
    <div class="item-list">
      ...
      <div class="show-tab" data-sid="<?php echo $output['store_info']['store_id'];?>">
        <a href="<?php if ($output['count']['new'] == 0) {?>javascript:;<?php } else {echo urlShop('m_favorite_store', 'more', array('store_id' => $output['store_info']['store_id'], 'sign' => 'new')) ;}?>" class="<?php if ($_GET['sign'] == 'new' || empty($_GET['sign'])) {?>current<?php }?> <?php if ($output['count']['new'] == 0) {?>disabled<?php }?>">店铺新品</a>
        <a href="<?php if ($output['count']['promotion'] == 0) {?>javascript:;<?php } else { echo urlShop('m_favorite_store', 'more', array('store_id' => $output['store_info']['store_id'], 'sign' => 'promotion')) ;}?>" class="<?php if ($_GET['sign'] == 'promotion') {?>current<?php }?> <?php if ($output['count']['promotion'] == 0) {?>disabled<?php }?>">优惠促销</a>
        <a href="<?php if ($output['count']['hot'] == 0) {?>javascript:;<?php } else { echo urlShop('m_favorite_store', 'more', array('store_id' => $output['store_info']['store_id'], 'sign' => 'hot')) ;}?>" class="<?php if ($_GET['sign'] == 'hot') {?>current<?php }?> <?php if ($output['count']['hot'] == 0) {?>disabled<?php }?>">热销商品</a>
       </div>
    ...
    <div id="page-more"><a href="<?php echo urlShop('m_favorite_store', 'more', array('store_id' => $output['store_info']['store_id'], 'sign' => $_GET['sign']));?>&curpage=2"></a> </div>
  </div>
</div>
