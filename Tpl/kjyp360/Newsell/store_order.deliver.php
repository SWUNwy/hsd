...
<form method="get" action="index.php" target="_self">
  <table class="search-form">
    <input type="hidden" name="m" value="store_deliver" />
  ...
  <tbody>
    ...
    <tr>
      <th colspan="21"><span class="ml5"><?php echo $lang['store_order_order_sn'].$lang['nc_colon'];?><strong><?php echo $order['order_sn']; ?></strong></span><span><?php echo $lang['store_order_add_time'].$lang['nc_colon'];?><em class="goods-time"><?php echo date("Y-m-d H:i:s",$order['add_time']); ?></em></span>
        ...
        <a href="index.php?m=store_deliver&a=search_deliver&order_sn=<?php echo $order['order_sn']; ?>" class="ncbtn-mini"><i class="icon-compass"></i><?php echo $lang['store_order_show_deliver'];?></a>
        <?php }?>
        <a href="index.php?m=store_order&a=order_print&order_id=<?php echo $order['order_id'];?>" target="_blank"  class="ncbtn-mini" title="<?php echo $lang['store_show_order_printorder'];?>"/><i class="icon-print"></i><?php echo $lang['store_show_order_printorder'];?></a></span></th>
    </tr>
    <?php $i = 0; ?>
      ...
      <td class="bdl bdr order-info w500" rowspan="<?php echo $order['goods_count'];?>"><dl>
        ...
        <dl>
          ...
          <dd>
            ...
            <span><a href="index.php?m=store_deliver&a=send&order_id=<?php echo $order['order_id'];?>" class="ncbtn-mini ncbtn-mint fr"><i class="icon-truck"></i><?php echo $lang['store_order_send'];?></a></span>
            <?php } elseif ($order['order_state'] == ORDER_STATE_SEND){?>
            <span>
            <a href="javascript:void(0)" class="ncbtn-mini ncbtn-bittersweet ml5 fr" uri="index.php?m=store_deliver&a=delay_receive&order_id=<?php echo $order['order_id']; ?>" dialog_width="480" dialog_title="延迟收货" nc_type="dialog" dialog_id="seller_order_delay_receive" id="order<?php echo $order['order_id']; ?>_action_delay_receive" /><i class="icon-time"></i></i>延迟收货</a>
            <a href="index.php?m=store_deliver&a=send&order_id=<?php echo $order['order_id'];?>" class="ncbtn-mini ncbtn-aqua fr"><i class="icon-edit"></i><?php echo $lang['store_deliver_modify_info'];?></a>
            </span>
            <?php }?>
            <?php }?>
          </dd>
