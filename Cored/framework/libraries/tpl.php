...
class Tpl{
	...
	public static function showpage($page_name='',$layout='',$time=2000){
		if (!defined('TPL_NAME')) define('TPL_NAME','kjyp360');
		self::getInstance();
			...
			//默认是带有布局文件
			if (empty($layout)){
				$layout = 'Layouts'.DS.self::$layout_file.'.php';
			}else {
				$layout = 'Layouts'.DS.$layout.'.php';
			}

			$layout_file = BASE_PATH.'/Tpl/'.TPL_NAME.DS.$layout;
			if(empty(self::$tpl_quna)){
				$tpl_file = BASE_PATH.'/Tpl/'.TPL_NAME.DS.$tpl_dir.$page_name.'.php';
				}else{
					$tpl_file = BASE_PATH.'/modules/'.self::$tpl_quna.'/Tpl/'.TPL_NAME.DS.$tpl_dir.$page_name.'.php';
					}
			...
			if (file_exists($tpl_file)){
				...
				if ($layout != ''){
					if (file_exists($layout_file)){
						...
						$error = '模版不存在:'.'ThinkPHP'.DS.$layout.' 没有找到';
				...
				$error = '模版不存在:'.'ThinkPHP'.DS.$tpl_dir.$page_name.'.php'.' 没有找到';
				throw_exception($error);
			}
	}
