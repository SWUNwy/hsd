...
function showMessage($msg,$url='',$show_type='html',$msg_type='succ',$is_show=1,$time=2000){
	...
	switch ($show_type){
		...
		case 'exception':
			...
			echo 'html{ overflow-y: scroll; }';
			echo 'body{ background: #fff; font-family: "微软雅黑"; color: #333; font-size: 16px; }';
			echo 'img{ border: 0; }';
			echo '.error{ padding: 24px 48px; }';
			echo '.face{ font-size: 100px; font-weight: normal; line-height: 120px; margin-bottom: 12px; }';
			echo 'h1{ font-size: 32px; line-height: 48px; }';
			echo '.error .content{ padding-top: 10px}';
			echo '.error .info{ margin-bottom: 12px; }';
			echo '.error .info .title{ margin-bottom: 3px; }';
			echo '.error .info .title h3{ color: #000; font-weight: 700; font-size: 16px; }';
			echo '.error .info .text{ line-height: 24px; }';
			echo '.copyright{ padding: 12px 48px; color: #999; }';
			echo '.copyright a{ color: #000; text-decoration: none; }';			
			echo '</style>';
			echo '</head>';
			echo '<body>';
			...
			echo '<p><a title="官方网站" href="http://www.thinkphp.cn">ThinkPHP</a><sup>3.2.3</sup> { Fast & Simple OOP PHP Framework } -- [ WE CAN DO IT JUST THINK ]</p>';
			echo '</div>';			
			echo '</body>';
			echo '</html>';
			exit;
...
function Model($model = null, $base_path = null){
	...
	$file_name = $base_path.'/Mode/'.$model.'.model.php';
...
function Logic($model = null, $base_path = null){
    ...
    if (!is_null($model) && isset($_cache[$cache_key])) return $_cache[$cache_key];
    $base_path = $base_path == null ? BASE_DATA_PATH : $base_path;
    $file_name = $base_path.'/Logical/'.$model.'.logic.php';
...
function getCMSArticleUrl($article_id) {
    if(URL_MODEL) {
        ...
        return CMS_SITE_URL.DS.'index.php?m=article&a=article_detail&article_id='.$article_id;
    }
}
...
function getCMSPictureUrl($picture_id) {
    if(URL_MODEL) {
        ...
        return CMS_SITE_URL.DS.'index.php?m=picture&a=picture_detail&picture_id='.$picture_id;
    }
}
...
function getCMSSpecialUrl($special_id) {
    return CMS_SITE_URL.DS.'index.php?m=special&a=special_detail&special_id='.$special_id;
}
...
function getShopSpecialUrl($special_id) {
    return SHOP_SITE_URL.DS.'index.php?m=special&a=special_detail&special_id='.$special_id;
}
...
function template($tplpath){
	if (strpos($tplpath,':') !== false){
		...
		return BASE_PATH.'/Tpl/'.TPL_NAME.'/'.$tplpath.'.php';
	}
}
...
function write_file($filepath, $data, $mode = null)
{
    ...
    $data = "<?php defined('ByShopKJYP') or exit('Access Invalid!'); return ".$data.";";
...
function nc_class($classname = null){
	...
	throw_exception('类错误:  类 '.$classname.' 不存在!');
}
...
function urlWinxin($act = '', $op = '', $args = array()){
    return url($act, $op, $args, false, SHOP_SITE_URL.'/weidian');
}
