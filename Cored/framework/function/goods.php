 ...
 *
 *
 * @package    function* www.kjyp360.com网店运维技术交流中心为你提供售后服务 以便你更好的了解
 */
...
function thumb($goods = array(), $type = ''){
    ...
    if ( !file_exists(BASE_UPLOAD_PATH.'/'.ATTACH_GOODS.'/'.$store_id.'/'.$file)) {    
        if(!file_exists(BASE_UPLOAD_PATH.'/'.ATTACH_GOODS.'/1/'.$file))        {
            if(file_exists(BASE_UPLOAD_PATH.'/'.ATTACH_GOODS.'/21/'.$file)){
                $store_id=21;
            }
            else{    
                return UPLOAD_SITE_URL.'/'.defaultGoodsImage($type);
...
function cthumb($file, $type = '', $store_id = false) {
    ...
    if ( !file_exists(BASE_UPLOAD_PATH . '/' . ATTACH_GOODS . '/' . $store_id . '/' . ($type == '' ? $file : str_ireplace('.', '_' . $type . '.', $file)) )) {    
        if(!file_exists(BASE_UPLOAD_PATH . '/' . ATTACH_GOODS . '/1/' . ($type == '' ? $file : str_ireplace('.', '_' . $type . '.', $file)) ))        {
            if(file_exists(BASE_UPLOAD_PATH . '/' . ATTACH_GOODS . '/21/' . ($type == '' ? $file : str_ireplace('.', '_' . $type . '.', $file)) ))
            {
                $store_id=21;
            }
            else
...
//新相册
function newcthumb($file,$type)
{
    $model_picture=Model("picture");
    $picture_info=$model_picture->getPictureInfo(array('pic_path'=>$file));
    if($type==60)
    {
        $file='x1_'.$file;
    }
    ...
    {
        $file= $picture_info['pic_atalog']."/".$file;
    }
    if (is_file(BASE_ROOT_PATH. '/picture/uploadfile/'.$file)) 
    {
        return  '/picture/uploadfile/'.$file;
    }
    else 
    {
        return UPLOAD_SITE_URL.'/'.defaultGoodsImage($type);
...
defined("ByShopKJYP") || exit("Access Invalid!");
