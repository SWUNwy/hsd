...
class pointcartControl extends BasePointShopControl {
    ...
    public function addOp() {
        ...
        if (!$data['state']){
            switch ($data['error']){
                case 'ParameterError':
                    echo json_encode(array('done'=>false,'msg'=>$data['msg'],'url'=>'index.php?m=pointprod&a=plist')); die;
                    break;
                default:
    ...
    public function step1Op(){
        ...
        if (!$data['state']){
            showDialog($data['msg'],'index.php?m=pointprod','error');
        }
    ...
    public function step2Op() {
        ...
        if (!$data['state']){
            showDialog($data['msg'],'index.php?m=pointcart','error');
        }
        ...
        if (!$data['state']){
            showDialog($data['msg'],'index.php?m=pointcart&a=step1','error');
        }
        $order_id = $data['data']['order_id'];
        @header("Location:index.php?m=pointcart&a=step3&order_id=".$order_id);
    }
    /**
     * 流程第三步
     */
