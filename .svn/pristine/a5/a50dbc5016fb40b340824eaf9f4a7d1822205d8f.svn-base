...
class searchControl extends SystemControl{
    ...
    private $_links = array(
        array('url'=>'m=search&a=index','text'=>'默认搜索'),
        array('url'=>'m=search&a=hot','text'=>'热门搜索')
    );
    ...
    public function hot_addOp() {
        ...
        if (chksubmit()) {
            if (count($search_list) >= 10) {
                showMessage('最多可设置10个热搜词','index.php?m=search&a=hot');
            }
            ...
            if ($result){
                showMessage('保存成功','index.php?m=search&a=hot');
            }else {
                showMessage('保存失败');
            }
        }
    ...
    public function hot_editOp() {
        ...
        if (!chksubmit()) {
            ...
            $result = $model_setting->updateSetting(array('rec_search'=>serialize($search_list)));
            if ($result){
                showMessage('编辑成功','index.php?m=search&a=hot');
            }
            showMessage('编辑失败');
        }
