...
class pointsControl extends SystemControl{
    ...
    public function __construct(){
        ...
        //判断系统是否开启积分功能
        if (C('points_isuse') != 1){
            showMessage(Language::get('admin_points_unavailable'),'index.php?m=setting','','error');
        }
    }
    ...
    public function addpointsOp(){
        if (chksubmit()){
            ...
            if (!is_array($member_info) || count($member_info)<=0){
                showMessage(Language::get('admin_points_userrecord_error'),'index.php?m=points&a=addpoints','','error');
            }

            $pointsnum = intval($_POST['pointsnum']);
            if ($_POST['operatetype'] == 2 && $pointsnum > intval($member_info['member_points'])){
                showMessage(Language::get('admin_points_points_short_error').$member_info['member_points'],'index.php?m=points&a=addpoints','','error');
            }
            ...
            if ($result){
                ...
                showMessage(Language::get('nc_common_save_succ'),'index.php?m=points&a=addpoints');
            }else {
                showMessage(Language::get('nc_common_save_fail'),'index.php?m=points&a=addpoints','','error');
            }
        }else {
			Tpl::setDirquna('shop');
