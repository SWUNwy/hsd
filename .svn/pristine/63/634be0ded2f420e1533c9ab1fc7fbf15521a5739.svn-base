$(document).ready(function(){
	...
	$('span[nc_type="inline_edit"]').click(function(){
		...
		$('<input type="text">')
                        ...
                        if(event.keyCode == 13)
                        {
							...
							//column 修改字段名
							//value 修改内容
                            $.get('index.php?m='+m+'&a=ajax',{branch:ajax_branch,id:s_id,column:s_name,value:$(this).attr('value')},function(data){
                                if(data === 'false')
                                {
                                    alert('名称已经存在，请您换一个');
	...
	$('img[nc_type="inline_edit"]').click(function(){
		...
		$.get('index.php?m='+m+'&a=ajax',{branch:ajax_branch,id:i_id,column:i_name,value:i_val},function(data){
		if(data == 'true')
			{
				if(i_val == 0)
				{
 ...
 $.fn.inline_edit= function(options) {
     ...
     function onClick() {
         ...
         .blur(function(){
             ...
             if(new_value != '') {
                 $.get('index.php?m='+settings.m+'&a='+settings.a,{branch:s_name,id:column_id,value:new_value},function(data){
                     data = $.parseJSON(data);
                     if(data.result) {
     ...
     function onClick() {
         ...
         $btn_submit.click(function(){
             ...
             if(new_value !== '' && new_value !== old_value) {
                 $.post('index.php?m=' + settings.m + '&a=' + settings.a, {branch:s_name,id:column_id, value:new_value}, function(data) {
                     data = $.parseJSON(data);
                     if(data.result) {
                         $span.text(new_value);
                     } else {
