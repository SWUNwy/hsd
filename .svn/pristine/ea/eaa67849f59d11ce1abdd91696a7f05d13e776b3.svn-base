...
class goodsLogic {
	...
	*/
    ...
    private function _initCommonGoodsByParam($param, $store_id, $store_name, $store_state) {
        ...
        $common_array['goods_storage_id']   = $param['g_storage_id'];
        $common_array['goods_wmstype']      = $param['goods_wmstype'];        
        $common_array['expiry_date']        = $param['query_start_date']!=""?strtotime($param['query_start_date']):0;
    ...
    private function _addGoods($param, $common_id, $common_array) {
        ...
        if (is_array($param['spec'])) {
            foreach ($param['spec'] as $value) {
                ...
                $goods['goods_storage_id']  = $common_array['goods_storage_id'];
                $goods['goods_wmstype']	    = $common_array['goods_wmstype'];
            	$goods['expiry_date']		= $common_array['query_start_date']!=""?strtotime($common_array['query_start_date']):0;
        ...
        if (is_array($param['spec'])) {
            foreach ($param['spec'] as $value) {
                ...
                if (!empty($goods_info)) {
                    ...
                    $goods['goods_storage_id']  = $common_array['goods_storage_id'];
                    $goods['goods_wmstype']     = $common_array['goods_wmstype'];
                    $goods['expiry_date']       = $param['query_start_date']!=""?strtotime($param['query_start_date']):0;
                    $goods['color_id']          = intval($value['color']);
                    // 虚拟商品不能有赠品
                    if ($common_array['is_virtual'] == 1) {
                        $goods['have_gift']    = 0;
        ...
        foreach ($_POST['img'] as $key => $value) {
            ...
            foreach ($value as $v) {
                ...
                if ($k == 0 || $v['default'] == 1) {
					...
					$model_goods->editGoodsCommon(array('goods_image'=>$update_array['goods_image']), array('goods_commonid'=>$common_id));
                }
                $tmp_insert = array();
                $tmp_insert['goods_commonid']   = $common_id;
                $tmp_insert['store_id']         = $store_id;
