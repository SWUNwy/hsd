...
function update_data(id) {//更新
	var get_text = $.ajax({
		type: "POST",
		url: 'index.php?m=web_config&a=code_update',
...
function update_dialog(id) {//初始化数据
	switch (id) {
		...
		case "recommend_list":
			...
			break;
		case "brand_list":
			$("#show_brand_list").load('index.php?m=web_config&a='+id);//查询数据
...
function get_goods_class() {//查询子分类
	...
	if (gc_id>0) {
		$.get('index.php?m=web_config&a=category_list&id='+gc_id, function(data) {
			$(data).each(function(){
				var obj = $(this);
...
function get_recommend_goods() {//查询商品
	...
	if (gc_id>0 || goods_name!='') {
		$("#show_recommend_goods_list").load('index.php?m=web_config&a=recommend_list&'+$.param({'id':gc_id,'goods_name':goods_name }));
	}
}
...
function select_recommend_goods(goods_id) {//商品选择
	...
	text_append += '<a href="'+SHOP_SITE_URL+'/index.php?m=goods&goods_id='+goods_id+'" target="_blank">';
...
function get_recommend_brand() {//查询品牌
	...
	if (brand_name!='' || brand_initial!='') {
		$("#show_brand_list").load('index.php?m=web_config&a=brand_list&'+$.param({'brand_name':brand_name,'brand_initial':brand_initial }));
	}
}
...
function get_goods_list(){//查询商品
	...
	if (gc_id>0 || goods_name!='') {
		$("#show_sale_goods_list").load('index.php?m=web_config&a=goods_list&'+$.param({'id':gc_id,'goods_order':goods_order,'goods_name':goods_name }));
	}
}
...
function select_sale_goods(goods_id){//商品选择
	...
	text_append += '</span></div>';
	text_append += '<div class="goods-name">';
	text_append += '<a href="'+SITEURL+'/index.php?m=goods&goods_id='+goods_id+'" target="_blank">';
  	text_append += goods_name+'</a>';
	text_append += '</div>';
