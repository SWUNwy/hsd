...
$(function(){
    $("#flexigrid").flexigrid({
        url: 'index.php?m=goods&a=get_xml&type=<?php echo $output['type'];?>',
        colModel : [
    ...
    $('#ncsubmit').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?m=goods&a=get_xml&'+$("#formSearch").serialize(),query:'',qtype:''}).flexReload();
    });

    // 高级搜索重置
    $('#ncreset').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?m=goods&a=get_xml'}).flexReload();
        $("#formSearch")[0].reset();
    });
});
...
function fg_csv(ids) {
    id = ids.join(',');
    window.location.href = $("#flexigrid").flexSimpleSearchQueryString()+'&a=export_csv&type=<?php echo $output['type'];?>&id=' + id;
}


//商品下架
function fg_lonkup(ids) {
    _uri = "index.php?m=goods&a=goods_lockup&id=" + ids;
    CUR_DIALOG = ajax_form('goods_lockup', '违规下架理由', _uri, 640);
}
//分销设置
function fg_invite(ids) {
    _uri = "index.php?m=goods&a=goods_invite&id=" + ids;
    CUR_DIALOG = ajax_form('goods_invite', '分销设置', _uri, 640);
}
//添加评论
function fg_pinglun(gids) {
    _uri = "index.php?m=goods&a=add&goods_id=" + gids;
    CUR_DIALOG = ajax_form('goods_pinlun', '为此商品添加评论', _uri, 640);
}

function fg_sku(commonid) {
    CUR_DIALOG = ajax_form('login','商品"' + commonid +'"的SKU列表','index.php?m=goods&a=get_goods_sku_list&commonid=' + commonid, 580);
}
// 删除
function fg_del(id) {
    if(confirm('删除后将不能恢复，确认删除这项吗？')){
        $.getJSON('index.php?m=goods&a=goods_del', {id:id}, function(data){
            if (data.state) {
...
function fg_verify(ids) {
    _uri = "index.php?m=goods&a=goods_verify&id=" + ids;
