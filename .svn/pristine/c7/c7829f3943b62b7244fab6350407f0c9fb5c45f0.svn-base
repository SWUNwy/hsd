...
class m_messageControl extends BaseMemberControl {
    ...
    public function savemsgOp() {
        ...
        if (chksubmit()) {
            ...
            if($_GET['type'] == 'sns_board'){
                ...
                showDialog(Language::get('home_message_send_success'),'index.php?m=m_message&a=privatemsg','succ');
            }
        }
    }
    /**
    ...
    public function savereplyOp() {
        ...
        if (!$isallowsend){
            if($_GET['inajax'] == 1){
                ...
                showMessage(Language::get('home_message_noallowsend'),'index.php?m=m_message&a=message','html','error');
            }
        }
        if ($_POST['form_submit'] == 'ok') {
            $message_id = intval($_POST["message_id"]);
            if ($message_id <=0){
                showMessage(Language::get('wrong_argument'),'index.php?m=m_message&a=message','html','error');
            }
            $obj_validate = new Validate();
            $obj_validate->validateparam = array(
            ...
            if($_GET['inajax'] == 1){
                ...
                showMessage(Language::get('home_message_send_success'),'index.php?m=m_message&a=privatemsg');
            }
        }else {
            if($_GET['inajax'] == 1){
    ...
    private function profile_menu($menu_key='') {
        $menu_array = array(
            array('menu_key'=>'message', 'menu_name'=>Language::get('home_message_received_message'),'menu_url'=>'index.php?m=m_message&a=message'),
            array('menu_key'=>'private', 'menu_name'=>Language::get('home_message_private_message'), 'menu_url'=>'index.php?m=m_message&a=privatemsg'),
            array('menu_key'=>'system',  'menu_name'=>Language::get('home_message_system_message'),  'menu_url'=>'index.php?m=m_message&a=systemmsg'),
            array('menu_key'=>'ggmsg',  'menu_name'=>'系统公告',  'menu_url'=>'index.php?m=m_message&a=ggmsg'),
            array('menu_key'=>'close',   'menu_name'=>Language::get('home_message_close'),   'menu_url'=>'index.php?m=m_message&a=personalmsg'),
            array('menu_key'=>'setting', 'menu_name'=>'接收设置', 'menu_url'=>urlMember('m_message', 'setting'))
        );
        if($menu_key == 'sendmsg') {
            $menu_array[] = array('menu_key'=>'sendmsg','menu_name'=>Language::get('home_message_send_message'),'menu_url'=>'index.php?m=m_message&a=sendmsg');
