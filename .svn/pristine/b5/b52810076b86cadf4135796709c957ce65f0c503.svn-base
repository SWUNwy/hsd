...
<div class="page">
  ...
  <div class="ncap-form-default">
    ...
    <div class="bot">
      ...
      <a class="ncap-btn-big ncap-btn-green mr10" onclick="if (confirm('审核后将无法撤销，进入下一步付款环节，确认审核吗?')){return true;}else{return false;}" href="index.php?m=vr_bill&a=bill_check&ob_id=<?php echo $_GET['ob_id'];?>">审核</a>
      <?php }elseif ($output['bill_info']['ob_state'] == BILL_STATE_SYSTEM_CHECK){?>
      <a class="ncap-btn-big ncap-btn-blue mr10" href="index.php?m=vr_bill&a=bill_pay&ob_id=<?php echo $_GET['ob_id'];?>">付款完成</a>
      <?php }elseif ($output['bill_info']['ob_state'] == BILL_STATE_SUCCESS){?>
      <a class="ncap-btn-big ncap-btn-green" target="_blank" href="index.php?m=vr_bill&a=bill_print&ob_id=<?php echo $_GET['ob_id'];?>">打印</a>
      <?php }?>
    </div>
  </div>
  <div class="homepage-focus" nctype="sellerTplContent">
    <div class="title">
      <ul class="tab-base nc-row">
        <li><a class="<?php echo $_GET['query_type'] == '' ? 'current' : '';?>" href="index.php?m=vr_bill&a=show_bill&ob_id=<?php echo $_GET['ob_id'];?>&query_type=">已使用</a></li>
        <li><a class="<?php echo $_GET['query_type'] == 'timeout' ? 'current' : '';?>" href="index.php?m=vr_bill&a=show_bill&ob_id=<?php echo $_GET['ob_id'];?>&query_type=timeout">已过期</a></li>
      </ul>
    </div>
...
$(function(){
    $('#ncsubmit').click(function(){
    	$('input[name="a"]').val('show_bill');$('#formSearch').submit();
    });
});
</script> 
