...
class buyLogic {
    ...
    public function getBuyStep1Data($member_id, $data, $orderdiscount = 0) {
        ...
        list($store_cart_list,$store_goods_total,$store_goods_tax) = $this->_logic_buy_1->calcCartList($store_cart_list);
    ...
    private function _createOrderStep1() {
        ...
        $this->_order_data['order_from'] = $post['order_from'];
    ...
    private function _createOrderStep3() {
        ...
        if (!$input_is_book) {
            ...
            //加上运费后,重新计算
            list($store_cart_list,$store_goods_total,$store_goods_tax) = $this->_logic_buy_1->calcCartList($store_cart_list,$store_freight_total);
            //得到有效的代金券
        ...
        $address_info=$this->_order_data['input_address_info'];
        //判断地址是否包含特殊单词,如母婴,跨境之类   
        $addressfun = loadfunc("address");
        $res = checkAddress($address_info['address']);   
        if($res['state']){
        ...
        //$where['d_id'] = 1;
        $where['add_time'] = array('time',array($start,$end));
        $where['order_state'] = array('gt',0);
        $where['member_idcard'] =$address_info['member_idcard'] ;
        $order_sum=$model_order->getOrderCount($where);
        if($order_sum > $list_setting['order_same_idcard'])
        {             
            throw new Exception($list_setting['order_same_idcard_msg']);
        }
        //相同地址一个月只能下6单
        $model=Model();
        $on="orders.order_id=order_common.order_id";
        $count=$model->table('orders,order_common')->join('inner')->on($on)->where(array('orders.d_id'=>1,'orders.add_time'=>array('time',array($start,$end)),'orders.order_state'=>array('gt',0),'order_common.reciver_info'=>array('like',"%".$address_info['address']."%")))->select();
       
        if(count($count)>$list_setting['order_same_address'])
        {
            //print_r($address_info);
            //echo $address_info['address'];
            //exit();
            throw new Exception($list_setting['order_same_address_msg']);
        }
         
        //相同电话一个月只能下6单
        $model=Model();
        $on="orders.order_id=order_common.order_id";
        $count=$model->table('orders,order_common')->join('inner')->on($on)->where(array('orders.d_id'=>1,'orders.add_time'=>array('time',array($start,$end)),'orders.order_state'=>array('gt',0),'order_common.reciver_info'=>array('like',"%".$address_info['mob_phone']."%")))->select();
        if(count($count)>$list_setting['order_same_phone'])
        {
            throw new Exception($list_setting['order_same_phone_msg']);
        }
	
        //保存数据
        $this->_order_data['store_goods_total'] = $store_goods_total;
        $this->_order_data['store_goods_tax'] = $store_goods_tax;
    ...
    private function _createOrderStep4() {
        ...
        $member_idcard=unserialize($reciver_info);
        // 加价购换购商品 店铺分组
...
//             $should_goods_total = $store_final_order_total[$store_id]-$store_freight_total[$store_id]+$promotion_total;
            ...
            $member_name = $member_idcard['phone'];
            if($_SESSION['store_id']>0){
				$member_store_id =  $_SESSION['store_id']!=$store_id?$_SESSION['store_id']:$store_id;
				$member_store_name = $_SESSION['store_name'];
			}else{
				$model_store = Model("store");
                $store_info = $model_store->getStoreInfoByID($store_id);
				$member_store_id = $store_id;
                $member_store_name = $store_info['store_name'];      
			}
            $order['order_sn'] = $this->_logic_buy_1->makeOrderSn($order_pay_id);
            $order['pay_sn'] = $pay_sn;
            $order['store_id'] = $member_store_id;
            $order['store_name'] = $member_store_name;
            ...
            $order['order_tax'] = $store_goods_tax[$store_id];
            $order['goods_amount'] = $order['order_amount'] - $order['shipping_fee']-$order['order_tax'];
            $order['order_from'] = $order_from == '' ? 1 : $order_from;
			...
			$order['member_idcard']= $member_idcard['member_idcard'];
            //商品仓库
            $order['store_storage_id'] = $this->getStorage($goods_list);
...
//             }
            ...
            foreach ($goods_list as $goods_info) {
                ...
                if (!intval($goods_info['bl_id'])) {
					...
					$order_goods[$i]['goods_wmstype'] = $goods_invit['goods_wmstype'];
                    ...
                    $order_goods[$i]['goods_pay_price'] = $goods_total - $promotion_value < 0 ? 0 : $goods_total - $promotion_value+ $goods_info['goods_freight'];
    ...
    private function getStorage($goods_list){
        ...
        $storage_id = 0;
        foreach ($goods_list as $goods_info) {
            $goods = $gModel->field("goods_storage_id")->where(array('goods_id'=>$goods_info['goods_id']))->find();           
            if($storage_id == 0){
                $storage_id = $goods['goods_storage_id'];
            }else{
                if($storage_id!=$goods['goods_storage_id']){
                    throw new Exception('商品列表中的商品只能为一个仓库');
                }
            }    
        }
        return $storage_id;
