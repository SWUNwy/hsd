...
class m_refundControl extends BaseMemberControl {
    ...
    public function add_refundOp(){
        ...
        if ($order_id < 1 || $goods_id < 1) {//参数验证
            showDialog(Language::get('wrong_argument'),'index.php?m=m_order&a=index','error');
        }
        $condition = array();
        ...
        if (chksubmit() && $goods_id > 0){
            ...
            $show_url = 'index.php?m=m_return&a=index';
            ...
            if ($refund_array['refund_type'] != '2') {
                ...
                $show_url = 'index.php?m=m_refund&a=index';
            }
    ...
    public function add_refund_allOp(){
        ...
        if (chksubmit()) {
            ...
            if ($state) {
                $model_refund->editOrderLock($order_id);
                showDialog(Language::get('nc_common_save_succ'),'index.php?m=m_refund&a=index','succ');
            } else {
                showDialog(Language::get('nc_common_save_fail'),'reload','error');
            }
        }
