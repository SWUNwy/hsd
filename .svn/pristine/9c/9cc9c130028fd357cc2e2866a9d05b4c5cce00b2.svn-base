...
$('#edit_reciver').on('click',function(){
    ...
    var url = SITEURL+'/index.php?m=buy&a=load_addr';
...
function showShippingPrice(city_id,area_id) {
	$('#buy_city_id').val('');
    $.post(SITEURL + '/index.php?m=buy&a=change_addr', {'freight_hash':'<?php echo $output['freight_hash'];?>',city_id:city_id,'area_id':area_id}, function(data){
...
function showProductChain(city_id) {
	...
	$.post(SITEURL+'/index.php?m=buy&a=change_chain',{chain_id:chain_id,product:product.join('-')},function(data){
		if (data.state == 'success') {
