...
class dbControl extends SystemControl{
	...
	public function dbOp(){
		...
		if ($_POST['form_submit'] == 'ok'){
			...
			showMessage($lang['db_index_backup_to_wait'],'index.php?m=db&a=db&step=1');
		}
		/**
		 * 数据库备份步骤
		 */
		if (intval($_GET['step']) >= 1){
			...
			if (
				...
				$url = array(
					array(
						'url'=>'index.php?m=db&a=db&step='.(intval($_GET['step'])+1),
	...
	public function db_importOp(){
		...
		if ($_GET['dir_name'] != '' && file_exists(BASE_DATA_PATH.DS.'shopwwi_sql_back'.DS.$_GET['dir_name'])){
			...
			if ($result == 'succ'){
				$url = array(
					array(
						'url'=>'index.php?m=db&a=db_restore',
			...
			if ($result == 'continue'){
				$url = array(
					array(
						'url'=>'index.php?m=db&a=db_import&dir_name='.$_GET['dir_name'].'&step='.(intval($_GET['step'])+1),
