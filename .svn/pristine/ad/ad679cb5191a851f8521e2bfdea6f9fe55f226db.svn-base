...
class s_loginControl extends BaseSellerControl {

    public function __construct() {
        parent::__construct();
        if (!empty($_SESSION['seller_id'])) {
            @header('location: index.php?m=store_article&a=index');die;
        }
    }
        ...
        if ($result){
            if ($result === -11){
                @header('Content-type: text/html;charset=UTF-8');
     			echo "<script language=\"javascript\">alert('用户名密码错误!');location.href='index.php?m=s_login&a=show_login'</script>";
				exit();
            } elseif ($result === -12){
                @header('Content-type: text/html;charset=UTF-8');
     			echo "<script language=\"javascript\">alert('验证码错误!');location.href='index.php?m=s_login&a=show_login'</script>";
				exit();
            }
        } else {
                @header('Content-type: text/html;charset=UTF-8');
     			echo "<script language=\"javascript\">alert('非法提交!');location.href='index.php?m=s_login&a=show_login'</script>";
				exit();
        }
        ...
        if($seller_info) {
            ...
            if($member_info) {
                ...
                if($store_info['store_state'] == 0){
                    @header('Content-type: text/html;charset=UTF-8');
                    echo "<script language=\"javascript\">alert('账号已关闭!');location.href='index.php?m=s_login&a=show_login'</script>";
                    exit();
                }
                $_SESSION['is_login'] = '1';
                ...
                if(empty($_POST['ref_url'])){
                	redirect('index.php?m=itemIndex');
                }else{
