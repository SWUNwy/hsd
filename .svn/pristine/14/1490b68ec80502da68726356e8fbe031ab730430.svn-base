...
<ul>
  ...
  <li class="receive_add addr_item">
    ...
    &nbsp;<label><a class="del" href="<?php echo urlMember('m_address','address');?>" target="_blank">管理自提服务站 </a></label>
    <?php } ?>
  </li>
  <?php if ($_GET['ifchain']) { ?>
  <li class="receive_add addr_item">
...
function delAddr(id){
    $('#addr_list').load(SITEURL+'/index.php?m=buy&a=load_addr&ifchain=<?php echo $_GET['ifchain'];?>&id='+id);
}
$(function(){
    function addAddr() {
        $('#add_addr_box').load(SITEURL+'/index.php?m=buy&a=add_addr');
    }
    $('input[nc_type="addr"]').on('click',function(){
        ...
        if ($(this).val() == '0') {
        	...
        	if ($(this).val() == '-1') {
                $('.address_item').removeClass('ncc-selected-item');
                $('#add_addr_box').load(SITEURL+'/index.php?m=buy&a=add_chain');
        	} else {
                $('.address_item').removeClass('ncc-selected-item');
