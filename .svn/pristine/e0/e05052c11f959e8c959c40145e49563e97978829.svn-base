...
function update_flex(){
	$('.ncap-stat-general-single').load('index.php?m=stat_trade&a=get_plat_sale&'+$("#formSearch").serialize(),
		function(){
			$('.timer').each(count);
     	});
    $("#flexigrid").flexigrid({
        url: 'index.php?m=stat_trade&a=get_sale_xml&'+$("#formSearch").serialize(),
        colModel : [
...
function getStatdata(type){
	$('#'+type+'_div').load('index.php?m=stat_trade&a=sale_trend&type='+type+'&'+$("#formSearch").serialize());
}
function fg_operation(name, bDiv){
    var stat_url = 'index.php?m=stat_trade&a=sale';
