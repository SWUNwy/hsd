...
<ul>
  ...
  <li class="receive_add addr_item">
    ...
    &nbsp;<label style="display: none"><a class="del" href="<?php echo urlMember('m_address','address');?>" target="_blank">管理自提服务站 </a></label>
    <?php } ?>
  </li>
  <?php if ($_GET['ifchain']) { ?>
  <li class="receive_add addr_item">
...
function delAddr(id){
    $('#addr_list').load(SITEURL+'/index.php?m=store_buy&a=load_addr&ifchain=<?php echo $_GET['ifchain'];?>&id='+id);
}
$(function(){
    function addAddr() {       
        $('#add_addr_box').load(SITEURL+'/index.php?m=store_buy&a=add_addr');
    }
    $('input[nc_type="addr"]').on('click',function(){
        ...
        if ($(this).val() == '0') {
            //判断是否有这个DIV,没有就增加
            if($("#addr_list").find('#addr_list_new').length == 0){
            	$("#addr_list").prepend("<div id='addr_list_new'></div>");
            }
        	...
        	if ($(this).val() == '-1') {
                $('.address_item').removeClass('ncc-selected-item');
                $('#add_addr_box').load(SITEURL+'/index.php?m=store_buy&a=add_chain');
        	} else {
                $('.address_item').removeClass('ncc-selected-item');
    ...
    $('#hide_addr_list').on('click',function(){
        if ($('input[nc_type="addr"]:checked').val() == '0' || $('input[nc_type="addr"]:checked').val() == '-1'){
            ...
            var member_idcard = $('input[name="addr"]:checked').attr('member_idcard');
            ...
            hideAddrList(addr_id,true_name,member_idcard,address,phone);
        }
    });
