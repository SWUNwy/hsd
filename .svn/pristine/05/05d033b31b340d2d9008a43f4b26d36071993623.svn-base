...
class store_joininccControl extends BaseHomeControl {
	...
	public function __construct() {
		...
		if(!empty($seller_info)) {
            @header('location: index.php?m=s_login');
		}
            ...
            switch (intval($joinin_detail['joinin_state'])) {
                ...
                case STORE_JOIN_STATE_VERIFY_FAIL:
                    if(!in_array($_GET['op'], array('step1', 'step2', 'step3', 'step4'))) {
                        $this->show_join_message('审核失败:'.$joinin_detail['joinin_message'], SHOP_SITE_URL.DS.'index.php?m=store_joinincc&a=step1');
                    }
                    break;
                case STORE_JOIN_STATE_PAY_FAIL:
                    if(!in_array($_GET['op'], array('pay', 'pay_save'))) {
                        $this->show_join_message('付款审核失败:'.$joinin_detail['joinin_message'], SHOP_SITE_URL.DS.'index.php?m=store_joinincc&a=pay');
                    }
                    break;
    ...
    public function step4Op() {
        ...
        @header('location: index.php?m=store_joinincc');
    ...
    public function pay_saveOp() {
        ...
        @header('location: index.php?m=store_joinin');
    }
