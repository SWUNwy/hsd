...
function update_flex(){
    $("#flexigrid").flexigrid({
        url: 'index.php?m=web_channel&a=get_channel_xml',
        colModel : [
...
function fg_operation_add(name, bDiv){
    var _url = 'index.php?m=web_channel&a=add_channel';
    window.location.href = _url;
}
function fg_operation_del(channel_id){
    if(confirm('删除后将不能恢复，确认删除这项吗？')){
        var _url = 'index.php?m=web_channel&a=del_channel&channel_id='+channel_id;
