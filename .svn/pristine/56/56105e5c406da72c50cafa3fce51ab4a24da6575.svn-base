...
class billControl extends SystemControl{
    ...
    private $links = array(
        array('url'=>'m=bill&a=index','lang'=>'nc_manage'),
    );
        ...
        if ($_GET['query_type'] == 'refund') {
            ...
            foreach ($refund_list as $refund_info) {
                ...
                if ($refund_info['refund_type'] == 1) {
                    $list['operation'] = "<a class=\"btn green\" href=\"index.php?m=refund&a=view&refund_id={$refund_info['refund_id']}\"><i class=\"fa fa-list-alt\"></i>查看</a>";
                } else {
                    $list['operation'] = "<a class=\"btn green\" href=\"index.php?m=return&a=view&return_id={$refund_info['refund_id']}\"><i class=\"fa fa-list-alt\"></i>查看</a>";
                }
    ...
    public function bill_payOp(){
        ...
        if (chksubmit()){
            ...
            if ($update){
                ...
                showMessage('保存成功','index.php?m=bill');
            }else{
                $this->log('账单付款,账单号：'.$ob_id,1);
    ...
    public function export_orderOp(){
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){
                ...
                Tpl::output('list',$array);
                Tpl::output('murl','index.php?m=bill&a=show_bill&ob_id='.$ob_id);
								//网 店 运 维shop wwi.com
		Tpl::setDirquna('shop');
                Tpl::showpage('export.excel');
                exit();
    ...
    public function export_refund_orderOp(){
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){   //显示下载链接
                ...
                Tpl::output('list',$array);
                Tpl::output('murl','index.php?m=bill&a=show_bill&query_type=refund&ob_id='.$ob_id);
								//网 店 运 维shop wwi.com
		Tpl::setDirquna('shop');
                Tpl::showpage('export.excel');
                exit();
    ...
    public function get_statis_xmlOp(){
        ...
        foreach ($bill_list as $bill_info) {
            $list = array();
            $list['operation'] = "<a class=\"btn green\" href=\"index.php?m=bill&a=show_statis&os_month={$bill_info['os_month']}\"><i class=\"fa fa-list-alt\"></i>查看</a>";
    ...
    public function get_bill_xmlOp(){
        ...
        foreach ($bill_list as $bill_info) {
            ...
            if (in_array($bill_info['ob_state'],array(2,3))) {
                $list['operation'] = "<a class=\"btn orange\" href=\"index.php?m=bill&a=show_bill&ob_id={$bill_info['ob_id']}\"><i class=\"fa fa-gavel\"></i>处理</a>";
            } else {
                $list['operation'] = "<a class=\"btn green\" href=\"index.php?m=bill&a=show_bill&ob_id={$bill_info['ob_id']}\"><i class=\"fa fa-list-alt\"></i>查看</a>";
            }
