...
class adminControl extends SystemControl{
    private $links = array(
        array('url'=>'m=admin&a=admin','lang'=>'limit_admin'),
        array('url'=>'m=admin&a=gadmin','lang'=>'limit_gadmin'),
    );
    ...
    public function admin_addOp(){
        if (chksubmit()){
            ...
            if ($rs){
                ...
                showMessage(L('nc_common_save_succ'),'index.php?m=admin&a=admin');
            }else {
                showMessage(L('nc_common_save_fail'));
            }
        }
    ...
    public function admin_editOp(){
        if (chksubmit()){
            ...
            if ($result){
                ...
                showMessage(Language::get('admin_edit_success'),'index.php?m=admin&a=admin');
            }else{
                showMessage(Language::get('admin_edit_fail'),'index.php?m=admin&a=admin');
            }
        }else{
            //查询用户信息
            $admin_model = Model('admin');
            ...
            if (!is_array($admininfo) || count($admininfo)<=0){
                showMessage(Language::get('admin_edit_admin_error'),'index.php?m=admin&a=admin');
            }
    ...
    public function gadmin_addOp(){
        if (chksubmit()){
            ...
            if ($model->insert($data)){
                ...
                showMessage(L('nc_common_save_succ'),'index.php?m=admin&a=gadmin');
            }else {
                showMessage(L('nc_common_save_fail'));
            }
        }
    ...
    public function gadmin_delOp(){
        if (is_numeric($_GET['gid'])){
            ...
            showMessage(Language::get('nc_common_op_succ'),'index.php?m=admin&a=gadmin');
        }else {
