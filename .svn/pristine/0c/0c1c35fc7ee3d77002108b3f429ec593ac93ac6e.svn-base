...
class redpacketModel extends Model {
    ...
    public function getRptTemplateList($where, $field = '*', $limit = 0, $page = 0, $order = '', $group = '') {
        ...
        if (is_array($page)){
            if ($page[1] > 0){
                $list = $this->table('redpacket_template')->field($field)->where($where)->page($page[0],$page[1])->order($order)->group($group)->select();
            } else {
                $list = $this->table('redpacket_template')->field($field)->where($where)->page($page[0])->order($order)->group($group)->select();
            }
        } else {
            $list = $this->table('redpacket_template')->field($field)->where($where)->page($page)->order($order)->group($group)->select();
        }
        //会员级别
     ...
     * @return string
     */
    public function getCurrentAvailableRpt($condition = array(), $goods_total = 0) {
        $condition['rpacket_end_date'] = array('egt',TIMESTAMP);
        $condition['rpacket_start_date'] = array('elt',TIMESTAMP);
        $condition['rpacket_state'] = 1;
        $rpt_list = $this->table('redpacket')->field('rpacket_id,rpacket_end_date,rpacket_price,rpacket_limit,rpacket_t_id,rpacket_code,rpacket_owner_id')->where($condition)->key('rpacket_t_id')->select();
