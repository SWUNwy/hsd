    ...
    <table class="flex-table">
      ...
      <tbody>
        ...
        <tr class="hover">
          ...
          <a class="btn red" href="index.php?m=admin&a=admin_del&admin_id=<?php echo $v['admin_id'];?>" onclick="if(confirm('删除后将不能恢复，确认删除这  1 项吗？')){return true;} else {return false;}"><i class="fa fa-trash-o"></i>删除</a>
          <a class="btn blue" href="index.php?m=admin&a=admin_edit&admin_id=<?php echo $v['admin_id']; ?>"><i class="fa fa-pencil-square-o"></i><?php echo $lang['nc_edit'];?></a>
          <?php } else { ?>
          <span>--</span>
          <?php } ?>
          </td>
...
function fg_operation(name, grid) {
    if (name == 'add') {
        window.location.href = 'index.php?m=admin&a=admin_add';
    }
}
</script>