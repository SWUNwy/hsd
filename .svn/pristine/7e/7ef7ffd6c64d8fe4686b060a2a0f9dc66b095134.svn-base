...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title"><a class="back" href="index.php?m=goods_recommend&a=index" title="返回商品分类推荐列表"><i class="fa fa-arrow-circle-o-left"></i></a>
      <div class="subject">
  ...
  <form id="goods_form" method="post" action='index.php?m=goods_recommend&a=save'>
...
$(function(){
    ...
    $('#goods_search').on('click',function(){
    	...
    	if($("#goods_form").valid()){
    		...
    		<?php } ?>
    	    $('#show_recommend_goods_list').load('index.php?m=goods_recommend&a=get_goods_list&gc_id='+gc_id+'&goods_name='+$('#goods_name').val());
    	}
    });
...
function select_recommend_goods(goods_id) {
	...
	text_append += '<a href="<?php echo SHOP_SITE_URL?>/index.php?m=goods&goods_id='+goods_id+'" target="_blank">';
