...
class shopwwiControl extends SystemControl{
	 private $links = array(
	    array('url'=>'m=shopwwi&a=base','lang'=>'shopwwi_set'),
        array('url'=>'m=shopwwi&a=banner','lang'=>'top_set'),
        array('url'=>'m=shopwwi&a=lc','lang'=>'lc_set'),
		array('url'=>'m=shopwwi&a=sms','lang'=>'sms_set'),
		array('url'=>'m=shopwwi&a=rc','lang'=>'rc_set'),
		array('url'=>'m=shopwwi&a=webchat','lang'=>'webchat_set'),
    ...
    public function lc_addOp() {
        ...
        if (chksubmit()) {
            if (count($lc_list) >= 8) {
                showMessage('最多可设置8个楼层','index.php?m=shopwwi&a=lc');
            }
            ...
            if ($result){
                showMessage('保存成功','index.php?m=shopwwi&a=lc');
            }else {
                showMessage('保存失败');
            }
        }
    ...
    public function lc_editOp() {
        ...
        if (!chksubmit()) {
            ...
            $result = $model_setting->updateSetting(array('shopwwi_lc'=>serialize($lc_list)));
            if ($result){
                showMessage('编辑成功','index.php?m=shopwwi&a=lc');
            }
            showMessage('编辑失败');
        }
    ...
    public function rc_addOp() {
        ...
        if (chksubmit()) {
            if (count($rc_list) >= 8) {
                showMessage('最多可设置8个楼层','index.php?m=shopwwi&a=rc');
            }
            ...
            if ($result){
                showMessage('保存成功','index.php?m=shopwwi&a=rc');
            }else {
                showMessage('保存失败');
            }
        }
    ...
    public function rc_editOp() {
        ...
        if (!chksubmit()) {
            ...
            $result = $model_setting->updateSetting(array('shopwwi_rc'=>serialize($rc_list)));
            if ($result){
                showMessage('编辑成功','index.php?m=shopwwi&a=rc');
            }
            showMessage('编辑失败');
        }
