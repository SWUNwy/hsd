...
<div class="ncc-form-default">
  <form method="POST" id="addr_form" action="index.php">
    <input type="hidden" value="store_buy" name="m">
    <input type="hidden" value="add_addr" name="a">
    ...
    <dl>
      <dt><i class="required">*</i><?php echo "身份证".$lang['nc_colon'];?></dt>
      <dd>
        <input type="text" class="text w200" name = "member_idcard" maxlength="18" id="member_idcard" value=""/>
      </dd>
    </dl>
    <dl>
...
$(document).ready(function(){
    $("#mob_phone").blur(function(){	        
        ...
        if(mob_phone!=""){
            ...
            $('#addr_list_new').load('index.php?m=store_buy&a=load_addr&mob_phone='+mob_phone);
        }  
	});
    ...
    $('#addr_form').validate({
        ...
        messages : {
            ...
            member_idcard : {
                required : '<i class="icon-exclamation-sign"></i>请填写身份证信息',
                minlength :'<i class="icon-exclamation-sign"></i>身份证信息有误',
                maxlength :'<i class="icon-exclamation-sign"></i>身份证信息有误',
            },
            region : {
    ...
    if ($('#addr_form').valid()){
        ...
        $.post('index.php',datas,function(data){
            if (data.state){
              ...
              var member_idcard = $.trim($("#member_idcard").val());
              ...
              var mob_phone = $.trim($("#mob_phone").val());
            	...
            	hideAddrList(data.addr_id,true_name,member_idcard,area_info+'&nbsp;&nbsp;'+address,(mob_phone != '' ? mob_phone : tel_phone));
            }else{
                alert(data.msg);
            }
        },'json');
