...
<div class="wrap">
  <div class="tabmenu">
    <ul id="listpj" class="tab">
      <li class="active"><a href="index.php?m=m_goodsbrowse&a=list">我的足迹</a></li>
    </ul>
  ...
  <div class="ncm-browse">
    ...
    <div class="ncm-browse-class">
      <div class="title"><a href="index.php?m=m_goodsbrowse&a=list" class="<?php echo !$_GET['gc_id']?'selected':''; ?>"> 全部浏览历史</a></div>
      <ul id="sidebarMenu">
        <?php foreach ((array)$output['browseclass_arr'] as $k=>$v){ ?>
        <li class="side-menu"> <a href="index.php?m=m_goodsbrowse&a=list&gc_id=<?php echo $k;?>" class="<?php echo $_GET['gc_id'] == $k?'selected':''; ?>"><i></i><?php echo $v['gc_name'];?></a>
          <ul style="<?php echo $_GET['gc_id'] == $k || in_array($_GET['gc_id'],array_keys($v['sonclass']))?'display: block;':'display: none;'; ?>">
            ...
            <li class="<?php echo $_GET['gc_id'] == $k_son?'selected':''; ?>"><a href="index.php?m=m_goodsbrowse&a=list&gc_id=<?php echo $k_son;?>"><?php echo $v_son['gc_name'];?></a></li>
            <?php } ?>
          </ul>
        </li>
        <?php } ?>
...
$(document).ready(function(){
   ...
   $("[nc_type='delbtn']").bind('click',function(){
	   if(confirm("确实要删除吗？")){
		   ...
		   $.getJSON('index.php?m=m_goodsbrowse&a=del&goods_id='+data_str.goods_id,function(data){
				if(data.done == true){
