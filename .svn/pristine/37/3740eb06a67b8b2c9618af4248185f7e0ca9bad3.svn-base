...
$(function(){
    $("#flexigrid").flexigrid({
        url: 'index.php?m=store&a=get_xml',
        colModel : [
        ...
        buttons : [
			...
			{display: '<i class="fa fa-plus"></i>新增数据', name : 'shopwwi_add', bclass : 'add', title : '添加一条新数据到列表', onpress : fg_operations }					
        ],
        searchitems : [
    ...
    $('#ncsubmit').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?m=store&a=get_xml&'+$("#formSearch").serialize(),query:'',qtype:''}).flexReload();
    });

    // 高级搜索重置
    $('#ncreset').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?m=store&a=get_xml'}).flexReload();
        $("#formSearch")[0].reset();
    });
});
...
function fg_operations(name, bDiv) {
    if (name == 'shopwwi_add') {
        window.location.href = 'index.php?m=store&a=shopwwi_add';
    }
}

function fg_csv(ids) {
    id = ids.join(',');
    window.location.href = $("#flexigrid").flexSimpleSearchQueryString()+'&a=export_csv&id=' + id;
}
</script>