...
(function($) {
    $.show_nc_login = function(options) {
        ...
        login_dialog_html.append('<form class="bg" method="post" id="ajax_login" action="'+LOGIN_SITE_URL+settings.action+'"></form>').find('form')
        	...
        	.append('<dl><dt>验证码</dt><dd><input type="text" size="10" maxlength="4" class="text fl w60" name="captcha"><img border="0" onclick="this.src=\'index.php?m=seccode&amp;a=makecode&amp;nchash='+settings.nchash+'&amp;t=\' + Math.random()" name="codeimage" title="看不清，换一张" src="index.php?m=seccode&amp;a=makecode&amp;nchash='+settings.nchash+'" class="fl ml10"><span>不区分大小写</span></dd></dl>')
        	.append('<ul><li>›&nbsp;如果您没有登录账号，请先<a class="register" href="'+LOGIN_SITE_URL+'/index.php?m=login&amp;a=register">注册会员</a>然后登录</li><li>›&nbsp;如果您<a class="forget" href="'+LOGIN_SITE_URL+'/index.php?m=login&amp;a=forget_password">忘记密码</a>？，申请找回密码</li></ul>')
		                ...
		                success: function(data) {
		                	if(data){
		                		...
		                		if(data.follow != 2){
		                			...
		                			$('<div class="send-msg"> <a href="'+MEMBER_SITE_URL+'/index.php?m=m_message&a=sendmsg&member_id='+data.id+'" target="_blank"><i></i>站内信</a> </div>').appendTo(_bottom);
		                		}
    ...
    $.getJSON(url, {'goods_id':goods_id, 'quantity':quantity}, function(data) {
        if (data != null) {
            if (data.state) {
                ...
                $("#rtoolbar_cartlist").load(SHOP_SITE_URL + '/index.php?m=cart&a=ajax_load&type=html');
            } else {
                alert(data.msg);
            }
        }
...
$(function(){
    ...
    if (act == "store_list"){
        ...
        $('#hdSearchTab ul li').eq(0).attr('m','store_list');
