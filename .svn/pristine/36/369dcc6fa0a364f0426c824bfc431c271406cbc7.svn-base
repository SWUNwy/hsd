...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title"><a class="back" href="index.php?m=goods_class&a=goods_class&gc_id=<?php echo $output['parent_id']?>" title="返回上级分类列表"><i class="fa fa-arrow-circle-o-left"></i></a>
      <div class="subject">
      ...
      <tbody>
        ...
        <tr data-id="<?php echo $v['gc_id'];?>">
          ...
          <td class="handle">
            ...
            <ul>
              <li><a href="index.php?m=goods_class&a=goods_class_edit&gc_id=<?php echo $v['gc_id'];?>">编辑分类信息</a></li>
              <?php if ($output['deep'] != 3) {?>
              <li><a href="index.php?m=goods_class&a=goods_class_add&gc_parent_id=<?php echo $v['gc_id'];?>">新增下级分类</a></li>
              <?php }?>
              <?php if ($v['have_child'] == 1) {?>
              <li><a href="index.php?m=goods_class&a=goods_class&gc_id=<?php echo $v['gc_id'];?>">查看下级分类</a></li>
              <?php }?>
            </ul>
            </span></td>
...
$(function(){
    ...
    $('span[nc_type="inline_edit"]').inline_edit({m: 'goods_class',a: 'ajax'});
});

function fg_operation(name, bDiv) {
    if (name == 'add') {
        window.location.href = 'index.php?m=goods_class&a=goods_class_add&gc_parent_id=<?php echo $output['gc_id']?>';
    }
    if (name == 'del') {
...
function fg_del(ids) {
    ...
    if(confirm('删除后将不能恢复，确认删除这项吗？')){
        $.getJSON('index.php?m=goods_class&a=goods_class_del', {id:id}, function(data){
            if (data.state) {
                location.reload();
            } else {
                showError(data.msg)
