...
class store_albumControl extends SystemControl {
    ...
    public function album_cateOp(){
         ...
         * 验证是否存在默认相册
         */
        $return = $model_album->checkAlbum(array('album_aclass.store_id'=>$store_info['store_id'],'is_default'=>'1'));
        if(!$return){
            $album_arr = array();
            $album_arr['aclass_name'] = Language::get('album_default_album');
            $album_arr['store_id'] = $store_info['store_id'];
         ...
         */
        $param = array();
        $param['album_aclass.store_id'] = $store_info['store_id'];
...
//      Tpl::output('show_page',$page->show());
    ...
    public function album_addOp(){
         ...
         */
        $model_album = Model('album');
        $class_count = $model_album->countClass($store_info['store_id']);
    ...
    public function album_add_saveOp(){
        if (chksubmit()){
            ...
            if($class_count['count'] >= 20){
                showDialog(Language::get('album_class_save_max_20'),'index.php?m=store_album','error',empty($_GET['inajax'])?'':'CUR_DIALOG.close();');
            }
            /**
             * 实例化相册模型
             */
            ...
            if($return){
                showDialog(Language::get('album_class_save_succeed'),'index.php?m=store_album','succ',empty($_GET['inajax'])?'':'CUR_DIALOG.close();');
            }
        }
    ...
    public function album_editOp(){
        ...
        $param['value']     = array(intval($_GET['id']),$store_info['store_id']);
    ...
    public function album_edit_saveOp(){
         ...
         * 验证
         */
        $return = $model_album->checkAlbum(array('album_aclass.store_id'=>$store_info['store_id'],'album_aclass.aclass_id'=>intval($_POST['id'])));
        if($return){
            /**
             * 更新
             */
            ...
            if($re){
                showDialog(Language::get('album_class_edit_succeed'),'index.php?m=store_album','succ',empty($_GET['inajax'])?'':'CUR_DIALOG.close();');
            }
        }else{
    ...
    public function album_delOp(){
        ...
        $param['value']     = array('1',$store_info['store_id']);
        $class_info = $model_album->getOneClass($param);
        $param = array();
        $param['aclass_id'] = $class_info['aclass_id'];
        $return = $model_album->updatePic($param,array('aclass_id'=>intval($_GET['id'])));
        if($return){
            showDialog(Language::get('album_class_file_del_succeed'),'index.php?m=store_album','succ');
        }else{
            showDialog(Language::get('album_class_file_del_lose'));
        }
    }
    ...
    public function album_pic_listOp(){
        ...
        $param['album_pic.store_id']    = $store_info['store_id'];
        if($_GET['sort'] != ''){
            switch ($_GET['sort']){
                case '0':
                    $param['order']     = 'upload_time desc';
    ...
    public function pic_list_1Op()
    {
        ...
        */
        $param = array();
        if(!empty($_GET) && intval($_GET['id'])>0){
            $param['pic_channel']	= intval($_GET['id']);
            /**
             * 分类列表
            */
            $cparam = array();
            $cparam['field']		= array('aclass_id');
            $cparam['value']		= array(intval($_GET['id']));
            //$cinfo	= $model_album->getOneClass($cparam);
            $sql="select * from channel where cha_id=".intval($_GET['id']);
            $cinfo = Model()->query($sql);
    
            Tpl::output('class_name',$cinfo[0]['cha_name']);
        }
        $pic_list = $model_picture->getPictureList($param,$page);
        if(!empty($pic_list))
        {
            foreach ($pic_list as $key => $value) {
                if($value['pic_atalog']!="")
                {
                    $pic_list[$key]['pic_path_240']=$value['pic_atalog']."/".$value['pic_path_240'];
                }
            }
        }
        Tpl::output('pic_list',$pic_list);
        Tpl::output('show_page',$model_picture->showpage());
        /**
         * 分类列表
        */
        $param = array();
        ...
        $channel_list=Model()->query("select * from channel");
        $model_channel=Model("channel");
      
        foreach ($channel_list as $channel)
        {
            $model_channel->setNode($channel['cha_id'],$channel['cha_parent'],$channel['cha_name'],0,0);
        }
        $category=$model_channel->getChilds();
        $arr=array();
        foreach ($category as $key=>$id)
        {
    
            if($model_channel->getValue($id)!="")
            {
                $arr[$id]['layer']=$model_channel->getLayer($id, '　')."┣ ";
                $arr[$id]['id']=$id;
                $arr[$id]['name']=$model_channel->getValue($id);
            }
        }
    
        Tpl::output('class_list',$arr);
    
        switch($_GET['item']) {
            case 'goods':
                Tpl::showpage('goods_add.step2_master_image1','null_layout');
                break;
            case 'des':
                Tpl::showpage('goods_add.step2_desc_image1','null_layout');
                break;
            case 'groupbuy':
                Tpl::showpage('store_groupbuy.album1','null_layout');
                break;
            case 'store_sns_normal':
                Tpl::showpage('store_sns_add.album1', 'null_layout');
                break;
            case 'goods_image':
                Tpl::output('color_id', $_GET['color_id']);
                Tpl::showpage('goods_add.step3_goods_image1', 'null_layout');
                break;
            case 'mobile':
                Tpl::output('type', $_GET['type']);
                Tpl::showpage('goods_add.step2_mobile_image1', 'null_layout');
                break;
            }
    ...
    public function pic_listOp(){
        ...
        if(!empty($_GET) && $_GET['id'] != '0'){
            ...
            $cparam['value']        = array(intval($_GET['id']),$store_info['store_id']);
            $cinfo          = $model_album->getOneClass($cparam);
        ...
        switch($_GET['item']) {
        case 'goods':
            Tpl::showpage('goods_add.step2_master_image','null_layout');
            break;
        case 'des':
            Tpl::showpage('goods_add.step2_desc_image','null_layout');
            break;
        case 'groupbuy':
        ...
        case 'goods_image':
            Tpl::output('color_id', $_GET['color_id']);
            Tpl::showpage('goods_add.step3_goods_image', 'null_layout');
            break;
        case 'mobile':
            Tpl::output('type', $_GET['type']);
            Tpl::showpage('goods_add.step2_mobile_image', 'null_layout');
            break;
        }
    }
    /**
	...
	public function album_pic_watermarkOp(){
		...
		if ($store_wm_info['wm_image_name'] == '' && $store_wm_info['wm_text'] == ''){
			showDialog(Language::get('album_class_setting_wm'),"index.php?m=store_album&a=store_watermark",'error','CUR_DIALOG.close();');//"请先设置水印"
		}
    ...
    private function profile_menu($menu_type,$menu_key=''){
        ...
        switch ($menu_type) {
            case 'album':
                $menu_array = array(
                1=>array('menu_key'=>'album','menu_name'=>Language::get('nc_member_path_my_album'),'menu_url'=>'index.php?m=store_album'),
                2=>array('menu_key'=>'watermark','menu_name'=>Language::get('nc_member_path_watermark'),'menu_url'=>'index.php?m=store_album&a=store_watermark')
                );
                break;
            case 'album_pic':
                $menu_array = array(
                1=>array('menu_key'=>'album','menu_name'=>Language::get('nc_member_path_my_album'),'menu_url'=>'index.php?m=store_album'),
                3=>array('menu_key'=>'pic_list','menu_name'=>Language::get('nc_member_path_album_pic_list'),'menu_url'=>'index.php?m=store_album&a=album_pic_list&id='.intval($_GET['id'])),
                2=>array('menu_key'=>'watermark','menu_name'=>Language::get('nc_member_path_watermark'),'menu_url'=>'index.php?m=store_album&a=store_watermark')
                );
                break;
            case 'album_pic_info':
                $menu_array = array(
                1=>array('menu_key'=>'album','menu_name'=>Language::get('nc_member_path_my_album'),'menu_url'=>'index.php?m=store_album'),
                3=>array('menu_key'=>'pic_info','menu_name'=>Language::get('nc_member_path_album_pic_info'),'menu_url'=>'index.php?m=store_album&a=album_pic_info&id='.intval($_GET['id']).'&class_id='.intval($_GET['class_id'])),
                2=>array('menu_key'=>'watermark','menu_name'=>Language::get('nc_member_path_watermark'),'menu_url'=>'index.php?m=store_album&a=store_watermark')
                );
                break;
        }
