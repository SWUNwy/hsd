...
class pointsModel extends Model {
     ...
     * @return bool
     */
    function savePointsLog($stage,$insertarr,$if_repeat = true,$buy_points=0){
        ...
        switch ($stage){
			...
			break;
				case 'buy':
				if (!$insertarr['pl_desc']){
					$insertarr['pl_desc'] ="订单".$insertarr['order_sn']."金币抵扣".$buy_points."元";
				}
				$insertarr['pl_points'] = -$buy_points;
				break;	
            case 'comments':
                if (!$insertarr['pl_desc']){
