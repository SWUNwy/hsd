...
class seller_wechatControl extends BaseSellerControl {
    ...
    public function follow_indexOp()
    {
		...
		if(empty($find_data))
    	{
			@header('Location: index.php?m=seller_wechat&a=index');
    	...
    	$this->_curlocal('微信管理',url('app=wechat&m=follow_index'),'关注自动回复');*/
    	/* 当前用户中心菜单 */
    ...
    */
    ...
    function message_indexOp()
    {
    	...
    	//$this->_curlocal('微信管理',url('app=wechat&m=message_index'),'消息自动回复');
