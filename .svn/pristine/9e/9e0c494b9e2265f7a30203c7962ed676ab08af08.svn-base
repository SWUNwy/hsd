    ...
    <ul class="nc-sns-pinterest masoned mt20"  id="snsPinterest">
      ...
      <li class="item">
        ...
        <ul class="handle">
          ...
          <li class="delete"><a href="javascript:void(0)" onclick="ajax_get_confirm('<?php echo $lang['album_plist_delete_confirm_message'];?>','index.php?m=sns_album&a=album_pic_del&id=<?php echo $v['ap_id'];?>');"><i></i><?php echo $lang['nc_delete'];?></a></li>
        </ul>
        <?php }?>
        <dl>
          <dt class="goodspic"><span class="thumb size233"><i></i><a href="index.php?m=sns_album&a=album_pic_info&id=<?php echo $v['ap_id'];?>&class_id=<?php echo $v['ac_id']?>&mid=<?php echo $output['master_id'];?><?php if(!empty($_GET['sort'])){?>&sort=<?php echo $_GET['sort']; }?>&curpage=<?php echo ceil((36*($curpage-1)+$ii)/9);?>&start_index=<?php echo ($tmp = (36*($curpage-1)+$ii)%9) ? $tmp-1 :8;?>" title="<?php echo $v['ap_name']?>"> <img id="img_<?php echo $v['ap_id'];?>" src="<?php echo UPLOAD_SITE_URL.DS.ATTACH_MALBUM.DS.$output['master_id'].DS.str_ireplace('.', '_240.', $v['ap_cover']);?>"></a></span> </dt>
          <dd> <span class="pinterest-addtime"><?php echo $lang['album_plist_upload_time'].$lang['nc_colon'].date("Y-m-d",$v['upload_time'])?></span><!--<span class="ops-comment"><a href="index.php?m=m_snshome&a=goodsinfo&type=like&mid=<?php echo $v['share_memberid'];?>&id=<?php echo $v['share_id'];?>" title="<?php echo $lang['sns_comment'];?>"><i></i></a><em><?php echo $v['share_commentcount'];?></em> </span>-->
          </dd>
        </dl>
      </li>
      <?php }?>
...
function cover(id){
	if($('#aclass_cover').attr('src') != $('#img_'+id).attr('src')){
		ajaxget('index.php?m=sns_album&a=change_album_cover&id='+id);
	}else{
