...
class store_refundControl extends BaseSellerControl {
    ...
    public function editOp() {
        ...
        if (chksubmit()) {
            $reload = 'index.php?m=store_refund&lock=1';
            if ($refund['order_lock'] == '2') {
                $reload = 'index.php?m=store_refund&lock=2';
            }
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'refund':
                $menu_array = array(
                    array('menu_key'=>'2','menu_name'=>'售前退款',  'menu_url'=>'index.php?m=store_refund&lock=2'),
                    array('menu_key'=>'1','menu_name'=>'售后退款','menu_url'=>'index.php?m=store_refund&lock=1')
                );
                break;
        }
