      ...
      <tbody>
        ...
        <tr>
          ...
          <td class="handle"><?php if($v['sg_id'] == '1'){ ?>
            ...
            <?php }else { ?>
            <a class="btn red" href="javascript:if(confirm('<?php echo $lang['problem_del'];?>'))window.location = 'index.php?m=store_grade&a=store_grade_del&sg_id=<?php echo $v['sg_id'];?>';"><i class="fa fa-trash-o"></i><?php echo $lang['nc_del'];?></a>
            <?php } ?>
            <span class="btn"><em><i class="fa fa-cog"></i><?php echo $lang['nc_set'];?><i class="arrow"></i></em>
            <ul>
              <li><a href="index.php?m=store_grade&a=store_grade_edit&sg_id=<?php echo $v['sg_id'];?>">编辑选项</a></li>
              <li><a href="index.php?m=store_grade&a=store_grade_templates&sg_id=<?php echo $v['sg_id'];?>">选择模板</a></li>
            </ul>
            </span></td>
...
$(function(){
    ...
    function fg_operation(name, grid) {
        if (name == 'add') {
            window.location.href = 'index.php?m=store_grade&a=store_grade_add';
        }
    }
</script> 