...
<div class="ncsc-form-default">
  <form method="post" action="<?php echo empty($output['chain_info']) ? urlShop('store_chain', 'chain_add') : urlShop('store_chain', 'chain_edit', array('chain_id' => $output['chain_info']['chain_id']));?>" id="chain_form" enctype="multipart/form-data">
    ...
    <dl>
      ...
      <dd>
        <?php if ($output['chain_info']['chain_img'] != '') {?><p><img src="<?php echo UPLOAD_SITE_URL.DS.chain.DS.$_SESSION['store_id'].DS.$output['chain_info']['chain_img'];?>" style="max-width: 240px; max-height: 240px;" /></p>
        <?php }?>
        <p>
    ...
    $('#chain_form').validate({
        ...
        rules : {
            chain_user : {
                required : true,
                minlength: 3,
                remote   : 'index.php?m=store_chain&a=check_user<?php if (!empty($output['chain_info'])) {?>&no_id=<?php echo $output['chain_info']['chain_id'];}?>'
            },
            chain_pwd : {
                <?php if (empty($output['chain_info'])) {?>
                required : true,
