...
class store_brandControl extends BaseSellerControl {
    ...
    public function brand_saveOp() {
        ...
        if (chksubmit()) {
            ...
            if ($result){
                showDialog($lang['store_goods_brand_apply_success'],'index.php?m=store_brand&a=brand_list','succ',empty($_GET['inajax']) ?'':'CUR_DIALOG.close();');
            }else {
        ...
        if ($_POST['form_submit'] == 'ok' and intval($_POST['brand_id']) != 0) {
            ...
            if ($error != ''){
                ...
                if ($result){
                    ...
                    showDialog($lang['nc_common_save_succ'],'index.php?m=store_brand&a=brand_list','succ',empty($_GET['inajax']) ?'':'CUR_DIALOG.close();');
                }else {
                    showDialog($lang['nc_common_save_fail']);
                }
            }
    ...
    public function drop_brandOp() {
        ...
        if ($brand_id > 0){
            ...
            showDialog(Language::get('nc_common_del_succ'),'index.php?m=store_brand&a=brand_list','succ');
        }else {
    ...
    private function profile_menu($menu_type,$menu_key='',$array=array()) {
        ...
        switch ($menu_type) {
            case 'brand_list':
                $menu_array = array(
                    1=>array('menu_key'=>'brand_list', 'menu_name'=>$lang['nc_member_path_brand_list'], 'menu_url'=>'index.php?m=store_brand&a=brand_list')
                );
                break;
        }
        if(!empty($array)) {
