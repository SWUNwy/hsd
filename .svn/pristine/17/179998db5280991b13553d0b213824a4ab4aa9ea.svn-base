...
//
    ...
    $('#fileupload').each(function(){
        $(this).fileupload({
            dataType: 'json',
            url: 'index.php?m=pointprod&a=pointprod_pic_upload&item_id=<?php echo $output['prod_info']['pgoods_id']; ?>',
...
function del_file_upload(file_id)
{
    ...
    $.getJSON('index.php?m=pointprod&a=ajaxdelupload&file_id=' + file_id, function(result){
        if(result){
            $('#' + file_id).remove();
        }else{
