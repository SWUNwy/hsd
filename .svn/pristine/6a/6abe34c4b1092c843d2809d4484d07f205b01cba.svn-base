...
class paymentControl extends BaseHomeControl{
    ...
    public function notifyOp() {
        ...
        if(strlen($callback_info['out_trade_no'])==32){
            $md5_info = Model('order_pay_md5')->where(array('pay_md5'=>$callback_info['out_trade_no']))->find();
            $callback_info['out_trade_no'] = $md5_info['pay_sn'];
        }
        if($callback_info) {
            //验证成功
            $result = $this->_update_order($callback_info['out_trade_no'], $callback_info['trade_no']);
