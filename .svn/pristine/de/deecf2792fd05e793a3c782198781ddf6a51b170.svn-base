...
<form id="spec_form" method="post" action="<?php echo SHOP_SITE_URL;?>/index.php?m=store_spec&a=save_spec">
...
$(function(){
    ...
    $('a[nctype="submit-btn"]').click(function(){
        ...
        if (_submit) {
            ajaxpost('spec_form', '<?php echo SHOP_SITE_URL;?>/index.php?m=store_spec&a=save_spec', '', 'onerror');
        }
    });
    ...
    $('a[nctype="spec_del"]').click(function(){
        ...
        $.getJSON('index.php?m=store_spec&a=ajax_delspec&id='+data_str.id, function(data){
            if(data){
                $this.parents('tr:first').remove();
            }
        });
