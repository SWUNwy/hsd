...
class goodsModel extends Model{
    ...
    public function getGoodsCommendList($store_id, $limit = 5) {
        if (C('dbdriver') == 'mysql') {
            $goods_commend_list = $this->getGoodsOnlineList(array('store_id' => $store_id, 'goods_commend' => 1), 'goods_id,goods_name,goods_jingle,goods_image,store_id,goods_promotion_price,goods_marketprice', 0, 'rand()', $limit, 'goods_commonid');
        } elseif (C('dbdriver') == 'oracle') {
            ...
            if (!empty($goods_id_list)) {
                ...
                $goods_commend_list = $this->getGoodsOnlineList(array('goods_id' => array('in',$tmp)), 'goods_id,goods_name,goods_jingle,goods_image,store_id,goods_promotion_price,goods_marketprice', 0, 'rand()', $limit);
            }
        }
    ...
    public function getGoodsImageByKey($key) {
        ...
        if (empty($image_list)) {
            ...
            $image_more = $this->getGoodsImageList(array('goods_commonid' => $common_id, 'color_id' => $color_id), 'distinct(goods_image)');
     ...
     * @return array
     */
    public function getGoodsDetail($goods_id,$type="pc") {
        if($goods_id <= 0) {
            return null;
        }
        ...
        foreach ($spec_array as $key => $value) {
            ...
            //$tpl_spec['url'] = urlShop('goods', 'index', array('goods_id' => $value['goods_id']));//16/12/26 by wangchao
            if($type=="pc"){
            	$tpl_spec['url'] = urlShop('goods_itemIndex', 'index', array('goods_id' => $value['goods_id']));
            }else{
            	$tpl_spec['url'] = urlWinxin('store_goods', 'index', array('goods_id' => $value['goods_id']));
            }
	 ...
	 * 根据商品生成税金
	 */
	public function getGoodsTax($goods_info)
	{
		$model_taxrate = Model("taxrate");
		$goods_info['goods_tax'] = $model_taxrate->GetAllTax($goods_info['goods_id'], $goods_info['goods_price'],$shipping_fee);		
		return $goods_info;
	}
    /**
     * 处理商品消费者保障服务信息
