...
<?php if ($output['current_goods_info']['is_fcode']) { ?>
    <?php include template('Buyer/buy_fcode');?>
<?php } ?>
<!-- E fcode -->
...
<div class="ncc-main">
  <div class="ncc-title">
    ...
    <?php include template('Buyer/buy_address');?>
    <?php include template('Buyer/buy_payment');?>
    <?php include template('Buyer/buy_invoice');?>

    <?php if ($output['current_goods_info']['is_book']) { ?>
        <?php include template('Buyer/buy_book_goods');?>
    <?php } else { ?>
        <?php include template('Buyer/buy_goods_list');?>
    <?php } ?>

    <input value="buy" type="hidden" name="m">
    <input value="buy_step2" type="hidden" name="a">
    <!-- 来源于购物车标志 -->
