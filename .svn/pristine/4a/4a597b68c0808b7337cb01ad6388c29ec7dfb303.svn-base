...
<div class="ncc-main">
  <div class="ncc-title">
    ...
    <h5>订单详情内容可通过查看<a href="index.php?m=m_order" target="_blank">我的订单</a>进行核对处理。</h5>
  </div>
  ...
  <div class="wxpayment"> 
    ...
    <div class="pay-weixin">
      ...
      <div class="p-w-bd">
        <div class="p-w-box">
          <div class="pw-box-hd"> <img width="298" src="<?php echo SHOP_SITE_URL?>/index.php?m=payment&a=qrcode&data=<?php echo urlencode($output['pay_url']);?>"> </div>
          <div class="pw-box-ft">
            <p>请使用微信扫一扫</p>
            <p>扫描二维码支付</p>
          </div>
...
function queryOrderState(){
    $.ajax({
        type: "GET",
        url: "<?php echo SHOP_SITE_URL?>/index.php?m=payment&a=query_state&<?php echo $output['args'];?>",
        data: "",
        dataType: "json",
        timeout: 4000,
        async:false,
        success: function(result) {
            if(result.state==1){
                if (result.type == 'r') {
                	window.location.href = '<?php echo SHOP_SITE_URL.'/index.php?m=m_order'?>';
                } else {
                	window.location.href = '<?php echo SHOP_SITE_URL.'/index.php?m=m_vr_order&a=index'?>';
                }
            }
        }
    });
