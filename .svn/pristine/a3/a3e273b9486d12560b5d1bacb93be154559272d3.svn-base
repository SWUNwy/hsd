    ...
    <div class="favorite-goods-list">
      <ul>
        ...
        <li class="favorite-pic-list<?php if (!isset($favorites['goods']) || !$favorites['goods']['state']) {?> disable<?php }?>">
          <div class="favorite-goods-thumb"><a href="index.php?m=goods&goods_id=<?php echo $favorites['goods']['goods_id'];?>" title="<?php echo $favorites['goods']['goods_name'];?>" target="_blank"><img src="<?php echo thumb($favorites['goods'], 240);?>" /></a></div>
          <div class="handle">
            <a href="javascript:void(0)" class="fr ml5" onclick="ajax_get_confirm('<?php echo $lang['nc_ensure_del'];?>', 'index.php?m=m_favorite_goods&a=delfavorites&type=goods&fav_id=<?php echo $favorites['fav_id'];?>');" title="<?php echo $lang['nc_del_&nbsp'];?>"><i class="icon-trash"></i></a>
          ...
          <dl class="favorite-goods-info">
            <dt><a href="index.php?m=goods&goods_id=<?php echo $favorites['goods']['goods_id'];?>" title="<?php echo $favorites['goods']['goods_name'];?>" target="_blank"><?php echo $favorites['goods']['goods_name'];?></a></dt>
            <?php if (!isset($favorites['goods']) || !$favorites['goods']['state']) {?>
...
$(function(){	
    ...
    $(".favorite-pic-list").on('click', 'a[data-tid]', function(){
        ...
        ajaxget('index.php?m=voucher&a=getvouchersave&jump=0&tid=' + _tid);
    });
