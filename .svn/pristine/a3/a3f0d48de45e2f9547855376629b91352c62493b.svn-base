...
<dl>
  ...
  <dd>
    <input type="text" class="w340" onclick="oCopy(this)" value="<?php echo SHOP_SITE_URL.DS;?>index.php?m=voucher&a=getvoucher&tid=<?php echo $output['t_info']['voucher_t_id'];?>" readonly/>
<script>
	function oCopy(obj){
      obj.select();
...
<h3>已生成代金券
  <a id="voucher_exportbtn" class="ncbtn-mini" href="javascript:void(0);" onclick="javascript:go('index.php?m=store_voucher&a=voucher_export&tid=<?php echo $output['t_info']['voucher_t_id']; ?>');" title="导出Excel" style="float: right; margin-right: 10px; display: none;">导出Excel</a>
</h3>
...
<script type="text/javascript">
  $("#voucher_list_div").load('index.php?m=store_voucher&a=voucherlist&tid=<?php echo $output['t_info']['voucher_t_id']; ?>');
</script>
<?php } ?>
<div class="bottom">
  <a href="javascript:void(0);" class="submit" onclick="window.location='index.php?m=store_voucher&a=templatelist'" > <?php echo $lang['voucher_template_backlist'];?></a>
</div>
<script type="text/javascript">
$(document).ready(function(){
	$("#build_voucher").click(function(){
		ajaxget('index.php?m=store_voucher&a=bulidvoucher&tid=<?php echo $output['t_info']['voucher_t_id'];?>');
	});
});
</script>