...
class store_goods_classControl extends BaseSellerControl {
    ...
    public function goods_class_saveOp() {
        ...
        if(isset($_POST['stc_id'])) {
            ...
            if($state) {
                showDialog(Language::get('nc_common_save_succ'),'index.php?m=store_goods_class&a=index','succ',empty($_GET['inajax']) ?'':'CUR_DIALOG.close();');
            } else {
    ...
    public function drop_goods_classOp() {
        ...
        if ($drop_state){
            showDialog(Language::get('nc_common_del_succ'),'index.php?m=store_goods_class&a=store_goods_class','succ');
        }else{
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'store_goods_class':
                $menu_array = array(
                1=>array('menu_key'=>'store_goods_class','menu_name'=>'店铺分类',   'menu_url'=>'index.php?m=store_goods_class&a=store_goods_class'));
                break;
        }
