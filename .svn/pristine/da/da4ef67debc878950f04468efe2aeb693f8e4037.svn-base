...
<div class="left-content">
  ...
  <?php if(!empty($output['circle_list'])){?>
    ...
    <?php foreach($output['circle_list'] as $val){?>
      <li>
        <dl class="group-info">
          <dt class="group-name"><a href="<?php echo CIRCLE_SITE_URL;?>/index.php?m=group&c_id=<?php echo $val['circle_id'];?>"><?php echo $val['circle_name'];?></a></dt>
          <dd class="group-pic"><a href="<?php echo CIRCLE_SITE_URL;?>/index.php?m=group&c_id=<?php echo $val['circle_id'];?>"><img src="<?php echo circleLogo($val['circle_id']);?>" /></a></dd>
