...
class vr_orderControl extends SystemControl{
    ...
    public function get_xmlOp(){
        ...
        foreach ($order_list as $k => $order_info) {
            ...
            $list['operation'] = "<a class=\"btn green\" href=\"index.php?m=vr_order&a=show_order&order_id={$order_info['order_id']}\"><i class=\"fa fa-list-alt\"></i>查看</a>";
            ...
            if ($if_system_receive_pay) {
                $operation_detail .= "<li><a href=\"index.php?m=vr_order&a=change_state&state_type=receive_pay&order_id={$order_info['order_id']}\">收到货款</a></li>";
            }
            if ($operation_detail) {
    ...
    public function export_step1Op(){
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){   //显示下载链接
                ...
                Tpl::output('murl','index.php?m=vr_order&a=index');
				Tpl::setDirquna('shop');
