...
<form method="get" action="index.php" target="_self">
  <table class="search-form">
    <input type="hidden" name="m" value="store_vr_order" />
  ...
  <tbody>
    ...
    <tr>
      ...
      <td class="bdl bdr"><p><?php echo $order['state_desc']; ?> </p>
        ...
        <p><a href="index.php?m=store_vr_order&a=show_order&order_id=<?php echo $order['order_id'];?>" target="_blank"><?php echo $lang['store_order_view_order'];?></a></p></td>
      <td class="bdl bdr"><!-- 取消订单 -->
        
        <?php if($order['if_cancel']) { ?>
        <p><a href="javascript:void(0)" class="ncbtn ncbtn-grapefruit mt5" nc_type="dialog" uri="index.php?m=store_vr_order&a=change_state&state_type=order_cancel&order_sn=<?php echo $order['order_sn']; ?>&order_id=<?php echo $order['order_id']; ?>" dialog_title="<?php echo $lang['store_order_cancel_order'];?>" dialog_id="seller_order_cancel_order" dialog_width="400" id="order<?php echo $order['order_id']; ?>_action_cancel" /><i class="icon-remove-circle"></i><?php echo $lang['store_order_cancel_order'];?></a></p>
        <?php } ?>
    </tr>
    <?php } } else { ?>
    <tr>
