...
class store_callcenterControl extends BaseSellerControl {
    ...
    public function saveOp(){
        if(chksubmit()){
            ...
            showDialog(Language::get('nc_common_save_succ'), 'index.php?m=store_callcenter', 'succ');
        }
    }
    /**
     * 用户中心右边，小导航
    ...
    private function profile_menu($menu_key) {
        $menu_array = array(
            1=>array('menu_key'=>'store_callcenter','menu_name'=>Language::get('nc_member_path_store_callcenter'),'menu_url'=>'index.php?m=store_callcenter'),
        );
