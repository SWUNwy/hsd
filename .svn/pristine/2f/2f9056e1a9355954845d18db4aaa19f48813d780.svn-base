...
class hgdataModel extends Model{
		...
		$str = "<MessageBody>
				 <DTCFlow>
				  <ORDER_HEAD>
				    ...
				    <SHIP_TOOL_CODE>5</SHIP_TOOL_CODE>
					...
					<BUYER_ID>".trim($reciver_info['member_idcard'])."</BUYER_ID>
					<DISCOUNT>0</DISCOUNT>
		...
		foreach($order['extend_order_goods'] as $value){										   	
			  ...
			  $str.="
				  <ORDER_DETAIL>
				      ...
				      <GOODS_SPEC>".$value['goods_name']."</GOODS_SPEC>
		...
		$str = "<MessageBody>
			    <DTCFlow>
			      <LIST_HEAD>
			      	...
			      	<ESHOP_ENT_CODE>".$this->list_setting['hg_user']."</ESHOP_ENT_CODE>
			      	<ESHOP_ENT_NAME>".$this->list_setting['hg_qyname']."</ESHOP_ENT_NAME>
			      	<DESP_ARRI_COUNTRY_CODE>".$goods_record['goods_desp_arri_country_code']."</DESP_ARRI_COUNTRY_CODE>
	...
	{   
		...
		if(!empty($order['extend_order_goods']))
		{
			foreach ($order['extend_order_goods'] as $key => $goods) {
				...
				{
					$biz['customes_code'] = "8011";
					$biz['customes_name'] = "两路空港";					
					$biz['code'] = "I10";
					$biz['type'] = "直购进口";
	...
	/**
	 * 
	 * 生成日志 type:1.订单 2.支付单 3.清单 4.运单 5.退单 6.商品 7.淘宝
