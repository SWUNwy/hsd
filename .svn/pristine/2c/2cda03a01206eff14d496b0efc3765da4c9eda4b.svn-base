    ...
    <tbody>
      ...
      <tr>
        ...
        <td rowspan="<?php echo count($val['prodlist']);?>" class="bdl"><p><?php echo $val['point_orderstatetext']; ?></p>
          <p><a href="index.php?m=m_pointorder&a=order_info&order_id=<?php echo $val['point_orderid']; ?>" target="_blank"><?php echo $lang['member_pointorder_viewinfo'];?></a></p></td>
        <td class="bdl bdr w100" rowspan="<?php echo count($val['prodlist']);?>"><p></p>
          <?php if ($val['point_orderallowreceiving']) { ?>
          <p><a href="javascript:void(0)" class="ncbtn" onclick="ajax_confirm('<?php echo $lang['member_pointorder_confirmreceivingtip']; ?>','<?php echo MEMBER_SITE_URL;?>/index.php?m=m_pointorder&a=receiving_order&order_id=<?php echo $val['point_orderid']; ?>');"><?php echo $lang['member_pointorder_confirmreceiving']; ?></a></p>
          <?php } ?>
          <?php if ($val['point_orderallowcancel']) { ?>
          <p><a href="javascript:void(0)" class="ncbtn ncbtn-bittersweet" onclick="ajax_confirm('<?php echo $lang['member_pointorder_cancel_confirmtip']; ?>','<?php echo MEMBER_SITE_URL;?>/index.php?m=m_pointorder&a=cancel_order&order_id=<?php echo $val['point_orderid']; ?>');"><?php echo $lang['member_pointorder_cancel_title']; ?></a></p>
          <?php } ?></td>
        <?php } ?>
      </tr>
      <?php } ?>
