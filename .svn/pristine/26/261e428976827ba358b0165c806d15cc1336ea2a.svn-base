...
//布局换色设置
    ...
    $('div[data-param^="map-"]').find('i').click(function(){
        ...
        if ($this.parent().hasClass('selected')) {
            $.getJSON('index.php?m=common&a=common_operations', {type : 'del', value : _value}, function(data){
                if (data) {
            ...
            $.getJSON('index.php?m=common&a=common_operations', {type : 'add', value : _value}, function(data){
                if (data) {
    ...
    if ($.cookie('commonPendingMatters') == null) {
        $.ajax({
                url : 'index.php?m=common&a=ajax_pending_matters',
                async : false,
...
function openItem(param) {
    ...
    $('#workspace').attr('src', ADMIN_SITE_URL + '/modules/' + data_str[0] + '/index.php?m=' + data_str[1]);
