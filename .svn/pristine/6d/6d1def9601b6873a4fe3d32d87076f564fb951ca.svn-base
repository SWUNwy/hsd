...
class vr_groupbuyControl extends SystemControl
{
    public function __construct()
    {
        ...
        if (C('groupbuy_allow') != 1) {
            showMessage('虚拟团购功能尚未开启', 'index.php?m=setting', 'html', 'error');
        }
    }
    ...
    public function class_addOp()
    {
        if (chksubmit()) { //添加虚拟团购分类
            ...
            if ($res) {
                ...
                $url = array(
                    array(
                        'url'=>'index.php?m=vr_groupbuy&a=class_add&parent_class_id='.$params['parent_class_id'],
                        'msg'=>'继续添加',
                    ),
                    array(
                        'url'=>'index.php?m=vr_groupbuy&a=class_list',
                        'msg'=>'返回列表',
                    )
                );
                showMessage('添加成功', $url);
            } else {
                showMessage('添加失败', 'index.php?m=vr_groupbuy&a=class_list', '', 'error');
            }
        }
    ...
    public function class_editOp()
    {
        if (chksubmit()) {
            ...
            if ($res) {
                ...
                showMessage('编辑成功', 'index.php?m=vr_groupbuy&a=class_list', '', 'succ');
            } else {
                showMessage('编辑失败', 'index.php?m=vr_groupbuy&a=class_list', '', 'error');
            }
        }
    ...
    public function class_delOp()
    {
        ...
        showMessage('删除成功', 'index.php?m=vr_groupbuy&a=class_list', '', 'succ');
    }
...
<span class="btn">
  ...
  <ul>
    <li><a href="index.php?m=vr_groupbuy&a=area_edit&area_id={$val['area_id']}">编辑区域</a></li>
    <li><a  href="index.php?m=vr_groupbuy&a=area_view&parent_area_id={$val['area_id']}">下级区域</a></li>
  </ul>
</span>
EOB;
    ...
    public function area_addOp()
    {
        if (isset($_POST) && !empty($_POST)) {
            ...
            if ($res) {
                ...
                showMessage('添加成功','index.php?m=vr_groupbuy&a=area_list','','succ');
            } else {
                showMessage('添加失败','index.php?m=vr_groupbuy&a=area_list','','error');
            }
        }
    ...
    public function area_editOp()
    {
        if (isset($_POST) && !empty($_POST)) {
            ...
            if ($res) {
                ...
                showMessage('编辑成功', 'index.php?m=vr_groupbuy&a=area_list', '', 'succ');
            } else {
                showMessage('编辑失败', 'index.php?m=vr_groupbuy&a=area_list', '', 'error');
            }
        }
    ...
    public function area_dropOp()
    {
        ...
        if ($res) {
            ...
            showMessage('删除成功','index.php?m=vr_groupbuy&a=area_list','','succ');
        } else {
            showMessage('删除失败','index.php?m=vr_groupbuy&a=area_list','','error');
        }
    }
