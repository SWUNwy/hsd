$(function(){
		...
		$.get('index.php?m=index&a=login', function(result){
		    if(result=='0'){
				...
				if($('#tracereply_'+data.txtid).css("display")=='none'){
					//加载评论列表
			        $("#tracereply_"+data.txtid).load('index.php?m=store_snshome&a=commenttop&id='+data.txtid);
	...
	$("[nc_type='scomment_del']").live('click',function(){
		...
		showDialog('您确定要删除该信息吗？','confirm', '', function(){
	        ...
	        ajax_get_confirm('','index.php?m=store_snshome&a=delcomment&scid='+data_str.scid+'&stid='+data_str.stid);
			return false;
		});
	});
		...
		if(form.valid()){
			...
			if(!isNaN(cookienum) && cookienum >= MAX_RECORDNUM){
			    if (seccode.css('display') == 'none') {
	                ...
	                seccode.find("[name='codeimage']").attr('src','index.php?m=seccode&a=makecode&nchash='+nchash+'&t=' + Math.random());
	...
	$("[nc_type='sd_del']").live('click',function(){
        ...
        eval("data_str = "+data_str);
        var url = "index.php?m=store_snshome&a=deltrace&id="+data_str.txtid;
		showDialog('您确定要删除该信息吗？','confirm', '', function(){
			ajaxget(url);
			return false;
		});
