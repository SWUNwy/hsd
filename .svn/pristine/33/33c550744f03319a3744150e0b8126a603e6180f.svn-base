...
class store_buyControl extends BaseSellerControl {
    ...
    public function buy_step1Op() {
		...
		//店铺税费计算
		Tpl::output('store_goods_tax', $result['store_goods_tax']);
		...
		//商品最多可用金币和用户所有金币		
		Tpl::output('store_goods_points', $result['store_goods_points']);
		Tpl::output('available_pd_points', $result['member_points']);
        ...
        Tpl::showpage('store_buy_step1','sell_layout');
    }
    ...
    public function buy_step2Op() {
        ...
        if(!$result['state']) {
            showMessage($result['msg'], 'index.php?m=store_cart', 'html', 'error');
        }

        //转向到商城支付页面
        redirect('index.php?m=store_buy&a=pay&pay_sn='.$result['data']['pay_sn']);
    }
    ...
    public function payOp() {
        $pay_sn	= $_GET['pay_sn'];
        if (!(preg_match('/^\d{16}$/',$pay_sn)||preg_match('/^\d{18}$/',$pay_sn)) ){
            //showMessage("没有找到此支付单",'index.php?act=store_goods_online&op=index','html','error');
        }
        ...
        /*
        $myorder_info=$model_order->getOrderInfo(array('pay_sn'=>$pay_sn,'store_id'=>$_SESSION['store_id']));
        $pay_info = $model_order->getOrderPayInfo(array('pay_sn'=>$pay_sn,'buyer_id'=>$myorder_info['buyer_id']),true);
        ...
        */
        //批量支付
        $pay_sn_arr = explode(',', $pay_sn);
        $where = array();
        $where['pay_sn'] = array('in',$pay_sn);
        //查询支付单信息
        $model_order= Model('order');
        $pay_list = $model_order->getOrderPayList($where);

        if(count($pay_list)!=count($pay_sn_arr)){
            showMessage("没有找到部分订单",'index.php?act=store_goods_online&op=index','html','error');
        }
        Tpl::output('pay_sn',$pay_sn);

        //取子订单列表
        $condition = array();
        $condition['pay_sn'] = $where['pay_sn'];
        $condition['order_state'] = array('in',array(ORDER_STATE_NEW,ORDER_STATE_PAY));
        $order_list = $model_order->getOrderList($condition,'','*','','',array(),true);
        if (empty($order_list)) {
            showMessage('未找到需要支付的订单','index.php?m=store_order','html','error');
        }
        ...
        if (empty($pay['pay_amount_online']) && empty($pay['pay_amount_offline'])) {
            redirect('index.php?m=store_buy&a=pay_ok&pay_sn='.$pay_sn.'&is_chain='.$flag_chain.'&pay_amount='.ncPriceFormat($pay_amount));
        }
        ...
        if ($pay['pay_amount_online'] > 0) {
            ...
            if (empty($payment_list)) {
                showMessage('暂未找到合适的支付方式','index.php?m=m_order','html','error');
            }
        ...
        Tpl::showpage('buy_step2','sell_layout');
    }
    ...
    public function pay_okOp() {
        $pay_sn = $_GET['pay_sn'];
        if (!preg_match('/^\d{18}$/',$pay_sn)){
            //showMessage(Language::get('cart_order_pay_not_exists'),'index.php?m=store_order','html','error');
        }
        /*
        //查询支付单信息
        $model_order= Model('order');
        ...
        */
        ...
        $pay_sn_arr = explode(',', $pay_sn);
        $model_order= Model('order');
        $pay_info = $model_order->getOrderPayList($condition);
        if(count($pay_info)!=count($pay_sn_arr)){
            showMessage(Language::get('cart_order_pay_not_exists'),'index.php?m=store_order','html','error');
        }
        Tpl::output('pay_info',$pay_info);
        Tpl::output('buy_step','step4');
        Tpl::showpage('buy_step3','sell_layout');
    }
    ...
    public function load_addrOp() {
        ...
        if (!empty($_GET['id']) && intval($_GET['id']) > 0) {
            $model_addr->delAddress(array('address_id'=>intval($_GET['id']),'store_id'=>$_SESSION['store_id']));
        }
        $condition = array();
        ...
        if($_GET['mob_phone']!=""){
           $condition['mob_phone'] = $_GET['mob_phone'];
           $addr_info = $model_addr->where(array("store_id"=>$_SESSION['store_id']))->order('address_id desc')->find();          
           if(!empty($addr_info)){
               $condition['address_id'] = array('not in',$addr_info['address_id']);  
           }
           $list = $model_addr->getAddressList($condition,$order);
        }else{
           $limit = "30";
           $list = $model_addr->getAddressList($condition,$order,$limit);
        }
        Tpl::output('address_list',$list);
        Tpl::showpage($_GET['mob_phone']!=""?'buy_address_new.load':"buy_address.load",'null_layout');
    }
    ...
    public function change_chainOp() {
     ...
     public function add_addrOp(){
        ...
        if (chksubmit()){
            ...
            //判断地址是否包含特殊单词,如母婴,跨境之类   
            $addressfun = loadfunc("address");
            $res = checkAddress($_POST['address']);   
            if($res['state']){
               exit(json_encode(array('state'=>false,'msg'=>$res['message'])));
            }
            $data = array();
     ...
     *
     */
    public function change_pointsOp()
    {
    	
    	//金币数量
    	if (empty($_POST['store_id']) ) exit('0');
		$store_cart_list = $_SESSION['store_cart_list'] ;       
    	$points = $_POST['points'];
		$store_id = $_POST['store_id'];    	
    	//金币不需要按比例分    	    	
    	if(!empty($store_cart_list[$store_id]))
    	{
    		foreach ($store_cart_list[$store_id] as $key=> $cart_info)
    		{    			
    			$price=$cart_info['goods_price']*$cart_info['goods_num'];
    			$goods_points=$cart_info['goods_points']*$cart_info['goods_num'];
    			if($goods_points>0 )
    			{    				
    				if($points>=$goods_points)
    				{    				
    					$price=$price-$goods_points;
    					$points=$points-$goods_points;    					
    				}
    				else 
    				{
    					$price=$price-$points;
                ...
                $model_taxrate = Model("taxrate");                                
                $tmp_tax =$tmp_tax+ncPriceFormat($model_taxrate->GetAllTax($cart_info['goods_id'],$price,$this->store_fee[$cart_info['store_id']][$key]['fee']));
    		}    		
    	}
		showMessage($tmp_tax,'','json');
    }
    /**
     * F码验证
     */
    ...
    private function _buy_branch($post) {
        if (!$post['ifcart']) {
            ...
            if ($goods_info['is_virtual']) {
                redirect('index.php?m=buy_virtual&a=buy_step1&goods_id='.$goods_id.'&quantity='.$quantity);
            }
        }
    }
