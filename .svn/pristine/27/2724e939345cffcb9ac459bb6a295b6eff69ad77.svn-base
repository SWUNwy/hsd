...
class storeControl extends SystemControl{
    ...
    private $_links = array(
        array('url'=>'m=store&a=store','text'=>'管理'),
        array('url'=>'m=store&a=store_joinin','text'=>'开店申请'),
        array('url'=>'m=store&a=reopen_list','text'=>'续签申请'),
        array('url'=>'m=store&a=store_bind_class_applay_list','text'=>'经营类目申请'),
        array('url'=>'m=store&a=bill_cycle','text'=>'结算周期设置')
    );
    ...
    public function get_xmlOp() {
        ...
        foreach ($store_list as $value) {
            ...
            $operation = "<a class='btn green' href='index.php?m=store&a=store_joinin_detail&member_id=".$value['member_id']."'><i class='fa fa-list-alt'></i>查看</a><span class='btn'><em><i class='fa fa-cog'></i>" . L('nc_set') . " <i class='arrow'></i></em><ul><li><a href='index.php?m=store&a=store_edit&store_id=" . $value['store_id'] . "'>编辑店铺信息</a></li><li><a href='index.php?m=store&a=store_bind_class&store_id=" . $value['store_id'] . "'>修改经营类目</a></li>";
    ...
    public function get_bill_cycle_xmlOp() {
        ...
        foreach ($store_list as $value) {
            $param = array();
            $store_state = $this->getStoreState($value);
            $operation = "<a class='btn blue' href='index.php?m=store&a=bill_cycyle_edit&store_id=".$value['store_id']."'><i class='fa fa-pencil-square-o'></i>编辑</a>";
            $operation .= "</ul></span>";
            $param['operation'] = $operation;
            $param['store_id'] = $value['store_id'];
            $store_name = "<a class='" . $store_state . "' href='". urlShop('show_store', 'index', array('store_id' => $value['store_id'])) ."' target='blank'>";
    ...
    public function export_csvOp() {
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){   //显示下载链接
                ...
                Tpl::output('murl','index.php?m=store&a=index');
				Tpl::setDirquna('shop');
    ...
    public function store_editOp(){
        ...
        if (chksubmit()){
            ...
            if ($result){
                $url = array(
                array(
                'url'=>'index.php?m=store&a=store',
                'msg'=>$lang['back_store_list'],
                ),
                array(
                'url'=>'index.php?m=store&a=store_edit&store_id='.intval($_POST['store_id']),
    ...
    public function bill_cycyle_editOp(){
        ...
        if (chksubmit()){
            ...
            if ($result){
                ...
                showMessage($lang['nc_common_save_succ'],'index.php?m=store&a=bill_cycle');
            }else {
                $this->log('设置店铺结算周期['.$_POST['store_name'].']',1);
                showMessage($lang['nc_common_save_fail'],'index.php?m=store&a=bill_cycle');
            }
        }
    ...
    public function edit_save_joininOp() {
        if (chksubmit()) {
            ...
            if ($result) {
                showMessage(L('nc_common_op_succ'), 'index.php?m=store&a=store');
            } else {
    ...
    public function shopwwi_addOp()
    {
        if (chksubmit())
        {
            ...
            showMessage('操作成功','index.php?m=store&a=store');
            return;
        }
		Tpl::setDirquna('shop');
    ...
    public function get_joinin_xmlOp() {
        ...
        foreach ($store_list as $value) {
            ...
            if(in_array(intval($value['joinin_state']), array(STORE_JOIN_STATE_NEW, STORE_JOIN_STATE_PAY))) {
                $operation = "<a class='btn orange' href=\"index.php?m=store&a=store_joinin_detail&member_id=". $value['member_id'] ."\"><i class=\"fa fa-check-circle\"></i>审核</a>";
            } else {
                $operation = "<a class='btn green' href=\"index.php?m=store&a=store_joinin_detail&member_id=". $value['member_id'] ."\"><i class=\"fa fa-list-alt\"></i>查看</a>";
            }
            $param['operation'] = $operation;
    ...
    public function get_bind_class_applay_xmlOp() {
        ...
        foreach ($store_bind_class_list as $value) {
            $param = array();
            if($value['state'] == '0') {
                $operation = "<a class='btn orange' href=\"javascript:if(confirm('确认审核吗？'))window.location = 'index.php?m=store&a=store_bind_class_applay_check&bid=".$value['bid']."&store_id=".$value['store_id']."'\"><i class=\"fa fa-check-circle\"></i>审核</a>";
            } else {
                $operation = "<a class='btn red' href=\"javascript:if(confirm('".($value['state'] == '1' ? '该类目已经审核通过，删除它可能影响到商家的使用，' : null)."确认删除吗？'))window.location = 'index.php?m=store&a=store_bind_class_applay_del&bid=".$value['bid']."&store_id=".$value['store_id']."'\"><i class=\"fa fa-trash-o\"></i>删除</a>";
            }
            $param['operation'] = $operation;
            $param['store_id'] = $value['store_id'];
    ...
    private function store_joinin_verify_pass($joinin_detail) {
        ...
        if ($param['paying_amount'] > 0) {
            showMessage('店铺入驻申请审核完成','index.php?m=store&a=store_joinin');
        } else {
    ...
    private function store_joinin_verify_open($joinin_detail) {
        ...
        if($_POST['verify_type'] === 'pass') {
            ...
            if($state) {
                ...
                showMessage('店铺开店成功','index.php?m=store&a=store_joinin');
            } else {
                showMessage('店铺开店失败','index.php?m=store&a=store_joinin');
            }
        } else {
            showMessage('店铺开店拒绝','index.php?m=store&a=store_joinin');
        }
    }
