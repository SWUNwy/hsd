	function get_sms_captcha(type){
        if($("#phone").val().length == 11 && $("#image_captcha").val().length == 4){
            var ajaxurl = 'index.php?m=connect_sms&a=get_captcha&nchash=1&type='+type;
	...
	function check_captcha(){
        if($("#phone").val().length == 11 && $("#sms_captcha").val().length == 6){
            var ajaxurl = 'index.php?m=connect_sms&a=check_captcha';
			...
			$.ajax({
				...
				success: function(rs){
            	    if(rs == 'true') {
            	        $.getScript('index.php?m=connect_sms&a=register'+'&phone='+$('#phone').val());
