        ...
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                if ($result){
                    $url = array(
                        array(
                            'url'=>'index.php?m=article_class&a=article_class_add&ac_parent_id='.intval($_POST['ac_parent_id']),
                            'msg'=>$lang['article_class_add_class'],
                        ),
                        array(
                            'url'=>'index.php?m=article_class&a=article_class',
...
//              $update_array['ac_parent_id'] = intval($_POST['ac_parent_id']);
                ...
                if ($result){
                    $url = array(
                        ...
                        array(
                            'url'=>'index.php?m=article_class&a=article_class_edit&ac_id='.intval($_POST['ac_id']),
                            'msg'=>$lang['article_class_edit_again'],
                        ),
                    );
                    $this->log(l('nc_edit,article_class_index_class').'['.$_POST['ac_name'].']',1);
                    showMessage($lang['article_class_edit_succ'],'index.php?m=article_class&a=article_class');
                }else {
    ...
    public function article_class_delOp(){
        ...
        if (intval($_GET['ac_id']) > 0){
            ...
            showMessage($lang['article_class_index_del_succ'],'index.php?m=article_class&a=article_class');
        }else {
            showMessage($lang['article_class_index_choose'],'index.php?m=article_class&a=article_class');
        }
    }
    /**
     * ajax操作
