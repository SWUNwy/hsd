...
class cartControl extends BaseBuyControl {
    ...
    public function indexOp() {
        ...
        foreach ($cart_list as $cart) {        	
			...
			//商品税费
			$cart['goods_tax'] =ncPriceFormat($model_taxrate->GetAllTax($cart['goods_id'],$cart['goods_total'],$shipping_fee));				
    ...
    public function updateOp() {
        ...
        if ($update) {
			...
			$return['goods_tax'] = $model_taxrate->GetAllTax($goods_info['goods_id'],$return['subtotal'],$shipping_fee);
            $return['goods_price'] = $goods_info['goods_price'];
