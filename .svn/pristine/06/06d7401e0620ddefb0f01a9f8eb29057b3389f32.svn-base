...
class goods_recommendControl extends SystemControl{
    ...
    public function saveOp(){
        ...
        if ($insert) {
            showMessage('保存成功','index.php?m=goods_recommend&a=index');
        }
    }
    ...
    public function get_xmlOp(){
        ...
        foreach ($rec_list as $v) {
            $list = array();
            $list['operation'] = "<a class='btn red' onclick=\"fg_delete({$v['rec_gc_id']})\"><i class='fa fa-trash-o'></i>删除</a><a class='btn blue' href='index.php?m=goods_recommend&a=add&rec_gc_id={$v['rec_gc_id']}'><i class='fa fa-pencil-square-o'></i>编辑</a>";
