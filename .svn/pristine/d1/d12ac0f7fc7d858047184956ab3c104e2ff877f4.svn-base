...
function drop_cart_item(cart_id){
    ...
    showDialog('确认删除吗?', 'confirm', '', function(){
        $.getJSON('index.php?m=store_cart&a=del&cart_id=' + cart_id, function(result){
            if(result.state){
                //删除成功
...
function change_quantity(cart_id, input){
    ...
    $.getJSON('index.php?m=store_cart&a=update&cart_id=' + cart_id + '&quantity=' + _value, function(result){
