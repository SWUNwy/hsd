...
class advControl extends SystemControl{
        ...
        if(!chksubmit()){
            ...
            if ($error != ''){
                ...
                if ($result&&$result2){
                    ...
                    showMessage($lang['adv_add_succ'],'index.php?m=adv&a=adv&ap_id='.$_POST['ap_id']);
                }else {
                    showMessage($lang['adv_add_fail'],'index.php?m=adv&a=adv&ap_id='.$_POST['ap_id']);
                }
        }
      }
    }
    ...
    public function get_ap_xmlOp(){
        ...
        foreach ((array)$ap_list as $k => $ap_info) {
            ...
            $operation_detail = "<li><a href=\"index.php?m=adv&a=adv&ap_id={$ap_info['ap_id']}\"></i>管理广告</a></li>";
            $operation_detail .= "<li><a href='index.php?m=adv&a=ap_edit&ap_id={$ap_info['ap_id']}'>编辑属性</a></li>";
        ...
        if($_POST['form_submit'] != 'ok'){
            ...
            if ($error != ''){
                ...
                if ($result){
                    ...
                    showMessage($lang['ap_add_succ'],'index.php?m=adv&a=ap_manage','html','succ',1,4000);
                }else {
    ...
    public function get_adv_xmlOp(){
        ...
        foreach ((array)$adv_list as $k => $adv_info) {
            $list = array();$operation_detail = '';
            $list['operation'] = "<a class='btn red' onclick=\"fg_delete({$adv_info['adv_id']})\"><i class='fa fa-trash-o'></i>删除</a><a class='btn blue' href='index.php?m=adv&a=adv_edit&adv_id={$adv_info['adv_id']}'><i class='fa fa-pencil-square-o'></i>编辑</a>";
