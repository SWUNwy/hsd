...
<script>
$(function(){
    var flexUrl = 'index.php?m=redpacket&a=rptlist_xml';
...
function fg_operate(name, bDiv) {
	if (name == 'add') {
		window.location.href = 'index.php?m=redpacket&a=rptadd';
    }
}
function fg_del(id) {
    if(confirm('删除后将不能恢复，确认删除吗？')){
        $.getJSON('index.php?m=redpacket&a=rptdel', {tid:id}, function(data){
            if (data.state) {
