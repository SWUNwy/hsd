...
<script>var PURL = '<?php echo $output['purl'];?>';$(document).ready(function(){ $('#area_info').nc_region();});</script>
...
<style type="text/css">
.public-nav-layout,.classtab a.curr,.head-search-bar .search-form,.public-nav-layout .category .hover .class{background: #3760a2;}.public-head-layout .logo-test{ color:#3760a2}.public-nav-layout .category .sub-class{ border-color: #3760a2;}
...
.jcarousel-skin-tango .jcarousel-clip-horizontal { width: 724px !important; height: 225px !important;}
.jcarousel-skin-tango .jcarousel-item { height: 225px !important;}
.jcarousel-skin-tango .jcarousel-container-horizontal { width: 724px !important;}
</style>
...
<div class="sort-bar">
  <div class="shop_con_list" id="main-nav-holder">
    <nav class="nc-gl-sort-bar" id="main-nav">
      <form id="store_list" method="GET" action="index.php">
        ...
        <input type="hidden" name="m" value="store_list"/>
  ...
  <dl class="clearfix">
            <dt>店铺名称<?php echo $lang['nc_colon'];?></dt>
            <dd>
            <div class="selectbox">
              <input class="text" type="text" name="keyword" value="<?php echo $_GET['keyword'];?>"/>
            </div>
            <div class="selectbox" style=" margin-left:20px;"><label for="area_info">店铺所在地<?php echo $lang['nc_colon'];?></label></div><div class="selectbox"><input id="area_info" name="area_info" type="hidden" value=""/></div>
        ...
        <div class="sort-bar"><!-- 排序方式S -->
          ...
          <!-- 默认 -->
            ...
            <li <?php if($_GET['key'] == 'store_credit'){?>class="selected"<?php }?>><a href="javascript:void(0)" <?php if($_GET['key'] == 'store_credit'){?>class="<?php echo $_GET['order'];?>"<?php }?> onClick="javascript:ss_replaceParam('store_credit','<?php  echo ($_GET['order'] == 'desc' && $_GET['key'] == 'store_credit')?'asc':'desc' ?>');" title="<?php  echo ($_GET['order'] == 'desc' && $_GET['key'] == 'store_credit')?$lang['store_class_index_credit_asc']:$lang['store_class_index_credit_desc']; ?>">信用</a></li>
                        <li><a href="javascript:void(0)" class="more-on" attr='morep' nc_type='all'>切换为小图</dd></a></li>
          </ul>
          <!-- 排序方式E -->
...
<?php foreach($output['store_list'] as $skey => $store){?>
    ...
    <div class="wwi-store-left">
      <dl class="shop-info"> 
        <dd class="shop-pic"><a href="<?php echo urlShop('show_store','', array('store_id'=>$store['store_id']),$store['store_domain']);?>" title="" target="_blank"><span class="storelogo"><img src="<?php echo getStoreLogo($store['store_label']);?>"  alt="<?php echo $store['store_name'];?>" title="<?php echo $store['store_name'];?>" class="storelogo" /></span></a></dd>
 <dd class="shopkeeper"><a href="<?php echo urlShop('show_store','', array('store_id'=>$store['store_id']),$store['store_domain']);?>" target="_blank"><?php echo $store['store_name'];?></a><span>
        ...
        <?php }?></span></dd>
        <dd class="main-runs" title="<?php echo $store['store_zy']?>"><?php echo $store['store_zy']?></dd>
      ...
      <dl class="w150">
        ...
        <dd><?php if (empty($store['store_credit_average'])){ echo $lang['nc_common_credit_null']; }else {?>
          ...
          <?php list($integer_part, $decimal_part) = explode('.', ncPriceFormat($store['store_credit_average']));?>
          <span class="seller-heart level-<?php echo $integer_part; ?>"></span>
          <?php }?>
        </dd>
      ...
      <div class="nc-shop-goodslist" nc_type='goods_all'> 
	  <?php if(!empty($store['search_list_goods']) && is_array($store['search_list_goods'])){?>
        ...
        <li>
            <dl>
              <dt class="goods-pic"><span class="thumb size160"> <i></i> <a href="index.php?m=goods&goods_id=<?php echo $v['goods_id'];?>" target="_blank"> <img  onload="javascript:DrawImage(this,160,160);" alt="<?php echo $v['goods_name'];?>" src="<?php echo thumb($v,'small');?>"></a></span></dt>
              ...
              <dd class="goods-sales"><?php echo $lang['store_class_index_deal'];?><em><?php echo $v['goods_salenum'];?></em><?php echo $lang['store_class_index_jian'];?></dd>
            </dl>
          </li>
         <?php }?>
        </ul> <?php }else{?><ul class="jcarousel-skin-tango" nc_type="jcarousel" ><li>该店主很懒！还没上传产品</li></ul><?php }?>
      </div>
