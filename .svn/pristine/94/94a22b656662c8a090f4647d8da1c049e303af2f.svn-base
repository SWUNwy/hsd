  ...
  <div class="fixed-bar">
    <div class="item-title">
      ...
      <ul class="tab-base nc-row">
        ...
        <li><a href="index.php?m=chat_log&a=msg_log">聊天内容</a></li>
      </ul>
    </div>
  </div>
  <!-- 操作说明 -->
...
$(function(){
    ...
    $('#ncsubmit').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?m=chat_log&a=get_xml&'+$("#formSearch").serialize(),query:'',qtype:''}).flexReload();
    });

    // 高级搜索重置
    $('#ncreset').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?m=chat_log&a=get_xml'}).flexReload();
        $("#formSearch")[0].reset();
    });
    $("#flexigrid").flexigrid({
        url: 'index.php?m=chat_log&a=get_xml&'+$("#formSearch").serialize(),
        colModel : [
