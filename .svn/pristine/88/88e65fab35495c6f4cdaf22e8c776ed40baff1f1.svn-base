...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title"><a class="back" href="index.php?m=redpacket&a=rptlist" title="返回列表"><i class="fa fa-arrow-circle-o-left"></i></a>
      <div class="subject">
        <h3>平台红包 - 红包详情</h3>
        <h5>平台红包新增与管理</h5>
      </div>
    ...
    <div class="ncap-form-default">
      ...
      <dl class="row">
        ...
        <dd class="opt">
            <input type="text" onclick="oCopy(this)" value="<?php echo SHOP_SITE_URL.DS;?>index.php?m=redpacket&a=getredpacket&tid=<?php echo $output['t_info']['rpacket_t_id'];?>" readonly class='w350'/>
<script>
	function oCopy(obj){
		obj.select();
    ...
    <div class="ncap-form-default">
        <div style="text-align: center;"><a href="index.php?m=redpacket&a=rptlist" class="ncap-btn-big ncap-btn-green" id="submitBtn">返回列表</a></div>
    </div>
</div>
<script>
$(function(){
    var flexUrl = 'index.php?m=redpacket&a=rplist_xml&tid=<?php echo $output['t_info']['rpacket_t_id'];?>&gtype=<?php echo $output['t_info']['rpacket_t_gettype_key'];?>';
...
function fg_csv(ids) {
    id = ids.join(',');
    window.location.href = 'index.php?m=redpacket&a=export_step1&tid=<?php echo $output['t_info']['rpacket_t_id'];?>&rid=' + id;
}
</script>