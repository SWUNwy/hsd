...
$(function(){
    ...
    $('#ncsubmit').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?m=vr_refund&a=get_all_xml&waiting=<?php echo $_GET['waiting'];?>&'+$("#formSearch").serialize(),query:'',qtype:''}).flexReload();
    });
    // 高级搜索重置
    $('#ncreset').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?m=vr_refund&a=get_all_xml&waiting=<?php echo $_GET['waiting'];?>'}).flexReload();
        $("#formSearch")[0].reset();
    });
    $("#flexigrid").flexigrid({
        url: 'index.php?m=vr_refund&a=get_all_xml&waiting=<?php echo $_GET['waiting'];?>',
        colModel : [
...
function fg_csv(ids) {
    id = ids.join(',');
    window.location.href = $("#flexigrid").flexSimpleSearchQueryString()+'&a=export_step1&waiting=<?php echo $_GET['waiting'];?>&refund_id=' + id;
}
</script> 
