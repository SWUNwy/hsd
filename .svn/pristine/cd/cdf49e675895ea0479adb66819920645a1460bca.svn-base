...
class store_goods_onlineControl extends BaseSellerControl {
    ...
    public function goods_listOp() {
        ...
        //查询商品分类 
        $goods_class=Model('goods_class')->where(array('gc_parent_id'=>0))->select();
        $condition = array();
        Tpl::output('goods_class', $goods_class);
        if(intval($_GET['gc_id_1'])>0){
            $gcwhere['gc_parent_id']=trim($_GET['gc_id_1']);
            $goods_class_two=Model("goods_class")->where($gcwhere)->select();
            Tpl::output('goods_class_two', $goods_class_two);
            $condition['gc_id_1'] = intval($_GET['gc_id_1']);
        }
        //搜索的关键字
        $search_keyword = $_GET['keyword'];
        $condition['goods_name'] = array('like',"%$search_keyword%");
        $count = $model_goods->getGoodsCount($condition);  
        $goods_list = $model_goods->getGoodsCommonList($condition, $fields, self::PAGESIZE, $order, 0);
        //全文搜索 
        ...
        foreach ($goods_list as $key => $value) {
            ...
            $ext_goods_list = $model_goods->getGoodsList(array('goods_commonid'=>$goods_commonid),'*', '','goods_id asc');
            $all_goods_storage = 0;
            foreach ($ext_goods_list as $k=> $v) {
                $goods_spec = unserialize($v['goods_spec']);
				
                if(empty($goods_spec )){
                    $goods_list[$key]['goods_spec'][$k] = "1件";
                }else{
                    $goods_list[$key]['goods_spec'][$k] = reset($goods_spec);
                }
                $goods_list[$key]['my_goods_price'][$k] = $v['goods_price'];
                $goods_list[$key]['goods_id'][$k] = $v['goods_id'];
                if($v['goods_storage']>=10){
                    $storage = "充足";
                }elseif ($v['goods_storage']>0  && $v['goods_storage']<10) {
                    $storage = "<font style='red'>紧张</font>";
                }elseif( $v['goods_storage']==0 ){
                    $storage = "售罄" ;
                }
                $goods_list[$key]['goods_storage'][$k] = $storage;
                $all_goods_storage += $v['goods_storage'];
            }
            $goods_list[$key]['all_goods_storage'][$k] = $all_goods_storage ;
        }
    ...
    public function edit_imageOp() {
        ...
        if ($common_id <= 0) {
            showMessage(L('wrong_argument'), urlShop('s_center'), 'html', 'error');
        }
        $model_goods = Model('goods');
    ...
    public function goods_unshowOp() {
        ...
        if ($result['state']) {
            showDialog(L('store_goods_index_goods_unshow_success'), getReferer() ? getReferer() : 'index.php?m=store_goods_online&a=goods_list', 'succ', '', 2);
        } else {
