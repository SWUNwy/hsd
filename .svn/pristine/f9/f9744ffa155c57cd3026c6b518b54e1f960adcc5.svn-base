...
class store_infoControl extends BaseSellerControl {
    ...
    public function bind_class_saveOp() {
        ...
        if($result) {
            showDialog('申请成功，请等待系统审核','index.php?m=store_info&a=bind_class','succ',empty($_GET['inajax']) ?'':'CUR_DIALOG.close();');
        }else {
            showDialog($lang['nc_common_save_fail']);
        }
    }
...
//              $model_store->editStore(array('store_end_time'=>$data['re_end_time']),array('store_id'=>$_SESSION['store_id']));
    ...
    private function profile_menu($menu_type,$menu_key='',$array=array()) {
        ...
        switch ($menu_type) {
            case 'index':
                $menu_array[] = array('menu_key'=>'bind_class', 'menu_name'=>$lang['nc_member_path_bind_class'], 'menu_url'=>'index.php?m=store_info&a=bind_class');
                if (!checkPlatformStore()) {
                    $menu_array[] = array('menu_key'=>'index', 'menu_name'=>$lang['nc_member_path_store_info'], 'menu_url'=>'index.php?m=store_info&a=index');
                    $menu_array[] = array('menu_key'=>'reopen', 'menu_name'=>$lang['nc_member_path_store_reopen'], 'menu_url'=>'index.php?m=store_info&a=reopen');
                }
                break;
            case 'bind_class':
                $menu_array = array(
                array('menu_key'=>'index', 'menu_name'=>$lang['nc_member_path_bind_class'], 'menu_url'=>'index.php?m=store_bind_class&a=index'),
                );
                break;
            case 'add':
                $menu_array = array(
                array('menu_key'=>'index', 'menu_name'=>$lang['nc_member_path_bind_class'], 'menu_url'=>'index.php?m=store_bind_class&a=index'),
                array('menu_key'=>'add', 'menu_name'=>$lang['nc_member_path_bind_class_add'], 'menu_url'=>'index.php?m=store_bind_class&a=add')
                );
                break;
        }
        if(!empty($array)) {
