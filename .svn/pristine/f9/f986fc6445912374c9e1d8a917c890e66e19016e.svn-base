...
class store_goods_addControl extends BaseSellerControl {
    ...
    private function checkStore(){
        ...
        if ($goodsLimit > 0) {
            ...
            if ($goods_num >= $goodsLimit) {
                showMessage(L('store_goods_index_goods_limit') . $goodsLimit . L('store_goods_index_goods_limit1'), 'index.php?m=store_goods_online&a=goods_list', 'html', 'error');
            }
        }
    }
    ...
    public function save_goodsOp() {
        ...
        if(!$result['state']) {
            showMessage(L('error') . $result['msg'], urlShop('s_center'), 'html', 'error');
        }
