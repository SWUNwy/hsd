...
class orderLogic {
    ...
    public function changeOrderSend($order_info, $role, $user = '', $post = array()) {
        ...
        $param['param'] = array(
            ...
            'order_url' => urlShop('m_order', 'show_order', array('order_id' => $order_id))
        );
    ...
    public function changeOrderReceivePay($order_list, $role, $user = '', $post = array()) {
        ...
        foreach($order_list as $order_info) {
	         //推送到海关平台
            //$this->post_kjyp($order_info['order_id']);
            $order_id = $order_info['order_id'];
            //支付成功发送买家消息
            $param = array();
            ...
            $param['param'] = array(
                    'order_sn' => $order_info['order_sn'],
                    'order_url' => urlShop('m_order', 'show_order', array('order_id' => $order_info['order_id']))
            );
            QueueClient::push('sendMemberMsg', $param);
