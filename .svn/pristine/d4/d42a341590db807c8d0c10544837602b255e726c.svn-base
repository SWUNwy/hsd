...
<form method="get" action="index.php">
  <input type="hidden" name="m" value="store_return" />
...
<table class="ncsc-default-table">
  ...
  <tbody>
    ...
    <tr class="bd-line" >
        ...
        <dd><?php echo $lang['refund_order_ordersn'].$lang['nc_colon'];?><a href="index.php?m=store_order&a=show_order&order_id=<?php echo $val['order_id']; ?>" target="_blank"><?php echo $val['order_sn'];?></a></dd>
      ...
      <td class="nscs-table-handle">
        ...
        <span><a href="index.php?m=store_return&a=edit&return_id=<?php echo $val['refund_id']; ?>" class="btn-bluejeans"><i class="icon-edit"></i><p>处理</p></a></span>
        <?php } else { ?>
       <span> <a href="index.php?m=store_return&a=view&return_id=<?php echo $val['refund_id']; ?>" class="btn-bittersweet"><i class="icon-eye-open"></i><p><?php echo $lang['nc_view'];?></p></a></span>
        <?php } ?>
        <?php if ($val['seller_state'] == 2 && $val['return_type'] == 2 && $val['goods_state'] == 2) { ?>
        <span><a href="javascript:void(0)" class="btn-mint" nc_type="dialog" dialog_title="<?php echo $lang['nc_edit'];?>" dialog_id="return_order" dialog_width="480" uri="index.php?m=store_return&a=receive&return_id=<?php echo $val['refund_id']; ?>"><i class="icon-check-sign"></i><p><?php echo '收货';?></p></a></span>
        <?php } ?>
        </td>
    </tr>
    <?php } ?>
