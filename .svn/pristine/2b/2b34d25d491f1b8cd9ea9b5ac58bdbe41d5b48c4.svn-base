...
<table class="ncsc-default-table">
  ...
  <tbody>
    ...
    <tr class="bd-line">
      ...
      <span><a href="javascript:void(0)" class="btn-grapefruit" onclick="ajax_get_confirm('<?php echo $lang['nc_ensure_del'];?>', 'index.php?m=store_info&a=reopen_del&re_id=<?php echo $val['re_id']; ?>');"><i class="icon-trash"></i><p><?php echo $lang['nc_del'];?></p></a></span>
     <?php } ?>
      </td>
    </tr>
    <?php } ?>
...
<?php if ($output['upload_cert']) {?>
  <form method="post" action="index.php?m=store_info&a=reopen_upload" name="upload_form" id="upload_form" enctype="multipart/form-data">
...
<?php if ($output['applay_reopen']) {?>
  <form method="post" action="index.php?m=store_info&a=reopen_add" name="add_form" id="add_form">
    <input type="hidden" name="form_submit" value="ok" />
    <dl>
