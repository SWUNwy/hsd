...
$(function(){
    $("#flexigrid").flexigrid({
        url: 'index.php?m=vr_bill&a=get_bill_info_code_xml&query_type=<?php echo $_GET['query_type'];?>&ob_id=<?php echo $_GET['ob_id'];?>',
        colModel : [
...
function fg_csv(ids) {
    id = ids.join(',');
    window.location.href = 'index.php?m=vr_bill&ob_id=<?php echo $_GET['ob_id'];?>&query_type=<?php echo $_GET['query_type'];?>&a=export_order&rec_id='+id;
}
</script>
