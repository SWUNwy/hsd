      ...
      <tr>
        ...
        <td><?php if(is_array($output['pic_list']) && !empty($output['pic_list'])){?>
          ...
          <dd id="batchClass" style=" display:none;">
            ...
            <?php }else{?>
            <span><?php echo $lang['album_plist_move_album_only_one'];?><a href="JavaScript:void(0);" uri="index.php?m=store_album&a=album_add" nc_type="dialog" dialog_title="<?php echo $lang['album_class_add'];?>"><?php echo $lang['album_class_add'];?></a><?php echo $lang['album_plist_move_album_only_two'];?></span>
            <?php }?>
          </dd>
          </dl>
          <?php }?></td>
        ...
        <td class="w100"><?php if(is_array($output['pic_list']) && !empty($output['pic_list'])){?>
          <form name="select_sort" id="select_sort">
            <input type="hidden" name="m" value="store_album" />
            <input type="hidden" name="a" value="album_pic_list" />
    ...
    <div class="ncsc-picture-list">
      ...
      <ul>
        ...
        <li>
          <dl>
            <dt>
              <div class="picture"><a href="index.php?m=store_album&a=album_pic_info&id=<?php echo $v['apic_id'];?>&class_id=<?php echo $v['aclass_id']?><?php if(!empty($_GET['sort'])){?>&sort=<?php echo $_GET['sort']; }?>&curpage=<?php echo ceil((15*($curpage-1)+$ii)/9);?>&start_index=<?php echo ($tmp = (15*($curpage-1)+$ii)%9) ? $tmp-1 :6;?>"> <img id="img_<?php echo $v['apic_id'];?>" src="<?php echo thumb($v, 240);?>"></a></div>
            ...
            <dd class="buttons">
              ...
              <a href="JavaScript:void(0);" nc_type="dialog" dialog_title="<?php echo $lang['album_plist_move_album'];?>" uri="index.php?m=store_album&a=album_pic_move&cid=<?php echo $output['class_info']['aclass_id']?>&id=<?php echo $v['apic_id']?>"><i class="icon-move"></i><?php echo $lang['album_plist_move_album'];?></a> <a href="JavaScript:void(0);" onclick="cover(<?php echo $v['apic_id'];?>)"><i class="icon-picture"></i><?php echo $lang['album_plist_set_to_cover'];?></a> <a href="javascript:void(0)" onclick="ajax_get_confirm('<?php echo $lang['album_plist_delete_confirm_message'];?>','index.php?m=store_album&a=album_pic_del&id=<?php echo $v['apic_id'];?>');"><i class="icon-trash"></i><?php echo $lang['album_plist_delete_img'];?></a> </dd>
          </dl>
        </li>
        <?php }?>
      </ul>
...
$(function() {
    ...
    $('input[nctype="replace_image"]').each(function(){
        $(this).fileupload({
            dataType: 'json',
            url: '<?php echo SHOP_SITE_URL;?>/index.php?m=store_album&a=replace_image_upload&id=' + $(this).attr('id'),
    ...
    $('#fileupload').fileupload({
        dataType: 'json',
        url: '<?php echo SHOP_SITE_URL;?>/index.php?m=store_album&a=image_upload',
        add: function (e,data) {
...
function _save(obj){
		$.post('index.php?m=store_album&a=change_pic_name', {id:obj.id,name:obj.value}, function(data) {
			if(data == 'false'){
...
function submit_form(type){
	...
	$('#checkboxform').attr('action','<?php echo SHOP_SITE_URL?>/index.php?m=store_album&a=album_pic_'+type);
	ajaxpost('checkboxform', '', '', 'onerror');
}
// 相册封面
function cover(id){
	if($('#aclass_cover').attr('src') != $('#img_'+id).attr('src')){
		ajaxget('index.php?m=store_album&a=change_album_cover&id='+id);
	}else{
