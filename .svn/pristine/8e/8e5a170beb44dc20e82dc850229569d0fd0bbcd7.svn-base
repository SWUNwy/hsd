  ...
  <div class="favorite-goods-list">
    <ul>
      ...
      <li class="favorite-pic-list<?php if (!isset($favorites['goods']) || !$favorites['goods']['state']) {?> disable<?php }?>" >
        <div class="favorite-goods-thumb"><a href="index.php?m=goods&goods_id=<?php echo $favorites['goods']['goods_id'];?>" target="_blank" title="<?php echo $favorites['goods']['goods_name'];?>"><img src="<?php echo thumb($favorites['goods'], 240);?>" /></a></div>
        <div class="handle"> <a href="javascript:void(0)" onclick="ajax_get_confirm('<?php echo $lang['nc_ensure_del'];?>', 'index.php?m=m_favorite_goods&a=delfavorites&type=goods&fav_id=<?php echo $favorites['fav_id'];?>');" class="fr ml5" title="<?php echo $lang['nc_del'];?>"><i class="icon-trash"></i></a>
        ...
        <dl class="favorite-goods-info">
          <dt><a href="index.php?m=goods&goods_id=<?php echo $favorites['goods']['goods_id'];?>" target="_blank" title="<?php echo $favorites['goods']['goods_name'];?>"><?php echo $favorites['goods']['goods_name'];?></a></dt>
          <?php if (!isset($favorites['goods']) || !$favorites['goods']['state']) {?>
          ...
          <dd class="voucher <?php if (is_int($i/6)) { ?>r<?php }?>"><span class="pic"></span>
            <div class="voucher-box"><i></i>
              ...
              <?php }?>
              <p>我领到的店铺代金券都在这里，<a href="<?php echo urlMember('m_voucher', 'voucher_list');?>" target="_blank">去看看</a>。</p>
            </div>
          </dd>
          <?php }?>
          <?php }?>
