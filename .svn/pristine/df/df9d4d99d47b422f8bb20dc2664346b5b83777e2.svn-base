        ...
        if(chksubmit()){
            ...
            if($error != ''){
                ...
                if ($result){
                    $url = array(
                            array(
                                    'url'=>'index.php?m=sns_member&a=tag_add',
                                    'msg'=>Language::get('sns_member_add_once_more'),
                            ),
                            array(
                                    'url'=>'index.php?m=sns_member&a=member_tag',
...
Tpl::setDirquna('shop');
        ...
        if(chksubmit()){
            ...
            if($error != ''){
                ...
                if ($result){
                    ...
                    showMessage(Language::get('nc_common_op_succ'),'index.php?m=sns_member&a=member_tag');
                }else {
                    showMessage(Language::get('nc_common_op_fail'));
                }
            }
    ...
    public function get_xmlOp() {
        ...
        foreach ($tag_list as $value) {
            $param = array();
            $param['operation'] = "<a class='btn red' href='index.php?m=sns_member&a=tag_del&id=" . $value['mtag_id'] . "'><i class='fa fa-trash-o'></i>删除</a><span class='btn'><em><i class='fa fa-cog'></i>" . L('nc_set') . " <i class='arrow'></i></em><ul><li><a href='index.php?m=sns_member&a=tag_edit&id=" . $value['mtag_id'] . "'>编辑标签</a></li><li><a href='index.php?m=sns_member&a=tag_member&id=".$value['mtag_id']."'>标签会员</a></li><li><a href='javascript:void(0);' onclick='ajaxget(\"" . urlAdminShop('sns_member','tag_recommend',array('id'=> $value['mtag_id'], 'recommend' => ($value['mtag_recommend'] == 1 ? 0 : 1))) . "\")'>" . ($value['mtag_recommend'] == 1 ? '取消推荐' : '推荐标签') . "</a></li></ul></span>";
