...
class store_supplierControl extends BaseSellerControl {
    ...
    public function sup_delOp() {
        ...
        if ($sup_id > 0){
            ...
            showDialog(Language::get('nc_common_del_succ'),'index.php?m=store_supplier&a=sup_list','succ');
        }else {
    ...
    private function profile_menu($menu_type, $menu_key = '', $array = array()) {
        ...
        switch ($menu_type) {
        	case 'sup_list':
        	    $menu_array = array(
        	    array('menu_key'=>'sup_list', 'menu_name'=>'供货商', 'menu_url'=>'index.php?m=store_supplier&a=sup_list')
        	    );
        	    break;
        }
        if(!empty($array)) {
