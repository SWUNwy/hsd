...
<div class="ncsc-form-default">
  <form method="post" action="index.php?m=store_setting&a=store_mobile" enctype="multipart/form-data" id="my_store_form">
...
for ($k = 1; $k <= $output['max_mb_sliders']; $k++) { $v = $mbSliders[$k]; ?>
      <li>
        ...
        <div class="picture" nctype="file_<?php echo $k; ?>">
          ...
          <?php } ?>
          <a href="javascript:;" data-slider-dra="<?php echo $k; ?>" class="del" title="移除">&#215;</a> </div>
        <div class="url">
          <label>
...
$(function() {
    ...
    $('input.input-file').change(function() {
        ...
        $.ajaxFileUpload({
            url: 'index.php?m=store_setting&a=store_mb_sliders',
            secureuri: false,
            fileElementId: id,
            dataType: 'json',
            data: {id: id},
    ...
    $("a[data-slider-drop]").click(function() {
        ...
        $.getJSON('index.php?m=store_setting&a=store_mb_sliders_drop', {id: id}, function(d) {
            if (!d.success) {
                alert(d.error);
                return;
            }
