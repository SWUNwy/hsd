...
class buyControl extends BaseBuyControl {
    ...
    public function buy_step1Op() {
		...
		$_SESSION['store_cart_list'] = $result['store_cart_list'];
    ...
    public function buy_step2Op() {
        ...
        if(!$result['state']) {
            showMessage($result['msg'], 'index.php?m=cart', 'html', 'error');
        }

        //转向到商城支付页面
        redirect('index.php?m=buy&a=pay&pay_sn='.$result['data']['pay_sn']);
    }
    ...
    public function payOp() {
        ...
        if (!preg_match('/^\d{18}$/',$pay_sn)){
            showMessage(Language::get('cart_order_pay_not_exists'),'index.php?m=m_order','html','error');
        }
        ...
        if (empty($order_list)) {
            showMessage('未找到需要支付的订单','index.php?m=m_order','html','error');
        }
        ...
        if (empty($pay['pay_amount_online']) && empty($pay['pay_amount_offline'])) {
            redirect('index.php?m=buy&a=pay_ok&pay_sn='.$pay_sn.'&is_chain='.$flag_chain.'&pay_amount='.ncPriceFormat($pay_amount));
        }
