...
class buy_1Logic {
     ...
     * @return array
     */
    public function calcCartList($store_cart_list,$store_freight_total = array()) {
        ...
        $order_goods_total = 0;
		//存放每个店铺的税金总金额
		$store_goods_tax = array();
        foreach ($store_cart_list as $store_id => $store_cart) {
            $tmp_amount = 0;
            $tmp_tax = 0;
            foreach ($store_cart as $key => $cart_info) {
                $store_fee = $key == 0?$store_freight_total[$store_id]:0;
                $store_cart[$key]['goods_total'] = ncPriceFormat($cart_info['goods_price'] * $cart_info['goods_num']);
                $store_cart[$key]['goods_freight'] = $store_fee;
                $store_cart[$key]['goods_image_url'] = cthumb($store_cart[$key]['goods_image']);
                $store_cart[$key]['goods_tax'] = Model("newtaxrate")->getTaxByGoodsId($cart_info['goods_id'],$store_cart[$key]['goods_total']+$store_fee);
                $tmp_amount += $store_cart[$key]['goods_total'];
                $tmp_tax += $store_cart[$key]['goods_tax'];
            }
            $store_cart_list[$store_id] = $store_cart;
            $store_goods_total[$store_id] = ncPriceFormat($tmp_amount);
            $store_goods_tax[$store_id] = ncPriceFormat($tmp_tax);
        }
        return array($store_cart_list,$store_goods_total,$store_goods_tax);
    }
    ...
    public function getStoreFreightList($buy_list = array(), $free_freight_sid_list) {
        ...
        foreach ($buy_list as $goods_info) {
            ...
            $spec_arr = explode('：', $goods_spec);
            $goods_num = $goods_info['goods_num'];
            if(!empty($spec_arr[1])){
               $goods_num = intval($spec_arr[1])>0?intval($spec_arr[1]):$goods_num;
            }
            //$freight_list[$goods_info['store_id']][] = $goods_info['transport_id'];
            $freight_list[$goods_info['store_id']][$goods_info['transport_id']] += $goods_num;
        }
        //使用相同运费的模板的最后只留一个即可。
    ...
    public function calcStoreFreight($freight_list, $city_id) {
        ...
        if (!empty($tpl_ids_list) && is_array($tpl_ids_list)) {
            ...
            foreach ($tpl_ids_list as $store_id => $transport_id_list) {
                foreach ($transport_id_list as $transport_id => $buy_num){
    ...
    public function getReciverAddr($address_info = array()) {
       if (intval($address_info['dlyp_id'])) {
			...
			$reciver_info['member_idcard'] = $address_info['member_idcard'] ? $address_info['member_idcard'] : null;
            $reciver_info['address'] = $address_info['dlyp_area_info'] . ' ' . preg_replace("/(\s|\&nbsp\;| |\xc2\xa0)/","", $address_info['dlyp_address']);
            $reciver_info['area'] = $address_info['dlyp_area_info'];
            $reciver_info['street'] =preg_replace("/(\s|\&nbsp\;| |\xc2\xa0)/","", $address_info['dlyp_address']);
            $reciver_info['dlyp'] = 1;
            ...
            $reciver_info['member_idcard'] = $address_info['member_idcard'] ? $address_info['member_idcard'] : null;
           	$reciver_info['address'] = $address_info['area_info'] . ' ' . preg_replace("/(\s|\&nbsp\;| |\xc2\xa0)/","",$address_info['address']);
            $reciver_info['area'] = $address_info['area_info'];
            $reciver_info['street'] = preg_replace("/(\s|\&nbsp\;| |\xc2\xa0)/","",$address_info['address']);
            $reciver_str = serialize($reciver_info);
