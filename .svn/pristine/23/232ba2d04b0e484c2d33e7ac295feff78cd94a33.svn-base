...
<div id="pictureFolder" class="ncsc-picture-folder">
  <dl class="ncsc-album-intro">
    <dt class="albume-name"><a href="index.php?m=store_album&a=album_pic_list&id=<?php echo $output['class_info']['aclass_id']?>"><?php echo $output['class_info']['aclass_name']?></a></dt>
...
<script>
  $(function() {
	...
	$('.ad-back').live('click',function(){
		...
		$('.ad-thumb-list').load('index.php?m=store_album&a=album_ad_ajax&id=<?php echo $_GET['id'];?>&class_id=<?php echo $_GET['class_id'];?>&curpage='+curpage);
	});
...
function ajax_change_imgmessage(url){
	...
	$.getJSON("<?php echo SHOP_SITE_URL; ?>/index.php?m=store_album&a=ajax_change_imgmessage", {'url':url}, function(data){
