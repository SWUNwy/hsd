...
class areaControl extends SystemControl{
    ...
    public function get_xmlOp() {
        ...
        foreach ($list as $k => $info) {
            ...
            $operation_detail .= "<li><a href=\"index.php?m=area&a=edit&area_id={$info['area_id']}\">编辑地区</a></li>";
            if ($info['area_deep'] <= 3) {
                $operation_detail .= "<li><a href=\"index.php?m=area&a=add&parent_id={$info['area_id']}\">新增下级</a></li>";
    ...
    public function saveOp() {
        ...
        if ($result) {
            showMessage('操作成功',$_GET['area_id'] ? 'index.php?m=area' : '');
        } else {
