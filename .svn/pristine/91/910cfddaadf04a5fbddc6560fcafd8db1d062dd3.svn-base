...
.pw-box-ft { background: url("<?php echo SHOP_SITE_URL?>/Tpl/kjyp360/images/payment/icon-red.png") no-repeat scroll 50px 8px #ff7674; height: 44px; padding: 8px 0 8px 125px; }
.p-w-sidebar { background: url("<?php echo SHOP_SITE_URL?>/Tpl/kjyp360/images/payment/phone-bg.png") no-repeat scroll 50px 0 rgba(0, 0, 0, 0); float: left; height: 421px; margin-top: -20px; padding-left: 50px; width: 379px; }
...
<div class="ncc-main">
  <div class="ncc-title">
    ...
    <h5>订单详情内容可通过查看<a href="index.php?m=store_order&a=index" target="_blank">我的订单</a>进行核对处理。</h5>
  </div>
  ...
  <div class="wxpayment"> 
    ...
    <div class="pay-weixin">
      ...
      <div class="p-w-bd">
        <div class="p-w-box">
          <div class="pw-box-hd"> <img width="298" src="<?php echo SHOP_SITE_URL?>/index.php?m=store_payment&a=qrcode&data=<?php echo urlencode($output['pay_url']);?>"> </div>
          <div class="pw-box-ft">
            <p>请使用微信扫一扫</p>
            <p>扫描二维码支付</p>
          </div>
...
function queryOrderState(){
    $.ajax({
        type: "GET",
        url: "<?php echo SHOP_SITE_URL?>/index.php?m=store_payment&a=query_state&<?php echo $output['args'];?>",
        data: "",
        dataType: "json",
        timeout: 4000,
        async:false,
        success: function(result) {
            if(result.state==1){
                if (result.type == 'r') {
                	window.location.href = '<?php echo SHOP_SITE_URL.'/index.php?m=store_buy&a=pay_ok&pay_sn='.$output['pay_sn'].'&pay_amount='.ncPriceFormat($output['api_pay_amount']) ?>';
                } 
            }
        }
    });
