...
<div class="side-message">
  ...
  <div class="guest-form">
    <form id="send_form" action='<?php echo MEMBER_SITE_URL;?>/index.php?m=m_message&a=savemsg&type=sns_board' method="post">
  ...
  <div nctype="message_list" class="message-list">
    ...
    <dl nctype="dl<?php echo $val['message_id'];?>">
      <dt><a href="index.php?m=m_snshome&mid=<?php echo $val['from_member_id'];?>" data-param="{'id':<?php echo $val['from_member_id'];?>}" nctype="mcard"><?php echo $val['from_member_name'].$lang['nc_colon'];?></a><span><?php echo parsesmiles($val['message_body']);?></span></dt>
      <dd data-param="{'msgid':'<?php echo $val['message_id'];?>','fname':'<?php echo $val['from_member_name'];?>'}"><span class="time"><?php echo $val['message_time'];?></span><span class="handle">
        ...
        <a  onclick="ajax_get_confirm('<?php echo $lang['nc_common_op_confirm'];?>', '<?php echo MEMBER_SITE_URL;?>/index.php?m=m_message&a=dropcommonmsg&drop_type=sns_msg&message_id=<?php echo $val['message_id'];?>');" href="javascript:void(0);"><?php echo $lang['nc_delete'];?></a>
        <?php }?>
        </span> </dd>
      ...
      <dd>
        ...
        <dl class="re-content">
          <dt><a href="index.php?m=m_snshome&mid=<?php echo $v['from_member_id'];?>"><?php echo $v['from_member_name'];?><?php echo $lang['sns_reply'].$lang['nc_colon'];?></a><span><?php echo parsesmiles($v['message_body']);?></span></dt>
	...
	$('a[nctype="reply_msg"]').live('click', function(){
		...
		if(!p_dd.next().is('dd[nctyoe="replyform"]')){
			$('<dd nctyoe="replyform" class="re-msg"></dd>')
				.append('<i></i>')
				.append('<form id="replyform'+data.msgid+'" action="<?php echo MEMBER_SITE_URL;?>/index.php?m=m_message&a=savereply" method="post"></form>').children('form')
...
function leaveMsgSuccess(data){
	$('<dl></dl>')
		.append('<dt><a href="index.php?m=m_snshome&mid='+data.from_member_id+'">'+data.from_member_name+'<?php echo $lang['nc_colon'];?></a><span>'+data.msg_content+'</span></dt>')
		.append('<dd data-param="{\'msgid\':\''+data.msg_id+'\',\'fname\':\''+data.from_member_name+'\'}"><span class="time"><?php echo $lang['sns_just'];?></span><span class="handle"><a href="javascript:void(0);"onclick="ajax_get_confirm(\'<?php echo $lang['nc_common_op_confirm'];?>\', \'<?php echo MEMBER_SITE_URL;?>/index.php?m=m_message&a=dropcommonmsg&drop_type=sns_msg&message_id='+data.msg_id+'\');"><?php echo $lang['nc_delete'];?></a></span></dd>')
...
function replyMsgSuccess(data){
	...
	$('<dl class="re-content"></dl>')
		.append('<dt><a href="index.php?m=m_snshome&mid='+data.from_member_id+'">'+data.from_member_name+'<?php echo $lang['sns_reply'].$lang['nc_colon'];?></a><span>'+data.msg_content+'</span></dt>')
