...
class Db{
	...
	private function __construct(){
		if (!extension_loaded('mysqli')){
			throw_exception("数据库错误: mysqli is not install");
		}
	}
	...
	private static function connect($host = 'slave'){
		...
		if (mysqli_connect_errno()) throw_exception("数据库错误: database connect failed");
		...
		if (!self::$link[$host]->query($query_string)){
			throw_exception("数据库错误: ".mysqli_error(self::$link[$host]));
		}
	}
...
//         if (is_object(self::$link[$host]) && !self::$link[$host]->ping()) {
	...
	public static function select($param, $obj_page='', $host = 'slave'){
		...
		if (empty($param)) throw_exception('数据库错误: select param is empty!');
		...
		if (!empty($tmp_table) && count($tmp_table) > 1){
			...
			if ((count($tmp_table)-1) != count($param['join_on'])){
				throw_exception('数据库错误: join number is wrong!');
			}
	...
	public static function delete($table_name, $where = '', $host = 'master'){
		...
		if (trim($where) != ''){
			...
			throw_exception('数据库错误: the condition of delete is empty!');
		}
	}
    ...
    public static function commit($host = 'master'){
    	if (!self::$iftransacte){
    		...
    		if (!$result) throw_exception("数据库错误: ".mysqli_error(self::$link[$host]));
    	}
    }
