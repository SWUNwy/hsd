...
final class Base{
	...
	private static function parse_conf(&$setting_config){
		...
		$setting['shopwwi_version'] = '<span class="vol"><font class="b">kjyp360</font><font class="o">.com</font></span>';
	...
	private static function control(){
		...
		//二级域名
		if ($GLOBALS['setting_config']['enabled_subdomain'] == '1' && $_GET['m'] == 'index' && $_GET['a'] == 'index'){
			$store_id = subdomain();
			if ($store_id > 0) $_GET['m'] = 'show_store';
		}
		$act_file = realpath(BASE_PATH.'/Library/'.$_GET['m'].'.php');
		$class_name = $_GET['m'].'Control';
		if (!@include($act_file)){
		    if (C('debug')) {
...
private static function controladmin($admin){
		...
		$act_file = realpath(BASE_PATH.'/modules/'.$admin.'/control/'.$_GET['m'].'.php');
	...
	public static function autoload($class){
		if(strpos($class,'File')===0 || strpos($class,'Crypt')===0 || strpos($class,'Math')===0 || strpos($class,'Net')===0 || strpos($class,'System')===0 ){
			return ;
		}
	...
	private static function cp(){
		...
		if (strpos(self::CPURL,'||') !== false){
			...
			header('location: http://www.kjyp360.com');exit();
		}else{
