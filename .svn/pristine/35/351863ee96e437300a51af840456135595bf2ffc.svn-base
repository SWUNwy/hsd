...
.bill_alert {
  ...
  <div class="bill_alert bill-alert-block mt10">
    ...
    <a class="ncbtn mt5" onclick="ajax_get_confirm('一旦确认将无法恢复，系统将自动进入结算环节,<BR/>确认系统出账单计算无误吗?', 'index.php?m=store_vr_bill&a=confirm_bill&ob_id=<?php echo $_GET['ob_id'];?>');" href="javascript:void(0)">本期结算无误，我要确认</a>
    <?php } elseif ($output['bill_info']['ob_state'] == BILL_STATE_SUCCESS) {?>
    <a class="ncbtn mt5" target="_blank" href="index.php?m=store_vr_bill&a=bill_print&ob_id=<?php echo $_GET['ob_id'];?>">打印结算单</a>
    <?php } ?>
    </div>
