...
class m_sharemanageControl extends BaseMemberControl {
    public function __construct(){
        parent::__construct();
        Language::read('member_sharemanage');
        //判断系统是否开启站外分享功能
        if (C('share_isuse') != 1){
            showMessage(Language::get('sharebind_unused'),'index.php?m=m_information&a=member','html','error');
        }
    }
    /**
     * 分享绑定列表
    ...
    public function unbindOp(){
        $type = $_GET['type'];
        if (empty($type)){
            showDialog(Language::get('wrong_argument'),'index.php?m=m_sharemanage','error');
        }
        $model = Model('sns_binding');
    ...
    public function share_qqzoneOp(){
        ...
        if (C('share_qqzone_isuse') != 1){
            showMessage(Language::get('sharebind_unused'),'index.php?m=m_sharemanage','html','error');
        }
    ...
    private function profile_menu($menu_key='',$array=array()) {
        $menu_array     = array();
        $menu_array = array(
            1=>array('menu_key'=>'sns_binding', 'menu_name'=>'分享绑定',    'menu_url'=> urlMember('m_sharemanage', 'index')),
        );
        if(!empty($array)) {
            $menu_array[] = $array;
        }
