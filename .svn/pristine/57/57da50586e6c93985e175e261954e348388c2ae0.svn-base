...
class store_consultControl extends BaseSellerControl {
    ...
    public function reply_saveOp(){
        ...
        if($state){
            ...
            $param['param'] = array(
                ...
                'consult_url' => urlShop('m_consult', 'my_consult')
            );
    ...
    private function profile_menu($menu_type,$menu_key='',$array=array()) {
        ...
        switch ($menu_type) {
            case 'consult':
                $menu_array = array(
                1=>array('menu_key'=>'consult_list',    'menu_name'=>Language::get('nc_member_path_all_consult'),           'menu_url'=>'index.php?m=store_consult&a=consult_list'),
                2=>array('menu_key'=>'to_reply',    'menu_name'=>Language::get('nc_member_path_unreplied_consult'),         'menu_url'=>'index.php?m=store_consult&a=consult_list&type=to_reply'),
                3=>array('menu_key'=>'replied', 'menu_name'=>Language::get('nc_member_path_replied_consult'),           'menu_url'=>'index.php?m=store_consult&a=consult_list&type=replied'));
                break;
        }
        if(!empty($array)) {
            $menu_array[] = $array;
