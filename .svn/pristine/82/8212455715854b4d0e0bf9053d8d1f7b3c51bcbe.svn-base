...
class refundControl extends SystemControl{
    ...
    private $links = array(
            array('url'=>'m=refund','text'=>'待处理'),
            array('url'=>'m=refund&a=refund_all','text'=>'所有记录'),
            array('url'=>'m=refund&a=reason','text'=>'退款退货原因')
    );
    ...
    public function get_manage_xmlOp() {
        ...
        foreach ($refund_list as $k => $refund_info) {
            $list = array();
            $list['operation'] = "<a class=\"btn orange\" href=\"index.php?m=refund&a=edit&refund_id={$refund_info['refund_id']}\"><i class=\"fa fa-gavel\"></i>处理</a>";
    ...
    public function editOp() {
        ...
        if (chksubmit()) {
            ...
            if ($state) {
                ...
                $param['param'] = array(
                    'refund_url' => urlShop('m_refund', 'view', array('refund_id' => $refund['refund_id'])),
                    'refund_sn' => $refund['refund_sn']
                );
                QueueClient::push('sendMemberMsg', $param);

                $this->log('退款确认，退款编号'.$refund['refund_sn']);
                showMessage(Language::get('nc_common_save_succ'),'index.php?m=refund&a=index');
            } else {
                showMessage(Language::get('nc_common_save_fail'));
            }
        }
    ...
    public function add_reasonOp() {
        ...
        if (chksubmit()) {
            ...
            if ($state) {
                ...
                showMessage(Language::get('nc_common_save_succ'),'index.php?m=refund&a=reason');
            } else {
                showMessage(Language::get('nc_common_save_fail'));
            }
        }
    ...
    public function del_reasonOp() {
        ...
        if ($state) {
            ...
            showMessage(Language::get('nc_common_del_succ'),'index.php?m=refund&a=reason');
        } else {
