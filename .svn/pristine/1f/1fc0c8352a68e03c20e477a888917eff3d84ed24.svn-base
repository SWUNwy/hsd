...
class m_pointorderControl extends BaseMemberControl{
    ...
    public function cancel_orderOp(){
        ...
        if ($data['state']){
            showDialog(L('member_pointorder_cancel_success'),'index.php?m=m_pointorder','succ');
        }else {
            showDialog($data['msg'],'index.php?m=m_pointorder','error');
        }
    }
    /**
     * 确认收货
    ...
    public function receiving_orderOp(){
        ...
        if ($data['state']){
            showDialog(L('member_pointorder_confirmreceiving_success'),'index.php?m=m_pointorder','succ');
        }else {
            showDialog($data['msg'],'index.php?m=m_pointorder','error');
        }
    }
    ...
    public function order_infoOp(){
        ...
        if ($order_id <= 0){
            showDialog(L('member_pointorder_parameter_error'),'index.php?m=m_pointorder','error');
        }
        $model_pointorder = Model('pointorder');
        //查询兑换订单信息
        $where = array();
        ...
        if (!$order_info){
            showDialog(L('member_pointorder_record_error'),'index.php?m=m_pointorder','error');
        }
        //获取订单状态
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'pointorder':
                $menu_array = array(
                    1=>array('menu_key'=>'points',  'menu_name'=>'积分明细',    'menu_url'=>'index.php?m=m_points'),
                    2=>array('menu_key'=>'orderlist','menu_name'=>Language::get('member_pointorder_list_title'),    'menu_url'=>'index.php?m=m_pointorder&a=orderlist')
                );
                break;
            case 'pointorderinfo':
                $menu_array = array(
                    1=>array('menu_key'=>'points',  'menu_name'=>'积分明细',    'menu_url'=>'index.php?m=m_points'),
                    2=>array('menu_key'=>'orderlist','menu_name'=>Language::get('nc_member_path_pointorder_list'),  'menu_url'=>'index.php?m=m_pointorder&a=orderlist'),
