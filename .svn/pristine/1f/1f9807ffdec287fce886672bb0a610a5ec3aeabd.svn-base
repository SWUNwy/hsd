...
<body>
<?php require_once template('Layouts/layout_top');?>
  ...
  <div class="wrapper">
    ...
    <div class="search">
      <form id="formSearch" name="formSearch" action="index.php" method="get">
        <input id="search_act" type="hidden" value="search" name="m">
      ...
      <li><a href="javascript:void(0)" class="my-friend"><?php echo $lang['sns_my_attention'];?><i></i></a>
        <div class="friend-menu">
          <dl>
            ...
            <dd><a href="index.php?m=m_snshome&mid=<?php echo $val['friend_tomid'];?>"><img src="<?php if ($val['friend_tomavatar']!='') { echo UPLOAD_SITE_URL.'/'.ATTACH_AVATAR.DS.$val['friend_tomavatar']; } else { echo UPLOAD_SITE_URL.'/'.ATTACH_COMMON.DS.$output['setting_config']['default_user_portrait']; } ?>" /><?php echo $val['friend_tomname']?></a></dd>
            <?php }}else{?>
          ...
          <p>
            <?php if(!empty($output['my_attention'])){?>
            <a href="<?php echo MEMBER_SITE_URL;?>/index.php?m=m_snsfriend&a=follow"><?php echo $lang['sns_attention_more'];?></a>
            <?php }else{?>
            <a href="<?php echo MEMBER_SITE_URL;?>/index.php?m=m_snsfriend&a=find"><?php echo $lang['sns_search_friend'];?></a>
            <?php }?>
          </p>
        </div>
      </li>
      <li><a href="index.php?m=m_snshome"><?php echo $lang['sns_my_personal_homepage'];?></a></li>
    </ul>
    <?php }?>
  </div>
</header>
<div id="container" class="wrapper mt20">
  <div class="user-info">
    <div class="user-face">
      <div class="hover-layout"><span class="thumb size120"><i></i><img src="<?php if ($output['master_info']['member_avatar']!='') { echo UPLOAD_SITE_URL.'/'.ATTACH_AVATAR.DS.$output['master_info']['member_avatar']; } else { echo UPLOAD_SITE_URL.'/'.ATTACH_COMMON.DS.C('default_user_portrait'); } ?>" />
        ...
        <a href="<?php echo urlMember('m_information', 'avatar');?>" title="<?php echo $lang['nc_updateavatar'];?>"><?php echo $lang['sns_change_avatar'];?></a>
        <?php }?>
        </span></div>
    </div>
    <dl class="user-data">
      ...
      <dd>
        ...
        <?php }?>
        <?php if ($output['relation'] == '3'){?>
        <span><a href="<?php echo urlMember('m_information', 'member');?>" title="<?php echo $lang['sns_edit_data'];?>"><?php echo $lang['sns_edit_data'];?></a></span>
        <?php }?>
      </dd>
    </dl>
    <div class="user-stat">
  ...
  <div class="sns-nav">
    <ul>
      <li><a <?php if($output['menu_sign'] == 'snshome'){?>class="current"<?php }?> href="index.php?m=m_snshome<?php if(!empty($output['master_id'])){ echo '&mid='.$output['master_id']; }?>"><i class="home pngFix"></i><?php echo $lang['nc_index'];?></a></li>
      <li><a <?php if($output['menu_sign'] == 'sharegoods'){?>class="current"<?php }?> href="index.php?m=m_snshome&a=shareglist<?php if(!empty($output['master_id'])){ echo '&mid='.$output['master_id']; }?>"><i class="goods pngFix"></i><?php echo $lang['sns_treasure'];?></a></li>
      <li><a <?php if($output['menu_sign'] == 'sharestore'){?>class="current"<?php }?> href="index.php?m=m_snshome&a=storelist<?php if(!empty($output['master_id'])){ echo '&mid='.$output['master_id']; }?>"><i class="store pngFix"></i><?php echo $lang['sns_store'];?></a></li>
      <li><a <?php if($output['menu_sign'] == 'snsalbum'){?>class="current"<?php }?> href="index.php?m=sns_album<?php if(!empty($output['master_id'])){ echo '&mid='.$output['master_id']; }?>"><i class="album pngFix"></i><?php echo $lang['sns_album'];?></a></li>
      <li><a <?php if($output['menu_sign'] == 'snstrace'){?>class="current"<?php }?> href="index.php?m=m_snshome&a=trace<?php if(!empty($output['master_id'])){ echo '&mid='.$output['master_id']; }?>"><i class="news pngFix"></i><?php echo $lang['sns_fresh_news'];?></a></li>
      <?php if(intval(C('circle_isuse'))){?>
      <li><a <?php if($output['menu_sign'] == 'circle'){?>class="current"<?php }?> href="index.php?m=sns_circle<?php if(!empty($output['master_id'])){ echo '&mid='.$output['master_id']; }?>"><i class="circle pngFix"></i><?php echo $lang['sns_group'];?></a></li>
      <?php }?>
    </ul>
    <?php if($output['relation'] == '3'){?>
...
$(function(){
  ...
  $(".my-friend").click(function(){
	...
	$('a[nctype="change_skin"]').click(function(){
		ajax_form('change_skin', '<?php echo $lang['sns_change_skin'];?>', SITEURL + '/index.php?m=sns_setting&a=change_skin', '480');
	});
});
</script>
</body>
