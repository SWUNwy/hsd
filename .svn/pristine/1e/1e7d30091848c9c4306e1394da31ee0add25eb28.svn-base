...
class promotion_bookControl extends SystemControl{
    private $links = array(
        array('url'=>'m=promotion_book&a=index','text'=>'定金预售'),
        array('url'=>'m=promotion_book&a=presell_goods_list', 'text'=>'全款预售'),
        array('url'=>'m=promotion_book&a=book_quota_list','text'=>'套餐列表'),
        array('url'=>'m=promotion_book&a=book_setting','text'=>'设置')
    );
    public function __construct(){
        ...
        if (intval($_GET['promotion_allow']) !== 1 && intval(C('promotion_allow')) !== 1){
            $url = array(
                ...
                array(
                    'url'=>'index.php?m=promotion_book&promotion_allow=1',
                    'msg'=>L('open'),
                )
            );
