...
$(function(){
    $("#flexigrid").flexigrid({
        url: 'index.php?m=goods_album&a=get_xml',
        colModel : [
...
function fg_operation(name, bDiv) {
    if (name == 'add') {
        window.location.href = 'index.php?m=goods_album&a=pic_list';
    }
}
function fg_del(id) {
    if(confirm('删除后将不能恢复，确认删除这项吗？')){
        $.getJSON('index.php?m=goods_album&a=aclass_del', {id:id}, function(data){
            if (data.state) {
