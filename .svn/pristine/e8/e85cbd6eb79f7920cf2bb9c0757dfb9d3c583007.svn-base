  ...
  <div class="fixed-bar">
    <div class="item-title">
      ...
      <ul class="tab-base nc-row">
        ...
        <li><a href="index.php?m=help_store&a=help_type"><?php echo '帮助类型';?></a></li>
      </ul>
    </div>
  </div>
  <!-- 操作说明 -->
...
$(function(){
    $("#flexigrid").flexigrid({
        url: 'index.php?m=help_store&a=get_xml',
        colModel : [
...
function fg_operation(name, bDiv) {
    if (name == 'add') {
        window.location.href = 'index.php?m=help_store&a=add_help';
    }
}
function fg_del(id) {
    if(confirm('删除后将不能恢复，确认删除这项吗？')){
        $.getJSON('index.php?m=help_store&a=del_help', {id:id}, function(data){
            if (data.state) {
