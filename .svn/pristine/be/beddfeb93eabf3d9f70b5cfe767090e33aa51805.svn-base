...
//             $list = array();
//             $list['operation'] = "<a class=\"btn green\" href=\"index.php?m=vr_bill&a=show_statis&os_month={$bill_info['os_month']}\"><i class=\"fa fa-list-alt\"></i>查看</a>";
...
//     }
    ...
    public function get_bill_xmlOp(){
        ...
        foreach ($bill_list as $bill_info) {
            ...
            if (in_array($bill_info['ob_state'],array(2,3))) {
                $list['operation'] = "<a class=\"btn orange\" href=\"index.php?m=vr_bill&a=show_bill&ob_id={$bill_info['ob_id']}\"><i class=\"fa fa-gavel\"></i>处理</a>";
            } else {
                $list['operation'] = "<a class=\"btn green\" href=\"index.php?m=vr_bill&a=show_bill&ob_id={$bill_info['ob_id']}\"><i class=\"fa fa-list-alt\"></i>查看</a>";
            }
    ...
    public function get_bill_info_code_xmlOp(){
        ...
        foreach ($code_list as $code_info) {
            $list = array();
            $list['operation'] = "<a class=\"btn green\" href=\"index.php?m=vr_order&a=show_order&order_id={$code_info['order_id']}\"><i class=\"fa fa-list-alt\"></i>查看</a>";
    ...
    public function bill_payOp(){
        ...
        if (chksubmit()){
            ...
            if ($update){
                ...
                showMessage('保存成功','index.php?m=vr_bill');
            }else{
                $this->log('账单付款,账单号：'.$ob_id,1);
    ...
    public function export_orderOp(){
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){
                ...
                Tpl::output('list',$array);
                Tpl::output('murl','index.php?m=vr_bill&a=show_bill&ob_id='.$ob_id);
				Tpl::setDirquna('shop');
                Tpl::showpage('export.excel');
                exit();
            }else{
