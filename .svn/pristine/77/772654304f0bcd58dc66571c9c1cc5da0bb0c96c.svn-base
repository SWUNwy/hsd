...
class contractControl extends SystemControl{
    ...
    public function __construct(){
        ...
        if (C('contract_allow') != 1){
            showDialog('需开启“消费者保障服务”功能','index.php?m=operating','error');
        }
    ...
    public function citemeditOp(){
        ...
        if ($itemid_id <= 0){
            showDialog(L('param_error'),'index.php?m=contract&a=citemlist');
        }
        $model_contract = Model('contract');
        //查询模板信息
        $where = array();
        ...
        if(chksubmit()){
            ...
            if($rs){
                ...
                showDialog(L('nc_common_save_succ'),'index.php?m=contract&a=citemlist','succ');
            } else {
    ...
    public function contracteditOp(){
        ...
        if ($ct_id <= 0){
            showDialog(L('param_error'),'index.php?m=contract&a=contractlist');
        }
        $model_contract = Model('contract');
        //查询店铺保障服务信息
        $where = array();
        ...
        if(chksubmit()){
            ...
            if ($c_info['ct_closestate_key'] == $_POST['c_state']) {
                showDialog(L('nc_common_save_succ'),'index.php?m=contract&a=contractlist','succ');
            }
            try {
    ...
    public function applyeditOp(){
        ...
        if ($aid <= 0){
            showDialog(L('param_error'),'index.php?m=contract&a=applylist');
        }
        $model_contract = Model('contract');
        //查询申请信息
        $where = array();
        ...
        if(chksubmit()){
            if ($_POST['apply_state'] == $apply_info['cta_auditstate_key']) {
                showDialog('不能重复更改申请状态','index.php?m=contract&a=applylist');
            }
            if (!in_array($_POST['apply_state'],array_keys($curr_applystatearr))) {
                showDialog('编辑失败','index.php?m=contract&a=applyedit&aid='.$apply_info['cta_id']);
            }
            try {
                $model_contract->beginTransaction();
                //编辑申请状态
                ...
                showDialog(L('nc_common_save_succ'),'index.php?m=contract&a=applylist','succ');
            }catch (Exception $e){
                $model_contract->rollback();
                showDialog($e->getMessage(), '', 'error');
            }
    ...
    public function quiteditOp(){
        ...
        if ($qid <= 0){
            showDialog(L('param_error'),'index.php?m=contract&a=quitlist');
        }
        $model_contract = Model('contract');
        //查询申请信息
        $where = array();
        ...
        if(chksubmit()){
            if ($_POST['quit_state'] == $quit_info['ctq_auditstate_key']) {
                showDialog('不能重复编辑申请状态','index.php?m=contract&a=quitlist');
            }
            if (!in_array($_POST['quit_state'],array_keys($this->quit_auditstate_arr))) {
                showDialog('编辑失败','index.php?m=contract&a=quitedit&qid='.$quit_info['ctq_id']);
            }
            try {
                $model_contract->beginTransaction();
                //编辑申请状态
                ...
                showDialog(L('nc_common_save_succ'),'index.php?m=contract&a=quitlist','succ');
            }catch (Exception $e){
                $model_contract->rollback();
                showDialog($e->getMessage(), '', 'error');
            }
    ...
    private function show_menu($menu_key=''){
        $menu_array = array(
            1=>array('menu_key'=>'applylist','menu_name'=>'服务加入申请', 'menu_url'=>'index.php?m=contract&a=applylist'),
            2=>array('menu_key'=>'quitlist','menu_name'=>'服务退出申请', 'menu_url'=>'index.php?m=contract&a=quitlist'),
            3=>array('menu_key'=>'contractlist','menu_name'=>'店铺保障服务', 'menu_url'=>'index.php?m=contract&a=contractlist'),
            4=>array('menu_key'=>'citemlist','menu_name'=>'保障服务管理', 'menu_url'=>'index.php?m=contract&a=citemlist')
        );
