...
<div class="wrap">
  <div class="tabmenu">
    <?php include template('Layouts/submenu');?>
  </div>
  <form method="get" action="index.php" target="_self">
    <table class="ncm-search-table">
      <input type="hidden" name="m" value="member_order" />
  ...
  <table class="ncm-default-table order">
    ...
    <tbody<?php if (!empty($group_info['pay_amount']) && $p == 0) {?> class="pay" <?php }?>>
      ...
      <tr>
        <td colspan="19" class="pay-td"><span class="ml15">在线支付金额：<em>￥<?php echo ncPriceFormat($group_info['pay_amount']);?></em></span> <a class="ncbtn ncbtn-bittersweet fr mr15" href="index.php?m=buy&a=pay&pay_sn=<?php echo $order_pay_sn; ?>"><i class="icon-shield"></i>订单支付</a></td>
      </tr>
      <?php }?>
      <?php $p++;?>
      <tr>
        <th colspan="19"> <span class="ml10"> 
          ...
          <a href="javascript:void(0);" class="order-trash" onclick="ajax_get_confirm('您确定要删除吗?删除后该订单可以在回收站找回，或彻底删除', 'index.php?m=m_order&a=change_state&state_type=order_delete&order_id=<?php echo $order_info['order_id']; ?>');"><i class="icon-trash"></i>删除</a>
          <?php } ?>
          
          <!-- 还原订单 -->
          
          <?php if ($order_info['if_restore']) { ?>
          <a href="javascript:void(0);" class="order-trash" onclick="ajax_get_confirm('您确定要还原吗?', 'index.php?m=m_order&a=change_state&state_type=order_restore&order_id=<?php echo $order_info['order_id']; ?>');"><i class="icon-refresh"></i>还原</a>
          <?php } ?>
        </th>
      </tr>
      ...
      <tr>
        ...
        <td><!-- 退款 -->
          ...
          <p><a href="index.php?m=m_refund&a=add_refund&order_id=<?php echo $order_info['order_id']; ?>&goods_id=<?php echo $goods_info['rec_id']; ?>">退款/退货</a></p>
          <?php }?>
          <?php if (is_array($order_info['refund_all']) && !empty($order_info['refund_all'])) {?>
          <dd><a target="_blank" href="index.php?m=m_refund&a=view&refund_id=<?php echo $order_info['refund_all']['refund_id'];?>">查看退款</a></dd>
          <?php }else if($goods_info['extend_refund']['refund_type'] == 1) {?>
          <dd><a target="_blank" href="index.php?m=m_refund&a=view&refund_id=<?php echo $goods_info['extend_refund']['refund_id'];?>">查看退款</a></dd>
          <?php }else if($goods_info['extend_refund']['refund_type'] == 2) {?>
          <dd><a target="_blank" href="index.php?m=m_return&a=view&return_id=<?php echo $goods_info['extend_refund']['refund_id'];?>">查看退货</a></dd>
          <?php } ?>
          
          <!-- 投诉 -->
          
          <?php if ($order_info['if_complain']){ ?>
          <p><a href="index.php?m=m_complain&a=complain_new&order_id=<?php echo $order_info['order_id']; ?>&goods_id=<?php echo $goods_info['rec_id']; ?>">交易投诉</a></p>
          <?php } ?></td>
        ...
        <td class="bdl" rowspan="<?php echo $order_info['goods_count'];?>"><p><?php echo $order_info['state_desc']; ?> <?php echo $order_info['evaluation_status'] ? $lang['member_order_evaluated'] : '';?></p>
          ...
          <p><a href="index.php?m=m_order&a=show_order&order_id=<?php echo $order_info['order_id']; ?>" target="_blank"><?php echo $lang['member_order_view_order'];?></a></p>
          
          <!-- 物流跟踪 -->
          
          <?php if ($order_info['if_deliver']){ ?>
          <p><a href='index.php?m=m_order&a=search_deliver&order_id=<?php echo $order_info['order_id']; ?>&order_sn=<?php echo $order_info['order_sn']; ?>' target="_blank"><?php echo $lang['member_order_show_deliver']?></a></p>
          <?php } ?></td>
        <td class="bdl bdr" rowspan="<?php echo $order_info['goods_count'];?>"><!-- 永久删除 --> 
          ...
          <p><a href="javascript:void(0)" class="ncbtn ncbtn-grapefruit" nc_type="dialog" dialog_width="480" dialog_title="<?php echo $lang['member_order_cancel_order'];?>" dialog_id="buyer_order_cancel_order" uri="index.php?m=m_order&a=change_state&state_type=order_cancel&order_id=<?php echo $order_info['order_id']; ?>"  id="order<?php echo $order_info['order_id']; ?>_action_cancel"><i class="icon-ban-circle"></i> <?php echo $lang['member_order_cancel_order'];?></a></p>
          <?php } ?>
          
          <!-- 退款取消订单 -->
          
          <?php if ($order_info['if_refund_cancel']){ ?>
          <p><a href="index.php?m=m_refund&a=add_refund_all&order_id=<?php echo $order_info['order_id']; ?>" class="ncbtn"><i class="icon-legal"></i>订单退款</a></p>
          <?php } ?>
          
          <!-- 收货 -->
          
          <?php if ($order_info['if_receive']) { ?>
          <p><a href="javascript:void(0)" class="ncbtn" nc_type="dialog" dialog_id="buyer_order_confirm_order" dialog_width="480" dialog_title="<?php echo $lang['member_order_ensure_order'];?>" uri="index.php?m=m_order&a=change_state&state_type=order_receive&order_sn=<?php echo $order_info['order_sn']; ?>&order_id=<?php echo $order_info['order_id']; ?>" id="order<?php echo $order_info['order_id']; ?>_action_confirm"><?php echo $lang['member_order_ensure_order'];?></a></p>
          <?php } ?>
          
          <!-- 评价 -->
          
          <?php if ($order_info['if_evaluation']) { ?>
          <p><a class="ncbtn ncbtn-aqua" href="index.php?m=m_evaluate&a=add&order_id=<?php echo $order_info['order_id']; ?>"><i class="icon-thumbs-up-alt"></i><?php echo $lang['member_order_want_evaluate'];?></a></p>
          <?php } ?>
          
          <!-- 追加评价 -->
          
          <?php if ($order_info['if_evaluation_again']) { ?>
          <p><a class="ncbtn" href="index.php?m=m_evaluate&a=add_again&order_id=<?php echo $order_info['order_id']; ?>"><i class="icon-thumbs-up-alt"></i>追加评价</a></p>
          <?php } ?>
          <?php if ($order_info['if_drop']) { ?>
          <p><a href="javascript:void(0);" onclick="ajax_get_confirm('您确定要永久删除吗?永久删除后您将无法再查看该订单，也无法进行投诉维权，请谨慎操作！', 'index.php?m=m_order&a=change_state&state_type=order_drop&order_id=<?php echo $order_info['order_id']; ?>');" class="ncbtn ncbtn-grapefruit mt5"><i class="icon-trash"></i>永久删除</a></p>
          <?php } ?></td>
        <!-- E 合并TD -->
        <?php } ?>
      </tr>
