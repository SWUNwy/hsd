...
class store_activityControl extends BaseSellerControl {
    ...
    public function activity_applyOp(){
        ...
        if($activity_id <= 0){
            showMessage(Language::get('para_error'),'index.php?m=store_activity&a=store_activity','html','error');
        }
        ...
        if(empty($activity_info) || $activity_info['activity_type'] != '1' || $activity_info['activity_state'] != 1 || $activity_info['activity_start_date']>time() || $activity_info['activity_end_date']<time()){
            showMessage(Language::get('store_activity_not_exists'),'index.php?m=store_activity&a=store_activity','html','error');
        }
    ...
    public function activity_apply_saveOp(){
        ...
        if(empty($_POST['item_id'])){
            showDialog(Language::get('store_activity_choose_goods'),'index.php?m=store_activity&a=store_activity');
        }
        $activity_id = intval($_POST['activity_id']);
        if($activity_id <= 0){
            showDialog(Language::get('para_error'),'index.php?m=store_activity&a=store_activity');
        }
        ...
        if(empty($activity) || $activity['activity_type'] != '1' || $activity['activity_state'] != '1' || $activity['activity_start_date']>time() || $activity['activity_end_date']<time()){
            showDialog(Language::get('store_activity_not_exists'),'index.php?m=store_activity&a=store_activity');
        }
    ...
    private function profile_menu($menu_key='') {
        ...
        $menu_array = array(
                1=>array('menu_key'=>'store_activity','menu_name'=>Language::get('nc_member_path_activity_list'),'menu_url'=>'index.php?m=store_activity&a=store_activity'),
