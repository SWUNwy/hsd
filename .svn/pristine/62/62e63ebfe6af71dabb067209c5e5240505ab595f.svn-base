...
<?php if ($notOwnShop = !checkPlatformStore()) { ?>
  <a href="javascript:void(0)" class="ncbtn ncbtn-mint" nc_type="dialog" dialog_title="申请新的经营类目" dialog_id="my_goods_brand_apply" dialog_width="480" uri="index.php?m=store_info&a=bind_class_add">申请新的经营类目</a>
<?php } ?>
  </div>
...
<?php if ($notOwnShop) { ?>
      ...
      <?php if ($val['state'] == '0') {?>
      <span><a href="javascript:void(0)" class="btn-grapefruit" onclick="ajax_get_confirm('<?php echo $lang['nc_ensure_del'];?>', 'index.php?m=store_info&a=bind_class_del&bid=<?php echo $val['bid']; ?>');"><i class="icon-trash"></i><p><?php echo $lang['nc_del'];?></p></a></span>
     <?php } ?>
      </td>
<?php } ?>
    </tr>
