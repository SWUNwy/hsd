$(function(){
    ...
    $('i[nctype="ajaxGoodsList"]').toggle(
        function(){
            ...
            if (_div.html() == '') {
                $.getJSON('index.php?m=store_goods_online&a=get_goods_list_ajax' , {commonid : _commonid}, function(date){
                    if (date != 'false') {
                        ...
                        $.each(date, function(i, o){
                            ...
                            $('<div><a href="javascript:;" class="ncbtn-mini ncbtn-lavander mt5" onclick="ajax_form(\'ajax_jingle\', \'编辑独立描述\', \'index.php?m=store_goods_online&a=edit_iframe_ajax&goods_id=' + o.goods_id + '\', \'915\')">编辑独立描述</a></div>').appendTo(_li);
							if (o.is_distribution != '0') {$('<div><a href="javascript:;" class="ncbtn-mini ncbtn-aqua mt5" onclick="ajax_form(\'ajax_jingle\', \'编辑分销佣金\', \'index.php?m=store_goods_online&a=edit_invite_price&goods_id=' + o.goods_id + '\', \'650\')">编辑分销佣金</a></div>').appendTo(_li);}
                            _li.appendTo(_ul);
                        });
                        _ul.appendTo(_div);
                        _parenttr.next().show();
