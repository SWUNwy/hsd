...
function drop_cart_item(cart_id){
    ...
    showDialog('确认删除吗?', 'confirm', '', function(){
        $.getJSON('index.php?m=cart&a=del&cart_id=' + cart_id, function(result){
            if(result.state){
                //删除成功
...
function change_quantity(cart_id, input){
    ...
    var tax = $('#item' + cart_id + '_tax');
    //暂存为局部变量，否则如果用户输入过快有可能造成前后值不一致的问题
    var _value = input.value;
    $.getJSON('index.php?m=cart&a=update&cart_id=' + cart_id + '&quantity=' + _value, function(result){
        $(input).attr('changed', _value);
        if(result.state == 'true'){
            $('#item' + cart_id + '_price').html(parseFloat(result.goods_price).toFixed(2));
            tax.html(parseFloat(result.goods_tax).toFixed(2));
