...
function loadCompare(isrefresh){
	...
	if(isrefresh == true){
		$("#comparelist").load('index.php?m=compare&a=showcompare');
	}
}
//添加对比商品
function addCompare(gid){
	...
	if (gid > 0){
       	$.ajax({
            type:"GET",
            dataType: "json",
            url:'index.php?m=compare&a=addcompare&id='+gid,  
            async:false,
...
function delCompare(gid,type){	
	$.ajax({
        type:"GET",
        dataType: "json",
        url:'index.php?m=compare&a=delcompare&gid='+gid+'&type='+type,  
        async:false,
        success: function(data){  
            if(data.done == true) {
            	//将对比按钮置为未对比状态
            	...
            	if (type == 'mini'){
                    ...
                    go('index.php?m=compare&gids='+data.gid_str);
                }
            }
            $("#lockcompare").val('unlock');//解除加入对比按钮的锁定
        }
...
function initCompare(){
	...
	//根据是否已加入对比，显示不同样式
	$.getJSON('index.php?m=compare&a=checkcompare', function(data){
        if(data){
