...
class itemindex_searchControl extends BaseHomeControl {
    ...
    public function indexOp() {
		...
		/**
		$test_gc_id = $_GET['gc_id'];
		$test = $this->_model_search->getAttr($test_gc_id, $default_classid);
		print_r($test);
		**/
        //查询消费者保障服务
        $contract_item = array();
        ...
        if (!is_null($goods_list)){
            ...
            if($_GET['gc_id']>0){
                $goods_class = Model()->table("goods_class")->where(array('gc_id'=>$_GET['gc_id']))->find();
                if($goods_class['gc_parent_id']>0){
                     $condition['gc_id_2'] = $_GET['gc_id'];
                }else{
                    $condition['gc_id_1'] = $_GET['gc_id'];
                }
                //$condition['gc_id_1'] = $_GET['gc_id'];161230 by wangchao
            }
            //消费者保障服务
		...
		//$typeList = Model()->table('goods_class')->where(array('gc_parent_id'=>0))->select();161230 by wangchao
        $class = Model()->table('goods_class')->where(array('gc_id'=>$_GET['gc_id']))->find();
        $gc_id = $class['gc_parent_id']>0?$class['gc_parent_id']:$_GET['gc_id'];
		$typeList = Model()->table('goods_class')->where(array('gc_parent_id'=>$gc_id))->select();
		Tpl::output('typeList', $typeList);
		//print_r($typeList);
		...
		$goods_num = $model_goods->getGoodsCommonCount($condition);//计算数量
		Tpl::output('goods_num',  $goods_num);
		
		$goods_num1 = $model_goods->getGoodsCommonOnlineCount($condition);//出售中商品数量
		Tpl::output('goods_num1',  $goods_num1);
		/**
		$countpage = ceil($goods_num1/self::PAGESIZE);//计算最大页数
		Tpl::output('countpage',  $countpage);
		**/
        ...
        $nav_link_list = Model('goods_class')->getGoodsClassNav(intval($_GET['gc_id']),0);
...
//             Log::record('search\auto_complete'.$e->getMessage(),Log::RUN);
    ...
    /**
      * 搜索静态查询
      */
     public function wordtipOp(){
     	$model_goods_words = Model('goods');
     	$condition = array();
        //$condition['chain_id'] = $_SESSION['chain_id'];
    	$wordkey = $_GET['word'];
        if ($wordkey != '') {
        	$condition['goods_name'] =array('like',"%{$wordkey}%");
        }
     	$goods_words_list = $model_goods_words->getGoodsOnlineList($condition,'*','');
     	
     	if(!empty($goods_words_list)){
        	$WordsItemsList = array();
	        foreach($goods_words_list as $key=>$value){
	        	$WordsItems = array();
	        	$WordsItems 			= $value['goods_name'];
	        	$WordsItemsList[] = $WordsItems;
	        }	        
        }
        $postdata = array();
	    $postdata['code'] = 200;
	    $postdata['message'] = "";
	    $postdata['data'] = $WordsItemsList;
	    $postdata['models'] = array();
	    $postdata['totalCount'] = 0;
	    $postdata['totalPage'] = 0;
