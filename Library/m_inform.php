...
class m_informControl extends BaseMemberControl{
    ...
    public function inform_saveOp() {
        ...
        if($model_inform->saveInform($input)) {
            showDialog(Language::get('inform_success'),'index.php?m=m_inform&a=inform_list','succ');
        }
        else {
            showDialog(Language::get('inform_fail'),'index.php?m=m_inform&a=inform_list','error');
        }
    }
    ...
    private function profile_menu($menu_key='') {
        $menu_array = array(
            1=>array('menu_key'=>'inform_list','menu_name'=>'违规举报', 'menu_url'=>'index.php?m=m_inform&a=inform_list'),
        );
