...
class store_paymentControl extends BaseSellerControl{
    ...
    public function real_orderOp(){
        ...
        */
        ...
        foreach ($pay_sn_arr as  $value) {
            if(!preg_match('/^\d{16,18}$/',$value)){
                showMessage('参数错误','','html','error');
            }
        }
        //取订单列表
        $logic_payment = Logic('payment');
        $order_pay_info = $logic_payment->getRealOrderInfo($pay_sn);
        ...
        if (empty($api_pay_amount)) {
            redirect(SHOP_SITE_URL.'/index.php?m=store_buy&a=pay_ok&pay_sn='.$order_pay_info['data']['pay_sn'].'&pay_amount='.ncPriceFormat($pay_amount));
        }
    ...
    private function _api_pay($order_info, $payment_info) {
        ...
        if($payment_info['payment_code'] == 'chinabank'|| $payment_info['payment_code'] == 'allinpay') {
            $payment_api->submit();
        ...
        }else if($payment_info['payment_code'] == 'allinpaysm'){
             if (!extension_loaded('curl')) {
                showMessage('系统curl扩展未加载，请检查系统配置', '', 'html', 'error');
            }
            Tpl::setDir('Newsell');
    ...
    public function returnOp(){
        ...
        $url = SHOP_SITE_URL.'/index.php?m='.$act;
    ...
    public function query_stateOp() {
        /*if ($_GET['pay_id'] && intval($_GET['pay_id']) > 0) {
            $info = Model('order')->getOrderPayInfo(array('pay_id'=>intval($_GET['pay_id'])));
            exit(json_encode(array('state'=>($info['api_pay_state'] == '1'),'pay_sn'=>$info['pay_sn'],'type'=>'r')));
        } elseif (intval($_GET['order_id']) > 0) {
            $info = Model('vr_order')->getOrderInfo(array('order_id'=>intval($_GET['order_id'])));
        ...
        */
        if ($_GET['pay_sn'] != '') {
            ...
            $pay_list = Model('order')->getOrderPayList($condition);
            $flag = true;
            foreach ($pay_list as  $value) {
                if($value['api_pay_state']!=1){
                    $flag = false;
                    break;
                }
            }
            exit(json_encode(array('state'=>$flag,'pay_sn'=>$pay_sn,'type'=>'r')));
        } elseif (intval($_GET['order_id']) > 0) {
            $info = Model('vr_order')->getOrderInfo(array('order_id'=>intval($_GET['order_id'])));
            exit(json_encode(array('state'=>($info['order_state'] == '20'),'pay_sn'=>$info['order_sn'],'type'=>'v')));
        }
