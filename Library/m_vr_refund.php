...
class m_vr_refundControl extends BaseMemberControl {
    ...
    public function add_refundOp(){
        ...
        if ($order_id < 1) {//参数验证
            showDialog(Language::get('wrong_argument'),'index.php?m=m_vr_refund&a=index','error');
        }
        $condition = array();
        ...
        if (!$order['if_refund']) {//检查状态,防止页面刷新不及时造成数据错误
            showDialog(Language::get('wrong_argument'),'index.php?m=m_vr_order&a=index','error');
        }
        if (chksubmit() && $order['if_refund']){
            ...
            if ($state) {
                showDialog(Language::get('nc_common_save_succ'),'index.php?m=m_vr_refund&a=index','succ');
            } else {
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'member_order':
                $menu_array = array(
                array('menu_key'=>'buyer_refund','menu_name'=>Language::get('nc_member_path_buyer_refund'), 'menu_url'=>'index.php?m=m_refund'),
                array('menu_key'=>'buyer_return','menu_name'=>Language::get('nc_member_path_buyer_return'), 'menu_url'=>'index.php?m=m_return'),
                array('menu_key'=>'buyer_vr_refund','menu_name'=>'虚拟兑码退款',    'menu_url'=>'index.php?m=m_vr_refund'));
                break;
        }
