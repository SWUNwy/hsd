...
class s_centerControl extends BaseSellerControl {
...
//         Tpl::output('help_list',$help_list);
    ...
    public function checkPwdOp()
    {
        ...
        $info=array();
        if($userPass==$userPwd2){
            $info['message']=0;
        }else if(md5($userPass)!=$member_info['member_passwd']){
            $info['message']=1;
        }
        else{
            $info['success']=1;
    ...
    public function editPwdOp(){
        $model_member=Model('member');
        $userPass=$_POST['userPass'];
        $res=$model_member->editMember(array('member_id'=>$_SESSION['member_id']),array('member_passwd'=>md5($userPass)));
        $info=array();
        if($res){
            $info['success']=1;
        }        	
