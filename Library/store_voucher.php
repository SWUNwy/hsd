...
class store_voucherControl extends BaseSellerControl{
    ...
    public function __construct() {
        ...
        if (C('voucher_allow') != 1){
            showMessage(Language::get('voucher_unavailable'),'index.php?m=store','html','error');
        }
        //申请记录状态
    ...
    public function voucher_exportOp(){
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){//显示下载链接
                ...
                Tpl::output('murl','index.php?m=store_voucher&a=voucher_export');
    ...
    public function quotaaddOp(){
        if (chksubmit()){
            ...
            if($reault){
                showDialog(Language::get('voucher_apply_buy_succ'),'index.php?m=store_voucher&a=quotalist','succ');
            } else {
                showDialog(Language::get('nc_common_op_fail'),'index.php?m=store_voucher&a=quotalist');
            }
        }else {
            //输出导航
    ...
    public function templateaddOp(){
        ...
        if ($isOwnShop = checkPlatformStore()) {
            ...
            if(empty($quotainfo)){
                showMessage(Language::get('voucher_template_quotanull'),'index.php?m=store_voucher&a=quotaadd','html','error');
            }
            ...
            if ($count >= C('promotion_voucher_storetimes_limit')){
                ...
                showMessage($message,'index.php?m=store_voucher&a=templatelist','html','error');
            }
        }
        ...
        if(empty($pricelist)){
            showMessage(Language::get('voucher_template_pricelisterror'),'index.php?m=store_voucher&a=templatelist','html','error');
        }
        //获取领取方式
        ...
        if(chksubmit()){
            ...
            if ($error){
                ...
                if($rs){
                    ...
                    showDialog(Language::get('nc_common_save_succ'),'index.php?m=store_voucher&a=templatelist','succ');
                }else{
                    showDialog(Language::get('nc_common_save_fail'),'index.php?m=store_voucher&a=templatelist','error');
                }
            }
        }else{
            //店铺分类
    ...
    public function templateeditOp(){
        ...
        if ($t_id <= 0){
            showMessage(Language::get('wrong_argument'),'index.php?m=store_voucher&a=templatelist','html','error');
        }
        $model = Model('voucher');
        //查询模板信息
        $param = array();
        ...
        if(chksubmit()){
            ...
            if ($error){
                ...
                if($rs){
                    ...
                    showDialog(Language::get('nc_common_op_succ'),'index.php?m=store_voucher&a=templatelist','succ');
                }else{
                    showDialog(Language::get('nc_common_op_fail'),'index.php?m=store_voucher&a=templatelist','error');
                }
            }
        }else{
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'voucher':
                $menu_array = array(
                1=>array('menu_key'=>'templatelist','menu_name'=>Language::get('nc_member_path_store_voucher'), 'menu_url'=>'index.php?m=store_voucher&a=templatelist'),
                );
                break;
            case 'quota_add':
                $menu_array = array(
                1=>array('menu_key'=>'templatelist','menu_name'=>Language::get('nc_member_path_store_voucher'), 'menu_url'=>'index.php?m=store_voucher&a=templatelist'),
                4=>array('menu_key'=>'quotaadd','menu_name'=>Language::get('voucher_applyadd'), 'menu_url'=>'index.php?m=store_voucher&a=quotaadd')
                );
                break;
            case 'template':
                $menu_array = array(
                1=>array('menu_key'=>'templatelist','menu_name'=>Language::get('nc_member_path_store_voucher'), 'menu_url'=>'index.php?m=store_voucher&a=templatelist'),
                2=>array('menu_key'=>'templateadd','menu_name'=>Language::get('voucher_templateadd'),   'menu_url'=>'index.php?m=store_voucher&a=templateadd'),
                );
                break;
            case 'templateedit':
                $menu_array = array(
