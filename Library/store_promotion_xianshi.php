...
class store_promotion_xianshiControl extends BaseSellerControl {
    ...
    const LINK_XIANSHI_LIST = 'index.php?m=store_promotion_xianshi&a=xianshi_list';
    const LINK_XIANSHI_MANAGE = 'index.php?m=store_promotion_xianshi&a=xianshi_manage&xianshi_id=';
    ...
    public function __construct() {
        ...
        if (intval(C('promotion_allow')) !== 1){
            showMessage(Language::get('promotion_unavailable'),'index.php?m=store','','error');
        }
    ...
    private function profile_menu($menu_key='') {
        $menu_array = array(
            1=>array('menu_key'=>'xianshi_list','menu_name'=>Language::get('promotion_active_list'),'menu_url'=>'index.php?m=store_promotion_xianshi&a=xianshi_list'),
        );
        switch ($menu_key){
            case 'xianshi_add':
                $menu_array[] = array('menu_key'=>'xianshi_add','menu_name'=>Language::get('promotion_join_active'),'menu_url'=>'index.php?m=store_promotion_xianshi&a=xianshi_add');
                break;
            case 'xianshi_edit':
                $menu_array[] = array('menu_key'=>'xianshi_edit','menu_name'=>'编辑活动','menu_url'=>'javascript:;');
                break;
            case 'xianshi_quota_add':
                $menu_array[] = array('menu_key'=>'xianshi_quota_add','menu_name'=>Language::get('promotion_buy_product'),'menu_url'=>'index.php?m=store_promotion_xianshi&a=xianshi_quota_add');
                break;
            case 'xianshi_manage':
                $menu_array[] = array('menu_key'=>'xianshi_manage','menu_name'=>Language::get('promotion_goods_manage'),'menu_url'=>'index.php?m=store_promotion_xianshi&a=xianshi_manage&xianshi_id='.$_GET['xianshi_id']);
                break;
        }
