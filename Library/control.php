...
//              @header('location: index.php?m=s_login&a=show_login');die;
//          }
//
//          // 验证店铺是否存在
    ...
    private function _getMenuList() {
        $menu_list = array(
            'trade' => array('name' => '交易中心', 'child' => array(
                'member_order'      => array('name' => '实物交易订单', 'url' => urlShop('m_order', 'index')),
                'member_vr_order'   => array('name' => '虚拟兑码订单', 'url' => urlShop('m_vr_order', 'index')),
                'member_evaluate'   => array('name' => '交易评价/晒单', 'url' => urlShop('m_evaluate', 'list')),
                'member_appoint'    => array('name' => '预约/到货通知', 'url' => urlShop('m_appoint', 'list'))
            )),
            'follow' => array('name' => '关注中心', 'child' => array(
                'member_favorite_goods' => array('name' => '商品收藏', 'url' => urlShop('m_favorite_goods', 'index')),
                'member_favorite_store' => array('name' => '店铺收藏', 'url' => urlShop('m_favorite_store', 'index')),
                'member_goodsbrowse'   => array('name' => '我的足迹', 'url' => urlShop('m_goodsbrowse', 'list'))
            )),
            'client' => array('name' => '客户服务', 'child' => array(
                'member_refund'     => array('name' => '退款及退货', 'url' => urlShop('m_refund', 'index')),
                'member_complain'   => array('name' => '交易投诉', 'url' => urlShop('m_complain', 'index')),
                'member_consult'    => array('name' => '商品咨询', 'url' => urlShop('m_consult', 'my_consult')),
                'member_inform'     => array('name' => '违规举报', 'url' => urlShop('m_inform', 'index')),
                'member_mallconsult'=> array('name' => '平台客服', 'url' => urlShop('m_mallconsult', 'index'))
            )),
            'info' => array('name' => '会员资料', 'child' => array(
                'member_information'=> array('name' => '账户信息', 'url'=>urlMember('m_information', 'member')),
                'member_address'    => array('name' => '收货地址', 'url'=>urlMember('m_address', 'address'))
            )),
            'property' => array('name' => '财产中心', 'child' => array(
                'predeposit'        => array('name' => '账户余额', 'url'=>urlMember('predeposit', 'pd_log_list')),
                'member_voucher'    => array('name' => '我的代金券', 'url'=>urlMember('m_voucher', 'index')),
                'member_redpacket'  => array('name' => '我的红包', 'url'=>urlMember('m_redpacket', 'index'))
            )),
        );
        return $menu_list;
    }
...
class BaseSNSControl extends Control {
    ...
    public function get_member_info() {
        ...
        if(empty($member_info)){
            showMessage(L('wrong_argument'), 'index.php?m=m_snshome', '', 'error');
        }
        //粉丝数
    ...
    private function check_relation(){
        ...
        if ($this->master_id <= 0){
            if ($_SESSION['is_login'] == 1){
                ...
                @header("location: " . urlLogin('login', 'index', array('ref_url' => urlShop('m_snshome'))));
            }
        }
...
//          }
//
//          if ($_SESSION['seller_is_admin'] !== 1 && $_GET['m'] !== 's_center' && $_GET['m'] !== 's_logout') {
//              if (!in_array($_GET['m'], $_SESSION['seller_limits'])) {
...
//          }
    ...
    protected function recordSellerLog($content = '', $state = 1){
        ...
        $seller_info['log_url'] = $_GET['m'].'&'.$_GET['a'];
        ...
        if (intval($is_admin) !== 1) {
            ...
            foreach ($menu_list as $key => $value) {
                foreach ($value['child'] as $child_key => $child_value) {
                    if (!in_array($child_value['m'], $limits)) {
    ...
    private function _getCurrentMenu($seller_function_list) {
        $current_menu = $seller_function_list[$_GET['m']];
    ...
    private function _getMenuList() {
        $menu_list = array(        
                'store' => array('name' => '店铺', 'child' => array(
                    array('name' => '平台公告', 'm'=>'store_article', 'a'=>'index'),                  
                )),                       
                 'order' => array('name' => '订单', 'child' => array(
                    array('name' => '所有订单', 'm'=>'store_order', 'a'=>'index'),
                    array('name' => '待支付订单', 'm'=>'store_order', 'a'=>'index','state_type'=>"state_new"),
                )),
                'goods' => array('name' => '商品', 'child' => array(
                    array('name' => '商品列表', 'm'=>'itemIndex', 'a'=>'index'),                
                ))
            ); 
        return $menu_list;           
    }

    private function _getMenuList1() {
        $menu_list = array(
            'goods' => array('name' => '商品', 'child' => array(
                array('name' => '商品发布', 'm'=>'store_goods_add', 'a'=>'index'),
				array('name' => 'CSV导入', 'm'=>'taobao_import', 'a'=>'index'),
                array('name' => '出售中的商品', 'm'=>'itemIndex', 'a'=>'index'),
                array('name' => '仓库中的商品', 'm'=>'store_goods_offline', 'a'=>'index'),
                array('name' => '预约/到货通知', 'm' => 'store_appoint', 'a' => 'index'),
                array('name' => '关联版式', 'm'=>'store_plate', 'a'=>'index'),
                array('name' => '商品规格', 'm' => 'store_spec', 'a' => 'index'),
                array('name' => '图片空间', 'm'=>'store_album', 'a'=>'album_cate'),
            )),
            'order' => array('name' => '订单物流', 'child' => array(
                array('name' => '实物交易订单', 'm'=>'store_order', 'a'=>'index'),
                array('name' => '虚拟兑码订单', 'm'=>'store_vr_order', 'a'=>'index'),
                array('name' => '发货', 'm'=>'store_deliver', 'a'=>'index'),
                array('name' => '发货设置', 'm'=>'store_deliver_set', 'a'=>'daddress_list'),
                array('name' => '运单模板', 'm'=>'store_waybill', 'a'=>'waybill_manage'),
                array('name' => '评价管理', 'm'=>'store_evaluate', 'a'=>'list'),
                array('name' => '物流工具', 'm'=>'store_transport', 'a'=>'index'),
				array('name' => '来单提醒', 'm'=>'order_call', 'a'=>'index'),
            )),
            'promotion' => array('name' => '促销', 'child' => array(
                array('name' => '团购管理', 'm'=>'store_groupbuy', 'a'=>'index'),
                array('name' => '加价购', 'm'=>'store_promotion_cou', 'a'=>'cou_list'),
                array('name' => '限时折扣', 'm'=>'store_promotion_xianshi', 'a'=>'xianshi_list'),
                array('name' => '满即送', 'm'=>'store_promotion_mansong', 'a'=>'mansong_list'),
                array('name' => '优惠套装', 'm'=>'store_promotion_bundling', 'a'=>'bundling_list'),
                array('name' => '推荐展位', 'm' => 'store_promotion_booth', 'a' => 'booth_goods_list'),
                array('name' => '预售商品', 'm' => 'store_promotion_book', 'a' => 'index'),
                array('name' => 'F码商品', 'm' => 'store_promotion_fcode', 'a' => 'index'),
                array('name' => '推荐组合', 'm' => 'store_promotion_combo', 'a' => 'index'),
                array('name' => '手机专享', 'm' => 'store_promotion_sole', 'a' => 'index'),
                array('name' => '代金券管理', 'm'=>'store_voucher', 'a'=>'templatelist'),
                array('name' => '活动管理', 'm'=>'store_activity', 'a'=>'store_activity'),
            )),
            'store' => array('name' => '店铺', 'child' => array(
                array('name' => '店铺设置', 'm'=>'store_setting', 'a'=>'store_setting'),
                array('name' => '店铺装修', 'm'=>'store_decoration', 'a'=>'decoration_setting'),
                array('name' => '店铺导航', 'm'=>'store_navigation', 'a'=>'navigation_list'),
                array('name' => '店铺动态', 'm'=>'store_sns', 'a'=>'index'),
                array('name' => '店铺信息', 'm'=>'store_info', 'a'=>'bind_class'),
                array('name' => '店铺分类', 'm'=>'store_goods_class', 'a'=>'index'),
                array('name' => '品牌申请', 'm'=>'store_brand', 'a'=>'brand_list'),
                array('name' => '供货商', 'm'=>'store_supplier', 'a'=>'sup_list'),
                array('name' => '实体店铺', 'm'=>'store_map', 'a'=>'index'),
                array('name' => '消费者保障服务', 'm'=>'store_contract', 'a'=>'index'),
            )),
            'consult' => array('name' => '售后服务', 'child' => array(
                array('name' => '咨询管理', 'm'=>'store_consult', 'a'=>'consult_list'),
                array('name' => '投诉管理', 'm'=>'store_complain', 'a'=>'list'),
                array('name' => '退款记录', 'm'=>'store_refund', 'a'=>'index'),
                array('name' => '退货记录', 'm'=>'store_return', 'a'=>'index'),
            )),
            'statistics' => array('name' => '统计结算', 'child' => array(
                array('name' => '店铺概况', 'm'=>'statistics_general', 'a'=>'general'),
                array('name' => '商品分析', 'm'=>'statistics_goods', 'a'=>'goodslist'),
                array('name' => '运营报告', 'm'=>'statistics_sale', 'a'=>'sale'),
                array('name' => '行业分析', 'm'=>'statistics_industry', 'a'=>'hot'),
                array('name' => '流量统计', 'm'=>'statistics_flow', 'a'=>'storeflow'),
                array('name' => '实物结算', 'm'=>'store_bill', 'a'=>'index'),
                array('name' => '虚拟结算', 'm'=>'store_vr_bill', 'a'=>'index'),
            )),
            'message' => array('name' => '客服消息', 'child' => array(
                array('name' => '客服设置', 'm'=>'store_callcenter', 'a'=>'index'),
                array('name' => '系统消息', 'm'=>'store_msg', 'a'=>'index'),
                array('name' => '聊天记录查询', 'm'=>'store_im', 'a'=>'index'),
            )),
            'account' => array('name' => '账号', 'child' => array(
                array('name' => '账号列表', 'm'=>'store_account', 'a'=>'account_list'),
                array('name' => '账号组', 'm'=>'store_account_group', 'a'=>'group_list'),
                array('name' => '账号日志', 'm'=>'seller_log', 'a'=>'log_list'),
                array('name' => '店铺消费', 'm'=>'store_cost', 'a'=>'cost_list'),
                array('name' => '门店账号', 'm'=>'store_chain', 'a'=>'index'),
            )),
			'webchat' => array('name' => '微信', 'child' => array(
                array('name' => '微信接口管理', 'm'=>'seller_wechat', 'a'=>'index'),
                array('name' => '关注自动回复', 'm'=>'seller_wechat_follow', 'a'=>'follow_index'),
                array('name' => '关键词自动回复', 'm'=>'seller_wechat_keyword', 'a'=>'keyword_index'),
                array('name' => '消息自动回复', 'm'=>'seller_wechat_message', 'a'=>'message_index'),
                array('name' => '自定义菜单', 'm'=>'seller_wechat_menu', 'a'=>'index'),
            ))
        );
        return $menu_list;
    }
    ...
    private function _getSellerFunctionList($menu_list) {    	
        ...
        foreach ($menu_list as $key => $menu_value) {
            foreach ($menu_value['child'] as $submenu_value) {
                $format_menu[$submenu_value['m']] = array(
                    'model' => $key,
