...
class store_vr_billControl extends BaseSellerControl {
    ...
    public function export_orderOp(){
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){
                ...
                Tpl::output('murl','index.php?m=store_vr_bill&a=show_bill&ob_id='.$_GET['ob_id']);
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'list':
                $menu_array = array(
                1=>array('menu_key'=>'list','menu_name'=>'虚拟订单结算', 'menu_url'=>'index.php?m=bill&a=list'),
                );
                break;
            case 'show':
                $menu_array = array(
                array('menu_key'=>'','menu_name'=>'已使用', 'menu_url'=>'index.php?m=store_vr_bill&a=show_bill&ob_id='.$_GET['ob_id']),
                array('menu_key'=>'timeout','menu_name'=>'已过期',  'menu_url'=>'index.php?m=store_vr_bill&a=show_bill&type=timeout&ob_id='.$_GET['ob_id'])
                );
                break;
        }
