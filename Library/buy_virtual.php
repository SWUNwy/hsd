...
class buy_virtualControl extends BaseBuyControl {
    ...
    public function buy_step3Op() {
        ...
        //转向到商城支付页面
        redirect('index.php?m=buy_virtual&a=pay&order_id='.$result['data']['order_id']);
    }
    ...
    public function payOp() {
        ...
        if ($order_id <= 0){
            showMessage('该订单不存在','index.php?m=m_vr_order','html','error');
        }
        ...
        if (empty($payment_list)) {
            showMessage('暂未找到合适的支付方式','index.php?m=m_vr_order','html','error');
        }
