...
class goods_itemIndexControl extends BaseGoodsControl {
    ...
    public function indexOp() {
        ...
        // 看了又看（同分类本店随机商品）
        $size = $goods_info['is_own_shop'] ? 8 : 8;
		...
		//取商品税金
		$goods_info = $model_goods->getGoodsTax($goods_info);
        //处理商品消费者保障服务信息
		...
		$ci_storage_list = Model("ci_storage")->getCiStroageList(array());
        ...
        if(!empty($goods_info['goods_spec'])){
            $str = end($goods_info['goods_spec']);
            preg_match('/^\d+/',$str,$content);
            if($content[0]>0){
               $num = $content[0];
            }
        }
        $goods_unit_price =  round($goods_info['goods_price']/$num,2);
        Tpl::output('goods_unit_price',$goods_unit_price);
        Tpl::showpage('item');
    }
    /**
     * 记录浏览历史
