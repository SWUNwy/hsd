...
class m_snsindexControl extends BaseMemberControl {
    ...
    public function indexOp(){
        ...
        //信息输出
        Tpl::output('nchash',substr(md5(SHOP_SITE_URL.$_GET['m'].$_GET['a']),0,8));
    ...
    public function addtraceOp(){
        ...
        if ($result){
            ...
            $js .= "$(\"#content_weibo\").val('');$(\"#privacy\").val('0');$('#friendtrace').lazyshow({url:\"index.php?m=m_snshome&a=tracelist&curpage=1\",'iIntervalId':true});";
    ...
    public function sharegoodsOp(){
        ...
        if ($_POST['form_submit'] == 'ok'){
            ...
            if ($result){
                ...
                $url = '';
                if ($_GET['irefresh']){
                    $js .= "$('#friendtrace').lazyshow({url:\"index.php?m=m_snsindex&a=tracelist&curpage=1\",'iIntervalId':true});";
                }else{
                    $url = 'reload';
                }
    ...
    public function deltraceOp(){
        ...
        if ($result){
            ...
            if ($_GET['type'] == 'href'){
                showDialog(Language::get('nc_common_del_succ'),'index.php?m=m_snshome&a=trace&mid='.$_SESSION['member_id'],'succ');
            }else {
                $js = "location.reload();";
    ...
    public function addcommentOp(){
        ...
        if ($result){
            ...
            if ($_POST['showtype'] == 1){
                $js .="$(\"#tracereply_{$originalid}\").load('index.php?m=m_snshome&a=commenttop&mid={$owner_id}&id={$originalid}&type={$originaltype}');";
            }else {
                $js .="$(\"#tracereply_{$originalid}\").load('index.php?m=m_snshome&a=commentlist&mid={$owner_id}&id={$originalid}&type={$originaltype}');";
            }
    ...
    public function addforwardOp(){
        ...
        if ($tracelog_info['trace_originalid'] > 0 || $tracelog_info['trace_from'] != 1){
            ...
            $content_str ="<div class=\"title\"><a href=\"%siteurl%index.php?m=m_snshome&mid={$tracelog_info['trace_memberid']}\" target=\"_blank\" class=\"uname\">{$tracelog_info['trace_membername']}</a>";
        ...
        if ($result){
            ...
            if ($_GET['irefresh']){
                ...
                if ($_GET['type']=='home'){
                    $js = "$('#friendtrace').lazyshow({url:\"index.php?m=m_snshome&a=tracelist&mid={$tracelog_info['trace_memberid']}&curpage=1\",'iIntervalId':true});";
                }else if ($_GET['type']=='snshome'){
                    $js = "$('#forward_".$originalid."').hide();$('#friendtrace').lazyshow({url:\"index.php?m=m_snshome&a=tracelist&mid={$tracelog_info['trace_memberid']}&curpage=1\",'iIntervalId':true});";
                }else {
                    $js = "$('#friendtrace').lazyshow({url:\"index.php?m=m_snsindex&a=tracelist&curpage=1\",'iIntervalId':true});";
                }
    ...
    public function sharegoods_oneOp(){
        ...
        if ($_GET['dialog']){
            $js = "CUR_DIALOG = ajax_form('sharegoods', '".Language::get('sns_sharegoods_tofriend')."', 'index.php?m=m_snsindex&a=sharegoods_one&gid={$gid}', 480);";
            showDialog('','','js',$js);
        }
        //查询商品信息
    ...
    public function sharestore_oneOp(){
        ...
        if ($_GET['dialog']){
            $js = "ajax_form('sharestore', '".Language::get('sns_sharestore')."', 'index.php?m=m_snsindex&a=sharestore_one&sid={$sid}', 480);";
            showDialog('','','js',$js);
        }
        //查询店铺信息
        $store_model = Model('store');
