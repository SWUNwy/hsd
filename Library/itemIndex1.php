...
class itemindexControl extends BaseSellerControl {
    //每页显示商品数
    const PAGESIZE = 20;
    ...
    public function indexOp() {
        ...
        $goods_common1 = $model_common->getGoodsOnlineList(array('gc_id_1'=>5),'*',10,'rand()');
        ...
        $goods_common2 = $model_common->getGoodsOnlineList(array('gc_id_1'=>4),'*',10,'rand()');
        Tpl::output('goods_common2', $goods_common2);
        //其他
        $goods_common3 = $model_common->getGoodsList(array('gc_id'=>array('in','263,35,109')),'*','','', 0, 10);
	 ...
	 * 导航列表
	 */
	public function getIndexCategoryListOp(){
		$IndexCategoryList = Model()->table('goods_class')->where(array('gc_parent_id'=>0))->select();
		if(!empty($IndexCategoryList)){
			$CategoryList = array();
	        foreach($IndexCategoryList as $key=>$value){
	        	$Items = array();
	        	$Items['categoryId'] 		= $value['gc_id'];
	        	$Items['categoryName'] 		= $value['gc_name'];
	        	$Items['orderNo'] 			= $value['gc_sort'];
	        	$Items['parentId'] 			= $value['gc_parent_id'];
	        	$Items['isHide'] 			= $value['gc_show'];
	        	$Items['level'] 			= 1;
	        	$Items['type'] 				= 1;
	        	$Items['tags'] 				= "";
	        	$Items['propertyDTOList'] 	= array();
	        	$Items['imagePath'] 		= "";
	        	$Items['iconFont'] 			= "";
	        	$Items['brandDTOList'] 		= "";
	        	$CategoryList[] = $Items;
