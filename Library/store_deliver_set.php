...
class store_deliver_setControl extends BaseSellerControl {
    ...
    public function daddress_addOp() {
        ...
        if (chksubmit()) {
            ...
            if (empty($address_info) && !is_array($address_info)){
                showMessage($lang['store_daddress_wrong_argument'],'index.php?m=store_deliver_set&a=daddress_list','html','error');
            }
    ...
    public function daddress_delOp() {
        ...
        if ($delete){
            showDialog(Language::get('store_daddress_del_succ'),'index.php?m=store_deliver_set&a=daddress_list','succ');
        }else {
            showDialog(Language::get('store_daddress_del_fail'),'','error');
        }
    }
   ...
   public function daddress_default_setOp() {
    ...
    public function print_setOp(){
        ...
        if(empty($store_info)){
            showDialog(Language::get('store_storeinfo_error'),'index.php?m=store_printsetup','error');
        }
        if(chksubmit()){
            $obj_validate = new Validate();
            $obj_validate->validateparam = array(
    ...
    private function profile_menu($menu_type, $menu_key = '') {
        ...
        switch ($menu_type) {
            case 'daddress':
                $menu_array = array(
                array('menu_key'=>'daddress',   'menu_name'=>Language::get('store_deliver_daddress_list'),  'menu_url'=>'index.php?m=store_deliver_set&a=daddress_list'),
                array('menu_key'=>'express',        'menu_name'=>Language::get('store_deliver_default_express'),    'menu_url'=>'index.php?m=store_deliver_set&a=express'),
                array('menu_key'=>'free_freight',   'menu_name'=>'免运费额度',      'menu_url'=>'index.php?m=store_deliver_set&a=free_freight'),
                array('menu_key'=>'deliver_region',   'menu_name'=>'默认配送地区',      'menu_url'=>'index.php?m=store_deliver_set&a=deliver_region'),
                array('menu_key'=>'print_set', 'menu_name'=>'发货单打印设置', 'menu_url'=>'index.php?m=store_deliver_set&a=print_set')
                );
                break;
        }
