      ...
      <tbody>
        ...
        <tr>
          ...
          <td class="bdl" rowspan="<?php echo count($output['prod_list']);?>"><?php if ($output['order_info']['point_orderallowcancel']) { ?>
            <p><a href="javascript:void(0)" class="ncbtn ncbtn-bittersweet" onclick="ajax_confirm('<?php echo $lang['member_pointorder_cancel_confirmtip']; ?>','<?php echo MEMBER_SITE_URL;?>/index.php?m=m_pointorder&a=cancel_order&order_id=<?php echo $output['order_info']['point_orderid']; ?>');"><?php echo $lang['member_pointorder_cancel_title']; ?></a></p>
            <?php } ?>
            <?php if ($output['order_info']['point_orderallowreceiving']) { ?>
            <p><a href="javascript:void(0)" class="ncbtn" onclick="ajax_confirm('<?php echo $lang['member_pointorder_confirmreceivingtip']; ?>','<?php echo MEMBER_SITE_URL;?>/index.php?m=m_pointorder&a=receiving_order&order_id=<?php echo $output['order_info']['point_orderid']; ?>');"><?php echo $lang['member_pointorder_confirmreceiving']; ?></a></p>
            <?php } ?></td>
          <?php } ?>
        </tr>
        <?php } }?>
...
$(function(){
    $('#show_shipping').on('hover',function(){
    	...
    	$.getJSON('index.php?m=m_pointorder&a=get_express&e_code=<?php echo $output['express_info']['e_code']?>&shipping_code=<?php echo $output['order_info']['point_shippingcode']?>&t=<?php echo random(7);?>',function(data){
    		if(data){
