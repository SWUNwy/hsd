...
class m_securityControl extends BaseMemberControl {
    ...
    public function send_bind_emailOp() {
        ...
        $verify_url = MEMBER_SITE_URL.'/index.php?m=login&a=bind_email&uid='.$uid.'&hash='.md5($seed);
	    ...
	    showDialog('验证邮件已经发送至您的邮箱，请于24小时内登录邮箱并完成验证！','index.php?m=m_security&a=index','succ','',5);
    ...
    public function authOp() {
        ...
        if (chksubmit(false,true)) {
            if (!in_array($_POST['type'],array('modify_pwd','modify_mobile','modify_email','modify_paypwd','pd_cash'))) {
                redirect('index.php?m=m_security&a=index');
            }
            ...
            $member_info['member_mobile_bind'] == '0') {
                showMessage('请先绑定邮箱或手机','index.php?m=m_security&a=index','html','error');
            }
    ...
    public function modify_pwdOp() {
        ...
        if (TIMESTAMP - $_SESSION['auth_modify_pwd'] > 1800) {
            showDialog('操作超时，请重新获得验证码','index.php?m=m_security&a=auth&type=modify_pwd','html','error');
        }
        ...
        showDialog($message,'index.php?m=m_security&a=index',$update ? 'succ' : 'error');
    ...
    public function modify_paypwdOp() {
        ...
        if (TIMESTAMP - $_SESSION['auth_modify_paypwd'] > 1800) {
            showMessage('操作超时，请重新获得验证码','index.php?m=m_security&a=auth&type=modify_paypwd','html','error');
        }
    ...
    public function modify_mobileOp() {
        ...
        if (chksubmit()) {
            ...
            showDialog('手机号绑定成功','index.php?m=m_security&a=index','succ');
        }
    }
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'index':
                $menu_array = array(
                array('menu_key'=>'index', 'menu_name'=>'账户安全','menu_url'=>'index.php?m=m_security&a=index'),
                );
                break;
            case 'modify_pwd':
                $menu_array = array(
                array('menu_key'=>'index', 'menu_name'=>'账户安全','menu_url'=>'index.php?m=m_security&a=index'),
                array('menu_key'=>'modify_pwd','menu_name'=>'修改登录密码','menu_url'=>'index.php?m=m_security&a=auth&type=modify_pwd'),
                );
                break;
            case 'modify_email':
                $menu_array = array(
                array('menu_key'=>'index', 'menu_name'=>'账户安全','menu_url'=>'index.php?m=m_security&a=index'),
                array('menu_key'=>'modify_email', 'menu_name'=>'邮箱验证','menu_url'=>'index.php?m=m_security&a=auth&type=modify_email'),
                );
                break;
            case 'modify_mobile':
                $menu_array = array(
                array('menu_key'=>'index', 'menu_name'=>'账户安全','menu_url'=>'index.php?m=m_security&a=index'),
                array('menu_key'=>'modify_mobile','menu_name'=>'手机验证','menu_url'=>'index.php?m=m_security&a=auth&type=modify_mobile'),
                );
                break;
            case 'modify_paypwd':
                $menu_array = array(
                array('menu_key'=>'index', 'menu_name'=>'账户安全','menu_url'=>'index.php?m=m_security&a=index'),
                array('menu_key'=>'modify_paypwd','menu_name'=>'设置支付密码','menu_url'=>'index.php?m=m_security&a=auth&type=modify_paypwd'),
                );
                break;
            case 'pd_cash':
                $menu_array = array(
                array('menu_key'=>'loglist','menu_name'=>'账户余额',    'menu_url'=>'index.php?m=predeposit&a=pd_log_list'),
                array('menu_key'=>'recharge_list','menu_name'=>'充值明细', 'menu_url'=>'index.php?m=predeposit&a=index'),
                array('menu_key'=>'cashlist','menu_name'=>'余额提现', 'menu_url'=>'index.php?m=predeposit&a=pd_cash_list'),
                array('menu_key'=>'pd_cash','menu_name'=>'提现申请','menu_url'=>'index.php?m=m_security&a=auth&type=pd_cash'),
                );
                break;
        }
