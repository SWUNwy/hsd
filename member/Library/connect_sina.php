...
class connect_sinaControl extends BaseLoginControl{
    ...
    public function bindsinaOp(){
        ...
        if (is_array($member_info) && count($member_info)>0){
            ...
            showMessage(Language::get('home_sconnect_binding_exist'),urlMember('m_bind', 'sinabind'),'html','error');
        }
        //处理sina账号信息
        require_once (BASE_PATH.DS.'api'.DS.'sina'.DS.'saetv2.ex.class.php');
        $c = new SaeTClientV2( C('sina_wb_akey'), C('sina_wb_skey') , $_SESSION['slast_key']['access_token']);
        ...
        if ($edit_state){
            showMessage(Language::get('home_sconnect_binding_success'),urlMember('m_bind', 'sinabind'));
        }else {
            showMessage(Language::get('home_sconnect_binding_fail'),urlMember('m_bind', 'sinabind'),'html','error');
        }
    }
}
