...
class predepositControl extends BaseMemberControl {
    ...
    public function recharge_addOp(){
        ...
        if ($insert) {
            //转向到商城支付页面
            redirect(SHOP_SITE_URL . '/index.php?m=buy&a=pd_pay&pay_sn='.$pay_sn);
        }
    }
    ...
    public function pd_cash_addOp(){
        if (chksubmit()){
            ...
            if (floatval($member_info['available_predeposit']) < $pdc_amount){
                showDialog(Language::get('predeposit_cash_shortprice_error'),'index.php?m=predeposit&a=pd_cash_list','error');
            }
            try {
                ...
                $model_pd->commit();
                showDialog(Language::get('predeposit_cash_add_success'),'index.php?m=predeposit&a=pd_cash_list','succ','CUR_DIALOG.close()');
            } catch (Exception $e) {
                $model_pd->rollback();
                showDialog($e->getMessage(),'index.php?m=predeposit&a=pd_cash_list','error');
            }
        }
    }
    ...
    public function pd_cash_infoOp(){
        ...
        if ($pdc_id <= 0){
            showMessage(Language::get('predeposit_parameter_error'),'index.php?m=predeposit&a=pd_cash_list','html','error');
        }
        $model_pd = Model('predeposit');
        $condition = array();
        $condition['pdc_member_id'] = $_SESSION['member_id'];
        ...
        if (empty($info)){
            showMessage(Language::get('predeposit_record_error'),'index.php?m=predeposit&a=pd_cash_list','html','error');
        }
    ...
    private function profile_menu($menu_type,$menu_key=''){
        $menu_array = array(
            ...
            array('menu_key'=>'rcb_log_list',   'menu_name'=>'充值卡余额',   'menu_url'=>'index.php?m=predeposit&a=rcb_log_list',),
        );
        switch ($menu_type) {
            case 'rechargeinfo':
            ...
            case 'rechargecard_add':
                ...
                break;
            case 'cashadd':
                $menu_array[] = array('menu_key'=>'cashadd','menu_name'=>'提现申请',    'menu_url'=>'index.php?m=predeposit&a=pd_cash_add');
                break;
            case 'cashinfo':
