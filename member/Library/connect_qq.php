...
class connect_qqControl extends BaseLoginControl{
    ...
    public function bindqqOp(){
        ...
        if (is_array($member_info) && count($member_info)>0){
            ...
            showMessage(Language::get('home_qqconnect_binding_exist'),urlMember('m_bind', 'qqbind'),'html','error');//'该QQ账号已经绑定其他商城账号,请使用其他QQ账号与本账号绑定'
        }
        //获取qq账号信息
        require_once (BASE_PATH.'/api/qq/user/get_user_info.php');
        $qquser_info = get_user_info($_SESSION["appid"], $_SESSION["appkey"], $_SESSION["token"], $_SESSION["secret"], $_SESSION["openid"]);
        ...
        if ($edit_state){
            showMessage(Language::get('home_qqconnect_binding_success'),urlMember('m_bind', 'qqbind'));
        }else {
            showMessage(Language::get('home_qqconnect_binding_fail'),urlMember('m_bind', 'qqbind'),'html','error');//'绑定QQ失败'
        }
    }
    /**
     * 绑定qq后自动登录
