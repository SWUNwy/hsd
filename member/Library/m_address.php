...
class m_addressControl extends BaseMemberControl{
    /**
     * 会员地址
     *
     * @param
    ...
    public function addressOp() {
        ...
        if (!empty($_GET['type'])){
            ...
            if (intval($_GET['id']) > 0){
                ...
                if ($address_info['member_id'] != $_SESSION['member_id']){
                    showMessage($lang['member_address_wrong_argument'],'index.php?m=m_address&a=address','html','error');
                }
                /**
                 * 输出地址信息
                 */
        ...
        if ($del_id > 0){
            ...
            if ($rs){
                showDialog(Language::get('member_address_del_succ'),'index.php?m=m_address&a=address','js');
            }else {
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'address':
                $menu_array = array(
                1=>array('menu_key'=>'address','menu_name'=>'地址列表',   'menu_url'=>'index.php?m=m_adderss&a=address'));
                break;
        }
