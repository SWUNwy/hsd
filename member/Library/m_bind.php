...
class m_bindControl extends BaseMemberControl {
    ...
    public function qqunbindOp(){
        ...
        session_unset();
        session_destroy();
        showMessage(Language::get('member_qqconnect_unbind_success'),urlLogin('login', 'index', array('ref_url' => urlMember('m_bind', 'qqbind'))));
    }
    /**
     * 新浪绑定
     */
    ...
    public function sinaunbindOp(){
        ...
        session_unset();
        session_destroy();
        showMessage(Language::get('member_sconnect_unbind_success'),urlLogin('login', 'index', array('ref_url' => urlMember('m_bind', 'sinabind'))));
    }
    /**
     * 微信绑定
     */
    ...
    public function weixinunbindOp(){
        ...
        session_unset();
        session_destroy();
        showMessage('微信解绑成功',urlLogin('login', 'index', array('ref_url' => urlMember('m_bind', 'weixinbind'))));
    }
    /**
     * 用户中心右边，小导航
     *
    ...
    private function profile_menu($menu_key='',$array=array()) {
        ...
        $menu_array     = array();
        $menu_array = array(
            1=>array('menu_key'=>'qq_bind', 'menu_name'=>'QQ绑定',   'menu_url'=>'index.php?m=m_bind&a=qqbind'),
            2=>array('menu_key'=>'sina_bind',   'menu_name'=>'新浪绑定', 'menu_url'=>'index.php?m=m_bind&a=sinabind'),
            3=>array('menu_key'=>'weixin_bind',   'menu_name'=>'微信绑定', 'menu_url'=>'index.php?m=m_bind&a=weixinbind'),
        );
        if(!empty($array)) {
            $menu_array[] = $array;
        }
