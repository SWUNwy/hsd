...
class loginControl extends BaseLoginControl {
    ...
    public function indexOp(){
        ...
        if ($_GET['inajax'] == 1 && C('captcha_status_login') == '1'){
            $script = "document.getElementById('codeimage').src='index.php?m=seccode&a=makecode&nchash=".getNchash()."&t=' + Math.random();";
        }
        ...
        if ($result !== false){
            ...
            if(empty($_GET['ref_url'])) {
                $ref_url = getReferer();
                if (!preg_match('/m=login&a=logout/', $ref_url)) {
		...
		*/
		if(C('ucenter_status')) {
			...
			*/
			...
			showMessage($lang['login_logout_success'],'index.php?m=login&ref_url='.urlencode($ref_url));			
		}else{
			redirect();
		}
	}
    ...
    public function registerOp() {
            ...
            {
	    $rec_id = $_GET['rec'];
		if(!empty($rec_id)){header('Location:'.WAP_SITE_URL.'/tmpl/member/register1.html?m=register&rec='.$rec_id);}     
		} else{
    ...
    public function usersaveOp() {
        ...
        if(!isset($member_info['error'])) {
            ...
            $_POST['ref_url']   = (strstr($_POST['ref_url'],'logout')=== false && !empty($_POST['ref_url']) ? $_POST['ref_url'] : urlMember('m_information', 'member'));
            if ($_GET['inajax'] == 1){
                showDialog('',$_POST['ref_url'] == '' ? 'reload' : $_POST['ref_url'],'js');
            } else {
                redirect($_POST['ref_url']);
    ...
    public function bind_emailOp() {
       ...
       showMessage('邮箱设置成功','index.php?m=m_security&a=index');
