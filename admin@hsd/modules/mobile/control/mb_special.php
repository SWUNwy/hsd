...
class mb_specialControl extends SystemControl{
    ...
    public function special_listOp() {
        ...
        if($_GET['keyword'] != ""){
        	...
        	$condition['special_desc'] = array('like',"%{$keyword}%");
        }
        
        $mb_special_list = $model_mb_special->getMbSpecialList($condition, 15,'special_sort desc');       
     ...
     * 编辑专题排序
     */
    public function update_special_sortOp() {
        $model_mb_special = Model('mb_special');

        $param = array();
        $param['special_sort'] = $_GET['value'];
        $result = $model_mb_special->editMbSpecial($param, $_GET['id']);

        $data = array();
        if($result) {
            $this->log('修改手机专题排序' . '[ID:' . $result. ']', 1);
            $data['result'] = true;
        } else {
            $this->log('修改手机专题排序' . '[ID:' . $result. ']', 0);
            $data['result'] = false;
            $data['message'] = '保存失败';
        }
        echo json_encode($data);die;
    ...
    public function goods_listOp() {
        ...
        $condition = array();
		$condition['store_id'] = 21;
		if( $_POST['keyword']!="")
		{
			$_SESSION['keyword']=$_POST['keyword'];
			$condition['goods_name'] = array('like', '%' . $_SESSION['keyword'] . '%');
     ...
     * 商品列表 
     */
    public function goods_list_selectOp() {
        $gc_id_2=$_POST['gc_id_2'];
        $condition = array();
        if($gc_id_2>0)
        {
            $_SESSION['gc_id_2']=$gc_id_2;
        }
		$condition['store_id'] = 21;
        $model_goods = Model('goods');       
        $condition['gc_id_2'] = $_SESSION['gc_id_2'];
