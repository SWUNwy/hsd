...
class linkControl extends SystemControl{
	...
	public function link_delOp(){
		...
		if (intval($_GET['link_id']) > 0){
			...
			dkcache('link',null);
			showMessage(删除成功,'index.php?m=link&a=link');
		}else {
			showMessage(删除失败,'index.php?m=link&a=link');
		}
	}
		...
		if ($_POST['form_submit'] == 'ok'){
			...
			if ($error != ''){
				...
				if ($result){
					...
					$url = array(
						array(
							'url'=>'index.php?m=link&a=link_add',
							'msg'=>再次发布,
						),
						array(
							'url'=>'index.php?m=link&a=link',
							'msg'=>返回列表,
						)
					);
		...
		if ($_POST['form_submit'] == 'ok'){
			...
			if ($error != ''){
				...
				if ($result){
					...
					$url = array(
						array(
							'url'=>'index.php?m=link&a=link_edit&link_id='.intval($_POST['link_id']),
							'msg'=>再次编辑
						),
						array(
							'url'=>'index.php?m=link&a=link',
