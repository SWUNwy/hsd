...
class memberControl extends SystemControl{
	...
	private $links = array(
	    array('url'=>'m=member&a=index','lang'=>'member_index_manage'),
		array('url'=>'m=member&a=predeposit_add','lang'=>'member_index_predeposit'), 
    );
        ...
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                if ($result){
                    $url = array(
                    array(
                    'url'=>'index.php?m=member&a=member',
                    'msg'=>$lang['member_edit_back_to_list'],
                    ),
                    array(
                    'url'=>'index.php?m=member&a=member_edit&member_id='.intval($_POST['member_id']),
        ...
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                if ($result){
                    ...
                    array(
                    'url'=>'index.php?m=member&a=member_add',
    ...
    public function get_xmlOp() {
        ...
        foreach ($member_list as $value) {
            $param = array();
            $param['operation'] = "<a class='btn blue' href='index.php?m=member&a=member_edit&member_id=" . $value['member_id'] . "'><i class='fa fa-pencil-square-o'></i>编辑</a>
    ...
    public function export_csvOp() {
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){   //显示下载链接
                ...
                Tpl::output('murl','index.php?m=member&a=index');
				Tpl::setDirquna('system');
	...
	public function predeposit_addOp(){
		if (chksubmit()){
			...
			if (!is_array($member_info) || count($member_info)<=0){
				showMessage(Language::get('admin_points_userrecord_error'),'index.php?m=member&a=predeposit_add','','error');
			}
			...
			if ($_POST['operatetype'] == 2 && $money > $available_predeposit){
				showMessage(('预存款不足，会员当前预存款').$available_predeposit,'index.php?m=member&a=predeposit_add','','error');
			}
			if ($_POST['operatetype'] == 3 && $money > $available_predeposit){
				showMessage(('可冻结预存款不足，会员当前预存款').$available_predeposit,'index.php?m=member&a=predeposit_add','','error');
			}
			if ($_POST['operatetype'] == 4 && $money > $freeze_predeposit){
				showMessage(('可恢复冻结预存款不足，会员当前冻结预存款').$freeze_predeposit,'index.php?m=member&a=predeposit_add','','error');
			}
			$model_pd = Model('predeposit');
			$order_sn = $model_pd->makeSn();
			$admininfo = $this->getAdminInfo();
			$log_msg = "操作会员[".$member_info['member_name']."]预存款，金额为".$money.",编号为".$order_sn;
			$admin_m="sys_add_money";
			...
			try {
				...
				$model_pd->commit();
				$this->log($log_msg,1);
				showMessage('操作成功','index.php?m=member&a=predeposit_add');
			} catch (Exception $e) {
				$model_pd->rollback();
				$this->log($log_msg,0);
				showMessage($e->getMessage(),'index.php?m=member&a=predeposit_add','html','error');
			}
		}else{
		   //输出子菜单
