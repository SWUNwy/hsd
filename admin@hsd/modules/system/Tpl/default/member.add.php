		...
		(
			{
				...
				{
					if (data.status == 1){
						ajax_form('cutpic','<?php echo $lang['nc_cut'];?>','<?php echo ADMIN_SITE_URL?>/index.php?m=common&a=pic_cut&type=member&x=120&y=120&resize=1&ratio=1&url='+data.url,690);
					}else{
						alert(data.msg);
					}
    ...
    $('#user_form').validate({
        ...
        rules : {
			member_name: {
				...
				maxlength: 20,
				remote   : {
                    url :'index.php?m=member&a=ajax&branch=check_user_name',
                    type:'get',
                    data:{
                        user_name : function(){
