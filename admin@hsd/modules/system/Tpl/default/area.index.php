...
$(function(){
    $("#flexigrid").flexigrid({
        url: 'index.php?m=area&a=get_xml',
        colModel : [
...
function fg_operate(name, grid) {
    if (name == 'add') {
        window.location.href = 'index.php?m=area&a=add&parent_id='+cur_parent_id;
    }else if (name == 'delete') {
...
function fg_show_children(area_id,parent_id) {
	$("#flexigrid").flexOptions({url: 'index.php?m=area&a=get_xml&parent_id='+area_id}).flexReload();
...
function fg_up() {
	...
	$("#flexigrid").flexOptions({url: 'index.php?m=area&a=get_xml&parent_id='+his_parent_ids[his_parent_ids.length-1]}).flexReload();
