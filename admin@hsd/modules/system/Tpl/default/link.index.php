    ...
    <table class="flex-table">
      ...
      <tbody>
        ...
        <tr data-id="<?php echo $v['link_id'];?>">
          ...
          <a class="btn blue" href="index.php?m=link&a=link_edit&link_id=<?php echo $v['link_id'];?>"><i class="fa fa-pencil-square-o"></i><?php echo $lang['nc_edit'];?></a>
          </td>
...
function submit_delete(id){
	...
	if(confirm('删除后将不能恢复，确认删除这 ' + id.length + ' 项吗？')){
		id = id.join(',');
        window.location.href = 'index.php?m=link&a=link_del&link_id='+id;
    }
}
function fg_operation(name, bDiv) {
    if (name == 'add') {
        window.location.href = 'index.php?m=link&a=link_add';
    }else if (name == 'delete') { 
