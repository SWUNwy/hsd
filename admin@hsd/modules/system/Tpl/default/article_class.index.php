      ...
      <tbody id="treet1">
        ...
        <tr <?php if ($v['deep'] != 1) {?>style="display:none;"<?php }?> nctype="<?php echo $v['ac_parent_id'];?>">
          <td class="sign">
            ...
            <?php } ?></td>
          <td class="handle">
            <?php if($v['ac_code'] == ''){?><a href="javascript:if(confirm('<?php echo $lang['article_class_index_ensure_del'];?>'))window.location = 'index.php?m=article_class&a=article_class_del&ac_id=<?php echo $v['ac_id'];?>';" class="btn red"><i class="fa fa-trash-o"></i><?php echo $lang['nc_del'];?></a>
            <?php }?>
            <span class="btn"><em><i class="fa fa-cog"></i>设置<i class="arrow"></i></em>
            <ul><li><a href="index.php?m=article_class&a=article_class_edit&ac_id=<?php echo $v['ac_id'];?>">编辑分类信息</a></li>
              <?php if ($v['deep'] == 1) {?>
              <li><a href="index.php?m=article_class&a=article_class_add&ac_parent_id=<?php echo $v['ac_id'];?>"><?php echo $lang['nc_add_sub_class'];?>分类</a></li>
              <?php }?>
            </ul>
            </span>
            </td>
...
function fg_operation(name, bDiv) {
    if (name == 'add') {
        window.location.href = 'index.php?m=article_class&a=article_class_add';
    }
}
</script> 
