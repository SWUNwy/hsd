<?php defined('ByShopKJYP') or exit('Access Invalid!');?>
	...
	<div class="ncsc-form-goods-pic" style="margin-top: 10px;">
	    <div class="container">
	      ...
	      <div class="ncsc-goodspic-list">
	        ...
	        <div class="ncsc-select-album">
	          <a class="ncbtn" href="index.php?m=store_album&a=pic_list&item=goods_image&color_id=<?php echo $value['sp_value_id'];?>" nctype="select-<?php echo $value['sp_value_id'];?>"><i class="icon-picture"></i>从图片空间选择</a>
	          <a href="javascript:void(0);" nctype="close_album" class="ncbtn ml5" style="display: none;"><i class=" icon-circle-arrow-up"></i>关闭相册</a>
	          
	          <a class="ncbtn" href="index.php?m=store_album&a=pic_list_1&item=goods" nctype="show_image"><i class="icon-picture"></i>从图片空间选择</a> 
	          <a href="javascript:void(0);" nctype="close_album" class="ncbtn ml5" style="display: none;"><i class="icon-circle-arrow-up"></i>关闭相册</a>
	          
	        </div>
	        <div id="demo" nctype="album-<?php echo $value['sp_value_id'];?>"></div>
	      </div>
	      <?php }?>
	      <?php }?>
	    </div>
...
<script type="text/javascript" src="<?php echo ADMIN_RESOURCE_URL;?>/js/goods_add.step3.js" charset="utf-8"></script>
<script>
...
$(function(){   
    ...
    $('a[nctype="show_image"]').ajaxContent({
        event:'click', //mouseover
        loaderType:"img",
        loadingMsg:SHOP_TEMPLATES_URL+"/images/loading.gif",
        target:'div[nctype="album-<?php echo $value['sp_value_id'];?>"]'
