  ...
  <div class="fixed-bar">
    <div class="item-title">
      ...
      <ul class="tab-base nc-row">
        ...
        <li><a href="index.php?m=predeposit&a=pd_cash_list"><?php echo $lang['admin_predeposit_cashmanage']; ?></a></li>
...
$(function(){
    ...
    $('#ncsubmit').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?m=predeposit&a=get_xml&'+$("#formSearch").serialize(),query:'',qtype:''}).flexReload();
    });
    // 高级搜索重置
    $('#ncreset').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?m=predeposit&a=get_xml'}).flexReload();
        $("#formSearch")[0].reset();
    });
    
    $("#flexigrid").flexigrid({
        url: 'index.php?m=predeposit&a=get_xml',
        colModel : [
...
function fg_csv(ids) {
    id = ids.join(',');
    window.location.href = $("#flexigrid").flexSimpleSearchQueryString()+'&a=export_step1&id=' + id;
}

function fg_delete(id) {
    if(confirm('删除后将不能恢复，确认删除这项吗？')){
        $.getJSON('index.php?m=predeposit&a=recharge_del', {id:id}, function(data){
            if (data.state) {
