...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title"><a class="back" href="index.php?m=member&a=member" title="返回列表"><i class="fa fa-arrow-circle-o-left"></i></a>
      <div class="subject">
...
$(function(){
	...
	function ajaxFileUpload()
	{
		...
		(
			{
				url : '<?php echo ADMIN_SITE_URL?>/index.php?m=common&a=pic_upload&form_submit=ok&uploadpath=<?php echo ATTACH_AVATAR;?>',
				secureuri:false,
				fileElementId:'_pic',
				dataType: 'json',
				...
				{
					if (data.status == 1){
						ajax_form('cutpic','<?php echo $lang['nc_cut'];?>','<?php echo ADMIN_SITE_URL?>/index.php?m=common&a=pic_cut&type=member&x=120&y=120&resize=1&ratio=1&filename=<?php echo UPLOAD_SITE_URL.'/'.ATTACH_AVATAR;?>/avatar_<?php echo $_GET['member_id'];?>.jpg&url='+data.url,690);
					}else{
						alert(data.msg);
					}
    ...
    $('#user_form').validate({
        ...
        rules : {
            ...
            member_email   : {
                ...
                email : true,
				remote   : {
                    url :'index.php?m=member&a=ajax&branch=check_email',
                    type:'get',
                    data:{
                        user_name : function(){
