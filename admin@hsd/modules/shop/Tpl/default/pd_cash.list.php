  ...
  <div class="fixed-bar">
    <div class="item-title">
      ...
      <ul class="tab-base nc-row">
        <li><a href="index.php?m=predeposit&a=predeposit"><?php echo $lang['admin_predeposit_rechargelist']?></a></li>
        <li><a href="JavaScript:void(0);" class="current"><?php echo $lang['admin_predeposit_cashmanage']; ?></a></li>
        <li><a href="index.php?m=predeposit&a=pd_log_list"><?php echo $lang['nc_member_predepositlog'];?></a></li>
      </ul>
    </div>
  </div>
  <!-- 操作说明 -->
...
$(function(){
    ...
    $('#ncsubmit').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?m=predeposit&a=get_cash_xml&'+$("#formSearch").serialize(),query:'',qtype:''}).flexReload();
    });
    // 高级搜索重置
    $('#ncreset').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?m=predeposit&a=get_cash_xml'}).flexReload();
        $("#formSearch")[0].reset();
    });
    
    $("#flexigrid").flexigrid({
        url: 'index.php?m=predeposit&a=get_cash_xml',
        colModel : [
...
function fg_csv(ids) {
    id = ids.join(',');
    window.location.href = $("#flexigrid").flexSimpleSearchQueryString()+'&a=export_cash_step1&id=' + id;
}
function fg_delete(id) {
    if(confirm('删除后将不能恢复，确认删除这项吗？')){
        $.getJSON('index.php?m=predeposit&a=pd_cash_del', {id:id}, function(data){
            if (data.state) {
