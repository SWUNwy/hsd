...
class vr_refundControl extends SystemControl{
    ...
    private $links = array(
            array('url'=>'m=vr_refund&a=refund_manage','text'=>'待处理'),
            array('url'=>'m=vr_refund&a=refund_all','text'=>'所有记录'),
    );
    ...
    public function get_all_xmlOp() {
        ...
        foreach ($refund_list as $k => $refund_info) {
            ...
            if ($_GET['waiting'] == 1 && $refund_info['admin_state'] == 1) {
                $list['operation'] = "<a class=\"btn orange\" href=\"index.php?m=vr_refund&a=edit&refund_id={$refund_info['refund_id']}\"><i class=\"fa fa-gavel\"></i>处理</a>";
            } else {
                $list['operation'] = "<a class=\"btn green\" href=\"index.php?m=vr_refund&a=view&refund_id={$refund_info['refund_id']}\"><i class=\"fa fa-list-alt\"></i>查看</a>";
            }
    ...
    public function editOp() {
        ...
        if (chksubmit()) {
            ...
            if ($state) {
                ...
                $param['param'] = array(
                    'refund_url' => urlShop('m_vr_refund', 'view', array('refund_id' => $refund['refund_id'])),
                    'refund_sn' => $refund['refund_sn']
                );
                QueueClient::push('sendMemberMsg', $param);

                $this->log('虚拟订单退款审核，退款编号'.$refund['refund_sn']);
                showMessage(Language::get('nc_common_save_succ'),'index.php?m=vr_refund&a=refund_manage');
            } else {
                showMessage(Language::get('nc_common_save_fail'));
            }
        }
