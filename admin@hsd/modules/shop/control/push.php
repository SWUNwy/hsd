...
class pushControl extends SystemControl{
  ...
  /**
   * [pushDingPayOp description]
   * 推送智付订单至智付报关
   * @return [type] [description]
   */
  public function pushDingPayOp(){
    if($_GET['order_id']) {
            $condition['order_id'] =$_GET['order_id'];
        }
    $model_order= Model('order');
    ...
    // die();
    if($order['payment_code'] !== "dingpay"){
      $result['state'] = false;
      $result['msg'] = "这不是智付支付订单";
      echo json_encode($result);
      die();
    }

    $model_dingpay = Model("dingpay");
    $res = $model_dingpay->SendPayInfo($order);
    if($res){
      $result = array();
      $result['state'] = true;
      $result['msg'] = '推送成功';
