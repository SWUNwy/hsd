...
class redpacketControl extends SystemControl{
    ...
    public function __construct(){
        ...
        if (C('redpacket_allow') != 1){
            showDialog('需开启“平台红包”功能','index.php?m=operation','succ');
        }
        ...
        if (chksubmit()){
            ...
            if ($error){
                ...
                if($rs){
                    ...
                    showDialog(L('nc_common_save_succ'),'index.php?m=redpacket&a=rptlist','succ');
                }else{
    ...
    public function rpteditOp(){
        ...
        if ($t_id <= 0){
            showDialog(L('param_error'),'index.php?m=redpacket&a=rptlist');
        }
        $model_redpacket = Model('redpacket');
        //查询模板信息
        $where = array();
    ...
    public function export_step1Op(){
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){//显示下载链接
                ...
                Tpl::output('murl','index.php?m=redpacket&a=rptinfo&tid='.$t_id);
				Tpl::setDirquna('shop');
