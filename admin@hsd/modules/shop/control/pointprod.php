...
EOB;
    ...
    public function prod_addOp(){
        ...
        if (chksubmit()){
            ...
            if($state){
                ...
                showDialog(L('admin_pointprod_add_success'),'index.php?m=pointprod&a=pointprod','succ');
            }
        }
        //模型实例化
        $where = array();
    ...
    public function prod_editOp(){
        ...
        if (!$pg_id){
            showDialog(L('admin_pointprod_parameter_error'),'index.php?m=pointprod&a=pointprod','error');
        }
        $model_pointprod = Model('pointprod');
        //查询礼品记录是否存在
        $prod_info = $model_pointprod->getPointProdInfo(array('pgoods_id'=>$pg_id));
        if (!$prod_info){
            showDialog(L('admin_pointprod_record_error'),'index.php?m=pointprod&a=pointprod','error');
        }
        if (chksubmit()){
            //验证表单
            $obj_validate = new Validate();
            ...
            if($state){
                ...
                showDialog(L('admin_pointprod_edit_success'),'index.php?m=pointprod&a=pointprod','succ');
            }
        }else {
            $where = array();
            $where['upload_type'] = '6';
...
Tpl::setDirquna('shop');
    ...
    public function order_dropOp(){
        ...
        if ($data['state']){
            showDialog(L('admin_pointorder_del_success'),'index.php?m=pointprod&a=pointorder_list','succ');
        } else {
            showDialog($data['msg'],'index.php?m=pointprod&a=pointorder_list','error');
        }
    }
    ...
    public function order_cancelOp(){
        ...
        if ($data['state']){
            showDialog(L('admin_pointorder_cancel_success'),'index.php?m=pointprod&a=pointorder_list','succ');
        }else {
            showDialog($data['msg'],'index.php?m=pointprod&a=pointorder_list','error');
        }
    }
    ...
    public function order_shipOp(){
        ...
        if ($order_id <= 0){
            showDialog(L('admin_pointorder_parameter_error'),'index.php?m=pointprod&a=pointorder_list','error');
        }
        $model_pointorder = Model('pointorder');
        //获取订单状态
        ...
        if (!$order_info){
            showDialog(L('admin_pointorderd_record_error'),'index.php?m=pointprod&a=pointorder_list','error');
        }
        if (chksubmit()){
            $obj_validate = new Validate();
            ...
            $error = $obj_validate->validate();
            if ($error != ''){
                showDialog(L('error').$error,'index.php?m=pointprod&a=pointorder_list','error');
            }
            //发货
            $data = $model_pointorder->shippingPointOrder($order_id, $_POST, $order_info);
            if ($data['state']){
                showDialog('发货修改成功','index.php?m=pointprod&a=pointorder_list','succ');
            }else {
                showDialog($data['msg'],'index.php?m=pointprod&a=pointorder_list','error');
            }
        } else {
