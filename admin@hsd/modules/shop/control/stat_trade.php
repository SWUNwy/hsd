...
class stat_tradeControl extends SystemControl{
    private $links = array(
        array('url'=>'m=stat_trade&a=income','lang'=>'stat_sale_income'),
        array('url'=>'m=stat_trade&a=predeposit','lang'=>'stat_predeposit'),
        array('url'=>'m=stat_trade&a=sale','lang'=>'stat_sale')
    );
    ...
    public function get_income_xmlOp(){
        ...
        if (!empty($list) && is_array($list)){
            ...
            foreach ($list as $k => $v){
                $out_array = array();
                $out_array['operation'] = '';
                //$out_array['operation'] = '<a class="btn green" href="index.php?m=stat_trade&a=sale&search_type=month&searchmonth_month='.$_GET['search_month'].
