...
class informControl extends SystemControl{
    private $links = array(
            array('url'=>'m=inform&a=inform_list','text'=>'未处理'),
            array('url'=>'m=inform&a=inform_handled_list','text'=>'已处理'),
            array('url'=>'m=inform&a=inform_subject_type_list','text'=>'类型设置'),
            array('url'=>'m=inform&a=inform_subject_list','text'=>'主题设置')
    );
    ...
    public function get_xmlOp(){
        ...
        if ($inform_list && is_array($inform_list)) {
            ...
            foreach ($inform_list as $k => $v){
                ...
                $list['operation'] = "<a class=\"btn orange\" href=\"index.php?m=inform&a=show_handle_page&inform_id={$v['inform_id']}&inform_goods_name={$v['inform_goods_name']}\"><i class=\"fa fa-gavel\"></i>处理</a>";
                } else {
                    $list['operation'] = '--';
                }
...
Tpl::setDirquna('shop');
    ...
    public function inform_subject_type_saveOp(){
        ...
        if ($error != ''){
            ...
            showMessage(Language::get('nc_common_save_succ'),'index.php?m=inform&a=inform_subject_type_list');
        }
    }
    ...
    public function inform_subject_type_dropOp(){
        ...
        if(empty($inform_type_id)) {
            showMessage(Language::get('param_error'),'index.php?m=inform');
        }
        ...
        showMessage(Language::get('nc_common_del_succ'),'index.php?m=inform&a=inform_subject_type_list');
    ...
    public function inform_subject_saveOp(){
        ...
        if ($error != ''){
            ...
            showMessage(Language::get('nc_common_save_succ'),'index.php?m=inform&a=inform_subject_list');
        }
    }
    ...
    public function inform_subject_dropOp(){
        ...
        showMessage(Language::get('nc_common_del_succ'),'index.php?m=inform&a=inform_subject_list');
    }
    ...
    public function inform_handleOp(){
        ...
        if($model_inform->updateInform($update_array,$where_array)) {
            ...
            showMessage(Language::get('nc_common_op_succ'),'index.php?m=inform&a=inform_list');
        }
        else {
