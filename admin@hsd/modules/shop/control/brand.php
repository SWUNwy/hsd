    ...
    public function get_xmlOp() {
        ...
        foreach ($brand_list as $value) {
            ...
            if ($_GET['type'] == 'apply') {
                ...
                $operation .= "<a class='btn blue' href='index.php?m=brand&a=brand_edit&brand_id=" . $value['brand_id'] . "'><i class='fa fa-pencil-square-o'></i>编辑</a>";
            }
        ...
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                if ($result){
                    $url = array(
                        array(
                            'url'=>'index.php?m=brand&a=brand_add',
                            'msg'=>$lang['brand_add_again'],
                        ),
                        array(
                            'url'=>'index.php?m=brand&a=brand',
        ...
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                if ($result){
                    ...
                    $url = array(
                        array(
                            'url'=>'index.php?m=brand&a=brand_edit&brand_id='.intval($_POST['brand_id']),
    ...
    public function export_step1Op(){
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){   //显示下载链接
                ...
                Tpl::output('list',$array);
                Tpl::output('murl','index.php?m=brand&a=brand');
                                //网 店 运 维shop wwi.com
        Tpl::setDirquna('shop');
                Tpl::showpage('export.excel');
            }else{  //如果数量小，直接下载
