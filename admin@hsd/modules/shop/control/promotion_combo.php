...
class promotion_comboControl extends SystemControl{
    private $links = array(
        array('url'=>'m=promotion_combo&a=index','text'=>'商品列表'),
        array('url'=>'m=promotion_combo&a=combo_quota_list','text'=>'套餐列表'),
        array('url'=>'m=promotion_combo&a=combo_setting','text'=>'设置')
    );
    public function __construct(){
        ...
        if (intval($_GET['promotion_allow']) !== 1 && intval(C('promotion_allow')) !== 1){
            $url = array(
                ...
                array(
                    'url'=>'index.php?m=promotion_combo&promotion_allow=1',
                    'msg'=>L('open'),
                )
            );
