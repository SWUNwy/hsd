...
class returnControl extends SystemControl{
    ...
    private $links = array(
            array('url'=>'m=return&a=return_manage','text'=>'待处理'),
            array('url'=>'m=return&a=return_all','text'=>'所有记录')
    );
    ...
    public function get_manage_xmlOp() {
        ...
        foreach ($return_list as $k => $return_info) {
            ...
            $list['operation'] = "<a class=\"btn orange\" href=\"index.php?m=return&a=edit&return_id={$return_info['refund_id']}\"><i class=\"fa fa-gavel\"></i>处理</a>";
    ...
    public function get_all_xmlOp() {
        ...
        foreach ($return_list as $k => $return_info) {
            ...
            $list['operation'] .= "<a class=\"btn green\" href=\"index.php?m=return&a=view&return_id={$return_info['refund_id']}\"><i class=\"fa fa-list-alt\"></i>查看</a>";
            $list['refund_sn'] = $return_info['refund_sn'];
            $list['refund_amount'] = ncPriceFormat($return_info['refund_amount']);
            if(!empty($return_info['pic_info'])) {
                $info = unserialize($return_info['pic_info']);
    ...
    public function editOp() {
        ...
        if (chksubmit()) {
            ...
            if ($state) {
                ...
                $param['param'] = array(
                    'refund_url' => urlShop('m_refund', 'view', array('return_id' => $return['refund_id'])),
                    'refund_sn' => $return['refund_sn']
                );
                QueueClient::push('sendMemberMsg', $param);

                showMessage(Language::get('nc_common_save_succ'),'index.php?m=return&a=return_manage');
            } else {
                showMessage(Language::get('nc_common_save_fail'));
            }
        }
