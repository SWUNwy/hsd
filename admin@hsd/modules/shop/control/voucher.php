...
class voucherControl extends SystemControl{
    ...
    public function __construct(){
        ...
        if (C('voucher_allow') != 1 || C('points_isuse')!=1){
            showMessage(Language::get('admin_voucher_unavailable'),'index.php?m=operation&a=point','html','succ',1,4000);
        }
        ...
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                if ($rs){
                    ...
                    showMessage(Language::get('nc_common_save_succ'),'index.php?m=voucher&a=pricelist');
                }else {
                    showMessage(Language::get('nc_common_save_fail'),'index.php?m=voucher&a=priceadd');
                }
            }
        }else {
    ...
    public function priceeditOp(){
        ...
        if ($id <= 0){
            showMessage(Language::get('param_error'),'index.php?m=voucher&a=pricelist');
        }
        $model = Model();
        if (chksubmit()){
            $obj_validate = new Validate();
            ...
            if ($error != ''){
                ...
                if ($rs){
                    ...
                    showMessage(Language::get('nc_common_save_fail'),'index.php?m=voucher&a=pricelist');
                }
            }
        }else {
    ...
    public function pricedropOp(){
        ...
        if ($rs){
            ...
            showMessage(Language::get('nc_common_del_succ'),'index.php?m=voucher&a=pricelist');
        }else{
            showMessage(Language::get('nc_common_del_fail'),'index.php?m=voucher&a=pricelist');
        }
    }
    ...
    public function templateeditOp(){
        ...
        if ($t_id <= 0){
            showMessage(Language::get('param_error'),'index.php?m=voucher&a=templatelist','','error');
        }
        $model = Model('voucher');
        //查询模板信息
        $param = array();
        ...
        if (empty($t_info)){
            showMessage(Language::get('param_error'),'index.php?m=voucher&a=templatelist','html','error');
        }
        if(chksubmit()){
            ...
            if($rs){
                ...
                showMessage(Language::get('nc_common_save_succ'),'index.php?m=voucher&a=templatelist','succ');
            }else{
                showMessage(Language::get('nc_common_save_fail'),'index.php?m=voucher&a=templatelist','error');
            }
        }else{            
    ...
    private function show_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
            case 'voucher':
                $menu_array = array(
                3=>array('menu_key'=>'templatelist','menu_name'=>Language::get('admin_voucher_template_manage'), 'menu_url'=>'index.php?m=voucher&a=templatelist'),
                2=>array('menu_key'=>'quotalist','menu_name'=>Language::get('admin_voucher_quota_manage'), 'menu_url'=>'index.php?m=voucher&a=quotalist'),
                5=>array('menu_key'=>'pricelist','menu_name'=>Language::get('admin_voucher_pricemanage'), 'menu_url'=>'index.php?m=voucher&a=pricelist'),
                4=>array('menu_key'=>'setting','menu_name'=>Language::get('admin_voucher_setting'), 'menu_url'=>'index.php?m=voucher&a=setting'),
                );
                break;
            case 'priceedit':
                $menu_array = array(
                1=>array('menu_key'=>'setting','menu_name'=>Language::get('admin_voucher_setting'), 'menu_url'=>'index.php?m=voucher&a=setting'),
                2=>array('menu_key'=>'pricelist','menu_name'=>Language::get('admin_voucher_pricemanage'), 'menu_url'=>'index.php?m=voucher&a=pricelist'),
                3=>array('menu_key'=>'priceedit','menu_name'=>Language::get('admin_voucher_priceedit'), 'menu_url'=>'')
                );
                break;
            case 'templateedit':
                $menu_array = array(
                1=>array('menu_key'=>'templatelist','menu_name'=>Language::get('admin_voucher_template_manage'), 'menu_url'=>'index.php?m=voucher&a=templatelist'),
