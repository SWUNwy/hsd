...
class store_gradeControl extends SystemControl{
            ...
            if (!empty($_POST['check_sg_id'])){
                if (is_array($_POST['check_sg_id'])){
                    ...
                    foreach ($_POST['check_sg_id'] as $k => $v){
                        ...
                        //判断是否默认等级，默认等级不能删除
                        if ($v == 1){
                            //showMessage('默认等级不能删除 ','index.php?m=store_grade&a=store_grade');
                            showMessage($lang['default_store_grade_no_del'],'index.php?m=store_grade&a=store_grade');
                        }
        ...
        if (chksubmit()){
            ...
            if ($error != ''){
                ...
                if ($result){
                    ...
                    showMessage($lang['nc_common_save_succ'],'index.php?m=store_grade&a=store_grade');
                }else {
    ...
    public function store_grade_editOp(){
        ...
        if (chksubmit()){
            if (!$_POST['sg_id']){
                showMessage($lang['grade_parameter_error'],'index.php?m=store_grade&a=store_grade');
            }
            /**
             * 验证
             */
            ...
            if ($error != ''){
                ...
                if (!$this->checkGradeName(array('sg_name'=>trim($_POST['sg_name']),'sg_id'=>intval($_POST['sg_id'])))){
                    showMessage($lang['now_store_grade_name_is_there'],'index.php?m=store_grade&a=store_grade_edit&sg_id='.intval($_POST['sg_id']));
                }
                //验证级别是否存在
                if (!$this->checkGradeSort(array('sg_sort'=>trim($_POST['sg_sort']),'sg_id'=>intval($_POST['sg_id'])))){
                    showMessage($lang['add_gradesortexist'],'index.php?m=store_grade&a=store_grade_edit&sg_id='.intval($_POST['sg_id']));
                }
                $update_array = array();
    ...
    public function store_grade_delOp(){
        ...
        if (intval($_GET['sg_id']) > 0){
            ...
            if (!$this->isable_delGrade(intval($_GET['sg_id']))){
                showMessage($lang['del_gradehavestore'],'index.php?m=store_grade&a=store_grade');
            }
            /**
             * 删除分类
             */
            ...
            showMessage($lang['nc_common_del_succ'],'index.php?m=store_grade&a=store_grade');
        }else {
            showMessage($lang['nc_common_del_fail'],'index.php?m=store_grade&a=store_grade');
        }
    }
