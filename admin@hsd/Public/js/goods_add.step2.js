$(function(){
    ...
    $('#goods_image').fileupload({
        dataType: 'json',
        url: 'index.php?m=goods_add&a=image_upload&upload_type=uploadedfile',
	...
	$('a[nctype="show_desc1"]').unbind().ajaxContent({
        event:'click', //mouseover
        loaderType:"img",
        loadingMsg:SHOP_TEMPLATES_URL+"/images/loading.gif",
        target:'#des_demo'
        ...
        $(this).hide();
        $('a[nctype="del_desc1"]').show();
    });
    $('a[nctype="del_desc1"]').click(function(){
        $('#des_demo').html('');
        $(this).hide();
        $('a[nctype="show_desc1"]').show();
    });
    ...
    $('#add_album').fileupload({
        dataType: 'json',
        url: SITEURL+'/index.php?m=store_goods_add&a=image_upload',
    ...
    $('a[nctype="mb_add_img"]').click(function(){
        $('a[nctype="meat_cancel"]').click();
        $('div[nctype="mea_img"]').show().load('index.php?m=store_album&a=pic_list&item=mobile');
    });
    // 关闭 图片选择
    ...
    $('div[nctype="mobile_pannel"]').on('click', 'a[nctype="mp_rpl"]', function(){
        $('a[nctype="meat_cancel"]').click();
        $('div[nctype="mea_img"]').show().load('index.php?m=store_album&a=pic_list&item=mobile&type=replace');
    });
    // 插图图片
