...
$lang['image_thumb_tool_tips']	= '商城預設使用GD庫生成縮略圖，GD使用廣泛但占用系統資源較多，ImageMagick速度快系統資源占用少，但需要伺服器有執行命令行命令的權限。可到 http://www.imagemagick.org 下載安裝，如改用ImageMagick，可編輯data/config/config.php檔案(用EditPlus): <br/>$config[\'thumb\'][\'cut_type\'] = \'im\';<br/>$config[\'thumb\'][\'impath\'] = \'ImageMagick下convert工具所在路徑\'; 如：<br/>$config[\'thumb\'][\'impath\'] = \'/usr/local/ImageMagick/bin\';';
...
$lang['ftp_access_url_tip'] = '支持 HTTP 和 FTP 協議，結尾不要加斜杠“/”；如果使用 FTP 協議，FTP 伺服器必需支持 PASV 模式，為了安全起見，使用 FTP 連接的帳號不要設置可寫權限和列表權限。如果是根目錄，直接輸入域名即可，如“http://www.kjyp360.com”，如果非根目錄，需要填寫域名+目錄形式，如“http://www.kjyp360.com/upload/shopnc”';
