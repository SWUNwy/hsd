...
define('BASE_CORE_PATH',BASE_ROOT_PATH.'/Cored');
define('BASE_DATA_PATH',BASE_ROOT_PATH.'/Apps');
define("BASE_UPLOAD_PATH", BASE_ROOT_PATH . "/Apps/upload");
define("BASE_RESOURCE_PATH", BASE_ROOT_PATH . "/Apps/Public");
...
define('DIR_WAP','wap');
define('DIR_RESOURCE','Apps/Public');
define('DIR_UPLOAD','Apps/upload');
...
define('ADMIN_MODULES_WXSHOP', 'modules/wxshop');
/*
 * 商家入驻状态定义
 */
//新申请
 ...
 * 初始化
 */
if (!@include(BASE_DATA_PATH.'/Conf/config.php')) exit('config.php isn\'t exists!');
...
define('SHOP_SITE_URL_WEIDIAN', $config['shop_site_url']).'/weidian';
...
define('WEIDIAN_SITE_URL', $config['weidian_site_url']);
...
define('BASE_RESOURCE_PATH',BASE_DATA_PATH.'/Public');
...
define('LOGIN_RESOURCE_SITE_URL',MEMBER_SITE_URL.'/Public');
...
$_GET['m'] = is_string($_GET['m']) ? strtolower($_GET['m']) : (is_string($_POST['m']) ? strtolower($_POST['m']) : null);
$_GET['a'] = is_string($_GET['a']) ? strtolower($_GET['a']) : (is_string($_POST['a']) ? strtolower($_POST['a']) : null);
...
//统一ACTION
$_GET['m'] = preg_match('/^[\w]+$/i',$_GET['m']) ? $_GET['m'] : 'index';
